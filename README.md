...
1. [chromatic.com](https://www.chromatic.com)에서 `Sign in with GitHub` → 이 저장소
   (`Figam-Dev-Variable-Tools/Auto-Builder`)로 프로젝트를 생성한다.
2. 프로젝트 설정에서 **Project Token**(`chpt_`로 시작)을 발급받는다.
3. GitHub 저장소 **Settings → Secrets and variables → Actions → New repository secret**
   에서 이름 `CHROMATIC_PROJECT_TOKEN`으로 등록한다.
   (직접 링크: `.../Auto-Builder/settings/secrets/actions`)
4. push하면 [.github/workflows/chromatic.yml](.github/workflows/chromatic.yml)이 자동 배포한다
   (토큰 미등록 시 배포를 건너뛰고 빌드 검증만 수행하는 조건 분기 내장).
5. 로컬 수동 배포 (오너 셸은 PowerShell — bash `VAR=x cmd` 문법은 실패):
   ```powershell
   $env:CHROMATIC_PROJECT_TOKEN='chpt_...'; pnpm chromatic
   ```
   (토큰 하드코딩 금지 — 환경변수로만.) 참고: 로컬 스크립트는 `--exit-zero-on-changes`라
   시각 변경이 있어도 종료코드 0이지만, CI의 `chromaui/action`은 미리뷰 변경 시 잡이 red로
   표시되므로 첫 빌드 baseline을 Chromatic에서 Accept해야 한다.
...
- [ ] **Chromatic**: chromatic.com에서 프로젝트 생성 → Project Token 발급 → GitHub 저장소
      Settings → Secrets and variables → Actions → New repository secret에
      `CHROMATIC_PROJECT_TOKEN` 등록 (이후 push 시 자동 배포)
...
- [ ] **story.to.design / Storybook Connect 연결** (Connect는 Chromatic **Public** 필요 — 가이드 §3-3·§5·§6)
- [ ] **figma-story-tools 발행** — 오너 npm 계정으로 `packages/figma-story-tools` 발행.
      `pnpm whoami`로 로그인 확인 → `pnpm build:manifest` →
      `pnpm --dir packages/figma-story-tools pack`(사전 확인) →
      `pnpm --dir packages/figma-story-tools publish --publish-branch main`
      (저장소가 `main` 브랜치라 `--publish-branch main` 필수 — pnpm 기본값은 `master`)
