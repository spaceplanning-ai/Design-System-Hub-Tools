...
| 문서 | 건수 | 무엇을 고정하는가 |
| --- | --- | --- |
| **FS** 기능명세서 | **67** (`FS-001`~`FS-070`) | 화면의 요소를 전수 넘버링한다(`FS-001-EL-008`). §4 예외 명세는 요소 × **7축**(빈 상태 · 로딩 · 실패 · 유효성 · 권한없음 · 경합 · 대량)을 **빈칸 없이** 채운다 |
| **BE** 백엔드 기능명세서 | **67** (`BE-001`~`BE-070`) | 엔드포인트 · 공통 에러 봉투 · 인증/권한 모델. §5 예외 매트릭스는 **9축**(400 검증 · 401 인증 · 403 vs 404 · 404 대상없음 · 409 충돌 · 422 상태위반 · 429 과부하 · 500 오류 · 타임아웃) |
| **NFR** 비기능명세서 | **53** (`NFR-015`~`NFR-070`) | `quality-bar.md` 의 **P0 30건을 그 화면에 전수 판정**한다. `적용` 축(`직접` / `상속` / `N/A`)으로 표면의 실재를 먼저 가리고, 성능 예산 · 가용성 · 데이터 보존을 덧붙인다 |
