...
- jsdelivr @gh (즉시): `https://cdn.jsdelivr.net/gh/Figam-Dev-Variable-Tools/Auto-Builder@main/packages/figma-story-tools/manifest.json`
- 또는 Pages 미러(① 완료 후): `https://figam-dev-variable-tools.github.io/Auto-Builder/manifest.json`
...
**③ Figma에서 생성** — 플러그인 [원격에서 가져오기(URL)]에 위 매니페스트 URL 입력 →
...
- [ ] **GitHub Pages 활성화** (Chromatic 대체): 저장소 Settings → Pages → Source: **GitHub Actions**.
      ...
      선언(manifest/tokens/docs-content)을 `figam-dev-variable-tools.github.io/Auto-Builder`에 배포
- [ ] **Figma 플러그인**: `pnpm --dir figma-plugin build` → Figma → Plugins → Development →
      Import plugin from manifest… → [원격에서 가져오기(URL)]에 매니페스트 URL(jsdelivr @gh는
