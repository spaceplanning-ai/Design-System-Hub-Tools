...
### Storybook → Figma (자체 구축 — 내장 매니페스트)

외부 SaaS(Chromatic·story.to.design) 없이, 선언(manifest/tokens/docs-content)을 플러그인에
내장해 Figma 디자인시스템을 생성한다.

**현재 배포 상태 — 원격 호스팅 없음.** 이 저장소는 **private**이라 원격 선언 배포 경로 두 개가
모두 막혀 있다:

- **GitHub Pages** — Free 플랜의 Pages는 public 저장소 전용이다. private에서 워크플로가
  Pages 사이트를 만들려 하면 `Resource not accessible by integration`으로 실패한다.
  그래서 배포 워크플로를 제거하고 검증 전용 [ci.yml](.github/workflows/ci.yml)로 대체했다.
- **jsdelivr @gh** — 공개 저장소만 서빙한다. private 저장소의 raw 파일은 CDN이 못 읽는다.

**따라서 Figma 생성은 플러그인 내장 매니페스트로 한다** — 플러그인 UI에서 URL을 비워두면
내장 기본값(빌드 시점의 `packages/figma-story-tools/manifest.json`)이 그대로 쓰인다.
`pnpm build:manifest` → `pnpm --dir figma-plugin build`로 최신 선언을 플러그인에 굽는다.

**원격 URL 로드를 되살리려면** 셋 중 하나가 필요하다:
1. 저장소를 **public으로 전환** — 원래 설계대로 Pages·jsdelivr가 모두 무료로 동작한다.
2. 조직을 **Team 이상 플랜**으로 — Pages는 켜지지만 jsdelivr는 여전히 불가하므로
   스냅샷 base URL([figma-plugin/src/code.ts](figma-plugin/src/code.ts))을 Pages URL로 바꿔야 한다.
3. **사내/외부 호스팅**에 manifest·tokens·docs-content·snapshots를 올리고 플러그인
   `networkAccess`(`figma-plugin/manifest.json`)에 해당 도메인을 추가한다.
...
- [ ] **문서 사이트 호스팅 결정**: 현재 private이라 Pages·jsdelivr 둘 다 불가(위 §Storybook → Figma
      참조). public 전환 / Team 플랜 / 사내 호스팅 중 하나를 골라야 원격 선언 로드와 문서 사이트가
      되살아난다. 그때까지는 `pnpm storybook` 로컬 실행 + 플러그인 내장 매니페스트로 동작
- [ ] **Figma 플러그인**: `pnpm build:manifest` → `pnpm --dir figma-plugin build` → Figma →
      Plugins → Development → Import plugin from manifest… → URL 입력란은 **비워둔 채**
      [디자인시스템 생성](내장 매니페스트 사용) → Variables·컴포넌트·문서 생성 확인
...
- [ ] **(선택) figma-story-tools npm 발행** — private 저장소에서 원격 선언을 공개하는 유일한
      우회로다(unpkg가 npm 패키지를 서빙 → 플러그인 [URL에서 불러오기]로 로드 가능).
      `pnpm whoami` → `pnpm build:manifest` →
      `pnpm --dir packages/figma-story-tools publish --publish-branch main`
