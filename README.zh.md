...
画面在动手做之前先以文档固定下来。`specs/` 中共 **187 件** —— 以画面编号为轴，三类文档成对存在，按画面放置于 `specs/<板块>/<子项>/`（例如 `specs/users/members/`）。

| 文档 | 件数 | 固定了什么 |
| --- | --- | --- |
| **FS** 功能规格书 | **67**（`FS-001`~`FS-070`） | 对画面元素全量编号（`FS-001-EL-008`）。§4 异常规格把 元素 × **7 轴**（空状态 · 加载 · 失败 · 校验 · 无权限 · 竞争 · 批量）**不留空格地**填满 |
| **BE** 后端功能规格书 | **67**（`BE-001`~`BE-070`） | 端点 · 通用错误信封 · 认证/权限模型。§5 异常矩阵覆盖 **9 轴**（400 校验 · 401 认证 · 403 vs 404 · 404 对象不存在 · 409 冲突 · 422 状态违规 · 429 过载 · 500 错误 · 超时） |
| **NFR** 非功能规格书 | **53**（`NFR-015`~`NFR-070`） | 把 `quality-bar.md` 的 **P0 30 件对该画面全量裁定**。先用 `适用` 轴（`直接` / `继承` / `N/A`）筛出该表面是否真实存在，再补上性能预算 · 可用性 · 数据留存 |
...
├── specs/                  画面规格 187 件 — FS-*(元素全量编号 + 异常 7 轴) · BE-*(异常 9 轴) · NFR-*(P0 30 全量裁定) · quality-bar.md
