...
/* ── 조사(助詞) — ERP-13 ────────────────────────────────────────────────────────
 *
 * 사용자 대상 문구에 리터럴 '이(가)'/'을(를)' 을 절대 출하하지 않는다. shared/format 에 공용 헬퍼가 없고
 * (@tds/ui Empty 안의 것은 export 되지 않는다) shared/** 는 이번 배치에서 수정 금지라 이 섹션 문구용으로
 * 여기 둔다. → shared/format 승격 대상으로 보고서에 올린다. */

/** 마지막 문자가 받침 있는 한글 음절인지 — 음절 U+AC00–U+D7A3, (코드-0xAC00)%28 !== 0 이면 종성 있음 */
...
/** 목적격 조사 '을/를' — 받침 있으면 '을' (한글 아님/빈 문자열은 받침 없음으로 본다) */
