...
import { requiredKoreanText } from '../_shared/validation';

export const emailTemplateSchema = z
  .object({
    name: requiredKoreanText('템플릿명', TEMPLATE_NAME_MAX),
    trigger: z.enum(TRIGGER_ID_VALUES),
    subject: requiredKoreanText('제목', EMAIL_SUBJECT_MAX),
    body: requiredKoreanText('본문', EMAIL_BODY_MAX),
  })
  .check((ctx) => {
