...
- 이 문서는 "빈칸 채우기" 템플릿이 아니라 contract.json 작성 가이드다.
  ...
  리뷰 큐에 진입할 수 있다 (G3 진입 조건).
- G3 승인 시 계약은 Frozen된다. 이후 변경은 변경 요청 → G3 재진입 + SemVer
...
| `owner` | O | `code`/`design`/`figma` 3개 모두 담당 역할 — 예: 컴포넌트 엔지니어/UI 디자인/Figma 컴포넌트 |
...
| prop 추가 (default 있음) | MINOR | additive-only → G3 우선순위 큐, SLA 2h |
...
1. 변경 필요 발견자(예: 컴포넌트 엔지니어)가 변경 요청 발행
2. 계약 엔지니어가 계약 수정 → §3 표로 SemVer 재판정 → `version` 갱신
3. G3 재진입 (계약 리뷰 재승인) → 재Frozen → `pnpm codegen` 재실행
