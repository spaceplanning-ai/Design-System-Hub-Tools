...
### 2.3 실패 방향은 fail-closed 가 기본이다 — **예외 1건**

이 저장소의 기본 관성은 **모르면 닫는다**이다. 권한 매트릭스가 그 전형이다: 저장값에 없는 리소스는
꺼진 것으로 보고, 새 역할은 전 권한 OFF 로 시작하고, 저장값이 모순이면 더 좁은 쪽으로 되돌린다.
`pgSellable`(PG 사용 여부)도 같다 — 켜 두고 상점 ID 가 비었으면 켠 것으로 치지 않는다.

**예외는 엔타이틀먼트(플랜) 축 하나다. 이 축만 fail-open 이다.**

```ts
// shared/entitlements/plan.ts — 모르면 연다
if (value === undefined) return GRANTED;   // 저장된 값이 없다
if (spec === null) return GRANTED;         // 카탈로그에 없는 키다
// localStorage 파손 · 버전 불일치 · 라우트 미매핑 — 전부 같은 방향
```

**왜 여기만 반대인가**: 권한을 잘못 열면 사고지만, 엔타이틀먼트를 잘못 닫으면 **고객이 이미 산 기능이
우리 조회 실패로 멈춘다.** 최악을 비교하면 전자는 "안 산 기능이 잠깐 열린다"이고 후자는
"결제한 고객의 앱이 죽는다"다. 회복 비용이 다르므로 방향도 다르다.

이 예외는 **판정 순서와 한 벌**이다 — 플랜은 권한 **바깥**에서 판정되므로(인증 → 플랜 → 권한 → 설정),
플랜이 열려도 권한이 여전히 닫는다. fail-open 이 최종 방어선을 무르게 만들지 않는 이유가 그것이다.

**새 축을 만들 때 방향을 임의로 고르지 마라.** 기본은 fail-closed 이고, 반대로 가려면
그 축의 최악 두 가지를 비교한 근거를 ADR 로 남긴다 — [ADR-0013](../adr/0013-entitlement-layer.md) 이 그 형식이다.

### 2.4 그 외 클린코드 점검 측정 축
| 축 | 임계값 |
|---|---|
...
**도입하지 않는 것**: 전역 상태 라이브러리(Redux/Zustand). 지금 전역 상태는 **판정 축 둘**(권한 · 플랜)과
결제 설정뿐이고, 각각 자기 모듈이 소유한다 — 권한은 `main.tsx` 의 `PermissionProvider`, 플랜은
`AppShell` 이 거는 스토어 구독(`usePlan`), 결제 설정은 모듈 스코프 변수 + 명시적 read/write 다.
공통 라이브러리를 끼워 넣을 만큼 서로 얽혀 있지 않다 — YAGNI.
