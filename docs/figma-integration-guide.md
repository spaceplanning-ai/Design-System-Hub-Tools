...
1. Chromatic으로 Storybook을 배포한다. PowerShell: `$env:CHROMATIC_PROJECT_TOKEN='chpt_...'; pnpm chromatic`
   (bash/CI: `CHROMATIC_PROJECT_TOKEN=xxxx pnpm chromatic`). story.to.design은 도달 가능한
   Storybook URL이면 로컬 터널로도 동작하지만, 표준 경로는 Chromatic 배포 URL이다.
