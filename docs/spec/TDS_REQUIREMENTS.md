...
| §6 Overlay 12종 — Modal·System Dialog/Alert/Confirm/Prompt(Dialog 통합)·Bottom Sheet·Action Sheet·Toast·Snackbar·Loading·Tooltip·Popover | **완료** (Phase 6 — OV1~OV2) | src/ds/ — portal 미사용(ThemeScope 토큰 스코프 유지), inline 문서 렌더 규약 |
| §6 Navigation 9종 — Header·Footer·Sidebar·Navbar·Tab·Breadcrumb·Pagination·Drawer·Dropdown | **완료** (Phase 6 — NAV) | src/ds/ — Select의 Chevron/useDismiss 재사용 |
| §6 Data Display 13종 — Card·Table(정렬)·List·Accordion·Tabs(Tab)·Timeline·Carousel·Statistics·Chart·Tree·Badge·Chip·Avatar | **완료** (Phase 6 — DD1~DD2) | src/ds/ — 제네릭 Table<T>, 재귀 Tree |
| §7 공통 State 15종·§8 Validation 13종 **전수 매트릭스 문서화** | **완료** (Phase 7) | '7. 상태 & 검증' 문서 페이지 — docs-content.json state/validation 매트릭스 (Figma 페이지 미러링 대상 포함) |
| §1 Component Playground | **완료** (Phase 7) | '8. Playground' — DS 12종 통합 폼 데모 (입력→검증→Dialog→Snackbar) |
| §1 Versioning / Design QA | **완료(문서화)** (Phase 7) | docs/versioning.md(릴리스 절차·호환성 규칙) + docs/design-qa.md(자동 게이트 5종 + Chromatic 리뷰 + 수동 체크리스트) |
...
| §3 Admin Components/Templates/Animations/Accessibility 섹션 | **완료** (Phase 8) | Admin(AdminShell·FilterBar·EmptyState) / Templates(Login·Dashboard·Settings·ListPage — DS 조합 풀페이지) / Animations(모션 토큰 레이어 + 데모) / '10. 접근성'(a11y 매트릭스 + prefers-reduced-motion 전역) |
