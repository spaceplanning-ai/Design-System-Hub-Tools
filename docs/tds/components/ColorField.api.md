<!-- AUTO-GENERATED from contracts/ColorField.contract.json — DO NOT EDIT (pnpm codegen) -->

# ColorField API

> ⚠️ **자동 생성 문서** — 이 파일은 계약에서 생성됩니다. 직접 수정하지 마세요.
> 변경이 필요하면 `contracts/ColorField.contract.json` 을 수정한 뒤 `pnpm codegen` 을 실행하세요.
...
|---|---|---|---|---|---|
| `value` | `string` | — | ✅ | — | 현재 색 — #RGB · #RRGGBB · #RRGGBBAA 를 받는다. 제어 값이며 유효한 hex 로 확정된 onChange 로만 바뀐다 |
| `label` | `string` | — | ✅ | — | 스크린 리더용 이름(aria-label) — 보이는 <label> 이 없는 자리(패널 상자 안)에서 무슨 색인지 알린다('Canvas color' 등). 스와치에는 '<label> swatch' 로 별도 이름을 준다(한 값에 컨트롤이 둘이라 이름이 갈려야 한다) |
| `id` | `string` | `""` | — | — | hex 텍스트 입력의 DOM id — 한 화면에 색 필드가 여럿일 때 호출부가 유니크 id 를 주입한다. 비우면 id 속성을 렌더하지 않는다 |
| `disabled` | `boolean` | `false` | — | `Disabled` | 비활성 — 스와치와 텍스트를 함께 잠그고 흐리게 표시한다. onChange 발화 없음 |

## Events

| 이름 | Payload | 발화 차단 상태 | 설명 |
|---|---|---|---|
| `onChange` | `string` | `disabled` | **유효한 hex 일 때만** 새 색 문자열을 발화한다 — 입력 도중의 부분 문자열은 올리지 않는다(캔버스 깜빡임 방지). disabled 에서는 발화 금지 — <input disabled> 가 네이티브로 막는다 (Storybook Play Function 이 전수 검증) |
...
| 키보드 | `Tab`, `Enter`, `Space` |
| focus-visible | 필수 |
| `label` | hex 텍스트 입력이 label prop 을 aria-label 로 갖고, 스와치는 '<label> swatch' 를 갖는다 — 한 값을 두 컨트롤이 편집하므로 이름이 갈려야 어느 쪽에 있는지 알 수 있다 |
...
| `swatchSize` | `space.6` | `--tds-space-6` |
| `swatchFill` | `color.chart.series-1` | `--tds-color-chart-series-1` |
| `swatchBorder` | `color.border.default` | `--tds-color-border-default` |
| `swatchBorderWidth` | `border-width.thin` | `--tds-border-width-thin` |
| `swatchRadius` | `radius.sm` | `--tds-radius-sm` |
| `hexText` | `color.text.default` | `--tds-color-text-default` |
| `hexTextDisabled` | `color.text.disabled` | `--tds-color-text-disabled` |
| `typography` | `typography.code.md` | `--tds-typography-code-md` |
