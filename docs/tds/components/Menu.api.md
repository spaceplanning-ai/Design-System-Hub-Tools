...
[레이어링 — 포털을 쓰지 않는다] 팝업은 position:absolute 로 트리거 래퍼(position:relative) 안에 그린다. Modal 처럼 body 로 포털하지 않는 이유는 둘이다: (1) 이 메뉴는 표 행·툴바 안에 살아 스크롤 컨테이너와 함께 움직여야 한다, (2) 리포에 이미 포털이 Storybook 테마 데코레이터를 벗어나 DarkTheme 스토리가 라이트로 렌더되는 실결함이 있다(Modal·ConfirmDialog). 같은 결함을 재생산하지 않는다.
