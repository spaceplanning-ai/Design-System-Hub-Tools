...
```bash
pnpm whoami                                 # 오너 npm 계정 로그인 확인
pnpm build:manifest                         # 소스에서 재직렬화 + 왕복 동일성 검증
pnpm --dir packages/figma-story-tools pack  # 드라이런(패키징 내용 확인)
# 발행 (오너 npm 계정): 저장소가 main 브랜치라 --publish-branch main 필수
pnpm --dir packages/figma-story-tools publish --publish-branch main
```

> 이 저장소는 pnpm 전용(corepack `packageManager: pnpm@11.11.0`)이므로 `npm publish`가
> 아니라 `pnpm publish`를 쓴다. `figma-story-tools`는 아직 미발행(첫 publish가 이름을
> 계정에 귀속) — 발행 전 `pnpm whoami`로 계정을 반드시 확인한다. 재발행은 version bump 필요.
