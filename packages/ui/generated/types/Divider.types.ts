...
/** `Divider.orientation` 허용 값 (계약이 유일한 원천) */
export type DividerOrientation = 'horizontal' | 'vertical';

/** 계약에 선언된 상호작용 상태 */
export type DividerState = 'default';
...
export interface DividerProps {
  /**
   * 선의 방향. `horizontal`(기본)은 부모 폭을 채우는 가로선이고, `vertical` 은 부모 높이에 맞춰(`align-self: stretch`) 늘어나는 세로선이다 — 툴바가 쓰는 쪽이 후자다. 논리 속성(inline-size/block-size)으로 그리므로 RTL 에서 따로 뒤집을 것이 없다
   * @default "horizontal"
   */
