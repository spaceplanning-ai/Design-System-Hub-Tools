...
- 입력: 계약 25종 · FS 14건 · 테스트 파일 88개 · 스토리 파일 35개
- **단언을 가진 실행 단위(= 테스트): 913건** / 단언 없는 실행 단위: 2건
...
| 2 | 계약 states 커버리지 (contracts/*.contract.json → states[]) | **blocker** | 79 | 79 | 0 | 미커버 상태 0건 (전수) | G5·G6 | PASS |
| 3 | 계약 events.blockedWhen 커버리지 (금지 동작의 비발생 단언) | **blocker** | 14 | 14 | 0 | 미커버 차단 조건 0건 (전수) | G5·G6 | PASS |
