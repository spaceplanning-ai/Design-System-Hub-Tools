...
- 판정: **WARN** (exit 0) — blocker 0건 · major 12774건
- 입력: 계약 55종 · FS 73건 · 테스트 파일 211개 · 스토리 파일 222개
- **단언을 가진 실행 단위(= 테스트): 3262건** / 단언 없는 실행 단위: 10건
...
| 4 | FS 예외 7축 커버리지 (요소 × 축 격자 — 동작이 정의된 칸만 · 래칫) | major | 137 | 12909 | 12772 | 미커버 칸 0건 (major) · **커버 칸 수 후퇴 = blocker** | G6 | VIOLATED |
...
|---|---|---|---|---|
| @tds/admin | `apps/admin` | **2380** | 3 | PASS |
...
## 축 4 — FS 예외 7축 커버리지 (요소 × 축 격자 — 동작이 정의된 칸만 · 래칫) (12772건, major)
...
| … 외 **12760건** | 전수 목록은 JSON 리포트 `gaps[]` 참조 | |
