...
| 형제 문서 | NFR-015(회사 정보) — **같은 골격**(단일 문서형)이라 P0 30건의 판정 구조가 같다. **EXC 계열 gap 3건은 동일하며 뿌리도 같다**(`useSaveDocument` 직접 사용). **다만 F3a 이후 A11Y-11 판정이 갈렸고, `a5c2639` 기준에서도 그 분기는 그대로다** — 이 화면은 pass(gap 3건), NFR-015 는 gap 유지(gap 4건)다. 사유: NFR-015 의 `profile-biznum` 은 hint 를 갖고도 valid 일 때 `aria-describedby` 로 물리지 않는데(`CompanyProfilePage.tsx:142-144` — 오류일 때만 배선), 이 화면의 손수 배선 입력(제목)에는 hint 가 없다. **⚠ PR #30 이 DS 의 A11Y-11 층을 손봤지만 이 분기는 움직이지 않았다** — #30 이 닫은 것은 `ImageUploadField` 의 **required 노출**(접근성 이름 꼬리표)이고, NFR-015 의 잔여 사유는 **hint 연결**이라는 별개 축이라 무관하다. 두 화면 다 사진/로고에 `required` 를 주지 않아 #30 의 수혜 대상도 아니었다. 이 문서는 그 판정을 재확인하고 **이 화면 고유의 차이**(본문 5000자 · `TextareaField` · 손실 규모)를 기술한다 |
| 갱신 규칙 | quality-bar 가 개정되면 §2 의 30행을 재판정한다. 이 화면의 코드가 바뀌면 근거의 `파일:라인` 을 갱신한다. **판정을 코드보다 먼저 고치지 않는다** |
| 판정 방식 | **E2E 미실행 — 판정 근거는 코드 대조다**(§6) |
| 판정 기준일 | **2026-07-17 · HEAD `a5c2639`** 기준 코드 대조. 직전 판정은 `4b805ad` 기준이었고, 이후 PR #22·#24·#26·#28·#30·#32·#34 가 머지됐다. **이번 기준 갱신으로 뒤집힌 판정은 없다.** A11Y-11 은 이미 pass 였고 그대로다 — PR #30 이 `ImageUploadField` 의 required 축을 닫았으나 **이 화면은 사진에 `required` 를 주지 않아 애초에 그 결함을 밟지 않았다**(§1 형제 문서 행 참조). TOKEN-01 도 그대로다 — PR #32 가 AppShell main padding 을 `space.6`→`space.7` 로 올렸으나 그것은 셸의 값이고, **이 화면의 `photoWrapStyle` 이 쓰는 `space-6` 파생 배수는 바뀌지 않았다**(§2 TOKEN-01 참조) — §2 각 행의 근거 라인 참조 |
...
| A11Y-11 | A11Y | 직접 | **F3a 에서 뒤집혔다(gap → pass) — 세 절이 모두 성립한다.** ① `aria-invalid` ↔ `aria-describedby` 페어링은 **성립한다**: 제목 입력(`CeoMessagePage.tsx:107-108`)이 `aria-invalid={errors.title !== undefined}` + `aria-describedby={errors.title !== undefined ? errorIdOf('ceo-title') : undefined}`, `FormField.tsx:109-118` 가 그 id 로 `role="alert"` `<p>` 를 그린다. 본문·사진은 DS 가 자동 배선한다(`TextareaField.tsx:67` · `ImageUploadField.tsx:225`) → **짝 없는 `aria-invalid` 0건.** ② **hint 연결**: 사진 필드의 hint 는 `ImageUploadField.tsx:225` 가 `describedBy = invalid ? errorIdOf(id) : hint !== undefined ? hintIdOf(id) : undefined` 로 **올바르게 연결한다**(hint `<p id={hintIdOf(id)}>` `:322`). 제목 입력은 hint 가 없어 해당 없음 → **이 절은 충족**(NFR-015 와 다른 점 — 그쪽은 `profile-biznum` hint 가 미연결). ③ **required 노출 — F3a 가 이 마지막 절을 닫았다(gap → pass).** 직전 판정은 '경로가 하나도 없다 · DS 계층의 gap 이라 이 화면 수정만으로 닫히지 않는다' 였고, **DS 가 실제로 그것을 닫았다** — 두 컨트롤이 서로 다른 경로로 충족한다: **제목** `<input id="ceo-title">`(`CeoMessagePage.tsx:99-110`) → `FormField` 가 `withAriaRequired()` 로 `required` 를 **런타임 `cloneElement` 로 자식의 `aria-required` 에 주입**한다(`FormField.tsx:50-56`, 주입 지점 `:107`, 대상 판별 `isRequirableChild` `:38-41` — 네이티브 `input`/`select`/`textarea` 와 DS `SelectField` 만). **본문** `<textarea>` → `TextareaField` 가 같은 자리에서 native `required` + `aria-required` 를 **직접** 낸다(`TextareaField.tsx:64-65`). **사진**(`ImageUploadField`)은 `required` 를 주지 않으므로 이 절의 대상이 아니다 — 이 화면이 당시 `ImageUploadField` 의 required 결함을 피해 간 이유다. **그 결함은 이후 PR #30 이 닫았다**(드롭존 접근성 이름에 `requiredNameSuffix` 로 '(필수)' 를 싣는다 — `ImageUploadField.tsx:55,250`; NFR-019·NFR-020·NFR-021 이 그 수정으로 A11Y-11 을 pass 로 뒤집었다). **이 화면의 판정에는 영향이 없다** — 여전히 사진을 필수로 요구하지 않는다. `*` 마커는 여전히 `aria-hidden` 장식이지만(`FormField.tsx:96-100`) **이제 그것이 유일한 경로가 아니다** — 마커의 시각 표현은 그대로 두고 AT 경로만 더한 설계다(`FormField.tsx:17-22` 주석) | ① `pages/company/ceo-message` grep: `aria-describedby` 없는 `aria-invalid` = 0 → 충족. ② 사진 필드 valid 상태 → `aria-describedby` 가 hint id 를 가리킴 → 충족. ③ RTL: `getByRole('textbox', { name: /제목/ })`·`{ name: /본문/ }` 의 `aria-required` 조회 → **둘 다 `"true"`** → 충족. **⚠ grep 으로 판정하지 말 것**: `grep -rn "aria-required" apps/admin/src` 는 여전히 1건(`members/components/CreateGroupModal.tsx:192` — 수동 override)뿐이다. 주입은 **런타임**이라 소스에 나타나지 않는다 — 판정은 **required FormField 자식의 타입**으로 한다 | pass |
