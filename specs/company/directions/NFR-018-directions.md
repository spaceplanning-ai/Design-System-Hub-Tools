...
| A11Y-11 | A11Y | 직접 | **세 계약 중 하나만 성립한다.** ① `aria-invalid` ↔ `aria-describedby` 쌍: 5필드 전부 배선됨(`DirectionsPage.tsx:142-143` · `161-165` · `185-187` · `206-208` · `238-240`) — 짝 없는 `aria-invalid` 0건 ✅. ② **hint 연결 없음** ❌: `addressDetail`·`latitude`·`longitude`·`transit` 4필드가 hint 를 갖고 `FormField.tsx:76-80` 이 `<p id={hintIdOf(htmlFor)}>` 를 렌더하지만, 호출부의 `aria-describedby` 는 **error 일 때만** 붙는다(`errors.X !== undefined ? errorIdOf(...) : undefined`) — valid 상태에서 hint id 를 물리지 않아 '예: 37.5000' 이 AT 에 전달되지 않는다. `FormField` 는 id 만 노출하고 배선은 호출부 책임이다(`FormField.tsx:9-10` 주석). ③ **required 미노출** ❌: 주소·위도·경도가 `required` 지만 `<input>` 에 `required`/`aria-required` 가 없다. `FormField.tsx:58-62` 의 `*` 마커는 `aria-hidden="true"` 라 **스크린리더에 필수 여부가 전달되는 경로가 0개**다 | RTL: `getByLabelText('위도')` 의 `aria-describedby` 가 valid 상태에서 `undefined` → hint `<p id="dir-lat-hint">` 미연결. `getByLabelText('주소')` 에 `required`·`aria-required` 둘 다 없음. (①은 통과: 빈 주소 제출 → `aria-describedby="dir-address-error"` === `role="alert"` `<p>` 의 id) | **gap** |
| A11Y-12 | A11Y | N/A | 표면 없음 — **이 화면에 좌측 필터 list item 이 없다**(단일 문서 폼). `filterItemStyle`/`aria-pressed`/`aria-current` 를 쓰지 않는다 | — | n-a |
| MOTION-01 | MOTION | 상속 | 표면: 이탈 가드 Modal 의 enter/exit. 이 화면은 Modal 을 소비만 한다 | DS Modal 소유 문서의 판정을 따른다 | 종속 |
| MOTION-02 | MOTION | 상속 | 표면: 저장 성공 토스트의 exit. 이 화면은 `toast.success` 를 호출만 한다 | ToastProvider 소유 문서의 판정을 따른다 | 종속 |
| MOTION-03 | MOTION | 상속 | 표면: 위 Modal·Toast 의 reduced-motion 게이트. **이 화면 자체에 transition/transform 선언이 0건**이므로 reduced-motion 이 끌 대상이 없다(ToggleSwitch 도 없다) | 전역 Motion config/DS 소유 문서의 판정을 따른다 | 종속 |
...
| IA-02 | IA | 직접 | title 소스가 **하나**다. `/company/directions` 는 **nav leaf** 이므로(`nav-config.ts:124` `['오시는 길', '/company/directions']`) `findNavLabel`(`:253-255`)이 exact 매치로 '오시는 길' 을 돌려주고 `AppHeader.tsx:92,101` 이 그것을 `<h1>` 로 그린다 — branch 라벨('기업 관리') 폴백(`:257-262`)에 **걸리지 않는다**. 화면 안에는 경쟁하는 `<h1>` 이 없다: `DocumentFormShell` 은 `CardTitle`(`:123`)만 그린다(`FormPageShell.tsx:159` 가 자체 `<h1>` 을 그리는 것과 대비). **하위 라우트가 없어**(FS-018 §1) sub-route 폴백 문제 자체가 성립하지 않는다 | `/company/directions` 진입 → 가시 primary title = '오시는 길'. `document.querySelectorAll('h1').length === 1` | pass |
...
| EXC-03 | EXC | 직접 | **읽기 게이팅은 성립하나 쓰기 게이팅이 없다.** ① read: `AppShell.tsx:490` `<RequirePermission>` 이 `<Outlet>` 을 감싸 이 라우트의 read 권한이 없으면 `ForbiddenScreen` 을 그린다(`RequirePermission.tsx:61-64`), 리소스는 `route-resource.ts:36-46` 이 경로에서 파생한다 ✅. ② **write: 이 화면이 `can(resource,'update')` 를 보지 않는다** ❌ — `DocumentFormShell.tsx:145-152` 의 저장 버튼은 `disabled={!dirty \|\| saving \|\| loading}` 뿐이고 권한 조건이 없다. `useRouteWritePermissions`(`RequirePermission.tsx:45-52`)가 존재하지만 **`apps/admin/src/pages/**` 전체에서 소비자 0건**(grep 확인). 저장 권한 없는 역할이 버튼을 보고 눌러 서버 403 을 받고, 그것이 '저장하지 못했습니다…'(FS-018-EL-003)로 표시된다 — '왜' 를 알 수 없다. ③ 강등 reconcile: 버튼이 애초에 권한을 구독하지 않으므로 강등해도 사라지지 않는다 ❌ | 권한 스토어에서 `company-directions` 의 `update` 를 끈 역할로 진입 → **저장 버튼이 그대로 보이고 눌린다**(기대: 미렌더 또는 disable). `read` 를 끄면 403 화면(이쪽은 통과). grep: `pages/**` 에서 `useRouteWritePermissions\|canCreate\|canUpdate\|canRemove` = 0건 | **gap** |
...
| ERP-06 | P1 | 사용자 대상 문구가 존댓말로 통일돼 있다. **다만 검증 문구가 조사 fallback 형을 출하한다** — `validation.ts:19` `${label}은(는) -${limit} ~ ${limit} 범위여야 합니다.` → 실제 화면에 '위도은(는) …' 이 그대로 렌더된다 | 위도에 '95' 입력 후 제출 → '위도은(는) -90 ~ 90 범위여야 합니다.' | gap (ERP-13 참조) |
| ERP-13 | P1 | **josa 헬퍼 미적용** — `validation.ts:19,26-28` 과 `requiredText`(`shared/crud/validation.ts`)가 '은(는)'/'을(를)' 리터럴을 생성한다. `Empty.tsx:68` 의 `subjectParticle` 이 앱에 존재하지만 이 화면의 검증 문구는 그것을 쓰지 않는다. quality-bar acceptanceCheck('`'이(가)'\|'을(를)'\|'은(는)'` grep = 0')를 이 화면이 위반한다 | 빈 주소 제출 → '주소을(를) 입력하세요.'(기대: '주소를 입력하세요.') | gap |
...
| EXC-07 | P1 | 422 `error.fields` → RHF `setError` 매핑이 **없다**(`useCrudForm.ts:176-186` 이 하는 일을 이 화면은 하지 않는다). 서버가 필드를 지목해도 폼 레벨 배너로만 뜬다 | `?status=save:422` → 인라인 필드 에러 0건, 배너만 | gap |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | **A11Y-11** | **P0** | hint id 를 `aria-describedby` 에 물리지 않아 4필드의 hint 가 AT 에 전달되지 않고, 필수 3필드에 `required`/`aria-required` 가 없다(`*` 마커는 `aria-hidden`) — 스크린리더에 필수 여부가 닿는 경로 0개 | **앱 전역** (`FormField` 가 배선을 호출부에 맡기는 계약 + 전 폼 호출부) | A40 (`FormField` 자동 배선 승격 검토) · A41 |
| 2 | **EXC-03** | **P0** | 쓰기 권한 게이팅 없음 — `useRouteWritePermissions` 소비자 0건. 저장 권한 없는 역할도 저장 버튼을 보고 누른다 | **앱 전역** (전 화면 공통) | A40 · A11 |
...
| 7 | ERP-13 / ERP-06 | P1 | 검증 문구가 '주소을(를)'·'위도은(는)' 조사 fallback 을 그대로 출하 | **앱 전역** (`shared/crud/validation.ts` `requiredText` + 각 화면 스키마) | A41 · A40 |
