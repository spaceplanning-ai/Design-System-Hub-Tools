...
| 판정 시점 | **2026-07-17 · HEAD `a5c2639`** 기준 코드 대조. 직전 판정은 `4b805ad` 기준이었고, 이후 PR #22·#24·#26·#28·#30·#32·#34 가 머지됐다. **이번 기준 갱신으로 뒤집힌 판정은 없다.** A11Y-11 은 이미 pass 였고 그대로다 — PR #30 이 `ImageGalleryField` 의 required 축을 접근성 이름으로 닫았으나(`ImageGalleryField.tsx:237`) **이 화면은 본문 이미지에 `required` 를 주지 않아**(`EsgFormPage.tsx:161-169`) 그 수정의 수혜 대상도, 직전 결함의 피해자도 아니었다 — §2 각 행의 근거 라인 참조 |
...
| STATE-02 | STATE | 직접 | 조회 실패가 요약·일괄바·표를 대체하는 **인라인 danger Alert + '다시 시도'** 다 — `CrudListShell.tsx:156-165`(`<Alert tone="danger">` + `controller.refetch`). 빈 상태로 폴백하지 않는다(`:113` 이 error 를 먼저 가른다). 에러 토스트 경로가 없다 — 이 화면의 `toast` 호출은 전부 성공/취소다(`useCrudList.tsx:108,146`) | `?fail=esg:list` → 위험 톤 배너 'ESG 활동 목록을 불러오지 못했습니다.' + '다시 시도'. 에러 토스트 0건. '다시 시도' → `refetch()`(`useCrudList.tsx:185`) 재발행 | pass |
...
| TOKEN-01 | TOKEN | 직접 | 이 화면의 style object 전부가 `var(--tds-*)` 만 참조한다 — `EsgListPage.tsx:25-46`(`layoutStyle`·`toolbarStyle`·`summaryCellStyle`) · `EsgFormPage.tsx:29-33`(`rowStyle`). **좌측 필터의 스타일은 이 화면에 없다** — 공용 `FilterPanel`/`FilterRail`(`shared/ui/FilterPanel.tsx`)이 소유하고 그것이 `shared/ui/styles.ts` 의 `filterPanelStyle`(`:209`)·`filterNavStyle`(`:217`)·`filterItemStyle`(`:266`) 등을 쓴다(`FilterPanel.tsx:23-31`). hex 0건 · px 리터럴 0건 · border 키워드 0건. 파생 치수는 `calc(var(--tds-space-6) * n)`(`EsgListPage.tsx:27,42` · `EsgFormPage.tsx:31`) | `apps/admin/src/pages/company/esg/**` grep: `#[0-9a-f]{3,6}` = 0 · `[1-9]px` = 0 · `(outline\|border):\s*(thin\|medium\|thick)` = 0. `shared/token-guard.test.ts` 통과. ESLint 0 warning | pass |
| TOKEN-02 | TOKEN | 상속 | 표면: 분류 필터 버튼(공용 `FilterPanel.tsx:144` `className="tds-ui-listitem tds-ui-focusable"` — 이 화면이 `FilterPanel` 을 소비해 얻는 표면이다) · 폼 입력(`EsgFormPage.tsx:122,136` `tds-ui-input tds-ui-focusable`) · DS `Button`/`SelectField`/`RowActions`/체크박스. focus ring 두께·색을 이 화면이 선언하지 않는다 | DS/`ui.css` 소유 문서의 판정을 따른다 | 종속 |
...
| TOKEN-05 | TOKEN | 상속 | 표면: 폼의 `<h1>` — `FormPageShell.tsx:160` `<h1 style={pageTitleStyle}>` → `styles.ts:51-61` 이 `--tds-typography-title-xl-*` 를 소비(`:158` 주석이 TOKEN-05 를 명시). AppHeader 의 `<h1>` 도 같은 토큰(`AppHeader.tsx:52-55`). ⚠ **그 둘이 동시에 뜨는 것**은 TOKEN 이 아니라 IA-02 의 문제다 | AppHeader/styles.ts 소유 문서의 판정을 따른다 | 종속 |
| COMP-10 | COMP | N/A | 표면 없음 — **이 화면에 text-search/filter 입력이 없다.** 툴바는 '등록' 버튼 하나뿐이고(`EsgListPage.tsx:85-92`) 좌측 필터는 **버튼 4개**다(`ESG_FILTER_OPTIONS`(`types.ts:61-68`)의 4항목을 공용 `FilterPanel` 이 `<button type="button">` 으로 그린다 — `FilterPanel.tsx:137-161`. 그 컴포넌트는 검색 입력 축 자체를 갖고 있지 않다). 텍스트 입력이 없으므로 IME 조합(`isComposing`)·debounce·최소 query 길이·stale 응답 경합이 성립할 표면이 0개다. 폼의 제목/내용 입력은 **저장 대상이지 query 를 발행하는 검색 입력이 아니다**(`EsgFormPage.tsx:132-159` — 어떤 조회도 트리거하지 않는다). ⚠ 검색 입력이 **추가되면** 그때 `useDebouncedSearch`(`shared/crud/useDebouncedSearch.ts` — 8파일이 소비 중)를 소비해야 하며, 그 시점에 이 행은 `직접`으로 바뀐다 | — | n-a |
...
| A11Y-01 | A11Y | 상속 | 표면: 삭제 성공 토스트(`useCrudList.tsx:108,146`) · 저장 성공 토스트(`useCrudForm.ts:222`) · 취소 토스트(`ConfirmDialog.tsx:39`). 지속 live region viewport 는 `ToastProvider` 의 것이다. (이 화면은 **목록 상태용 live region 을 별도로** 갖는다 — `CrudListShell.tsx:107-109`, 그것은 A11Y-16 소관) | ToastProvider 소유 문서의 판정을 따른다 | 종속 |
...
| A11Y-11 | A11Y | 직접 | **F3a 에서 뒤집혔다(gap → pass) — 세 계약이 모두 성립한다.** ① `aria-invalid` ↔ `aria-describedby` 쌍 ✅: `category`·`date`·`title` 은 호출부가 명시 배선(`EsgFormPage.tsx:106-107` · `125-126` · `141-142`), `summary` 는 DS 가 자동 배선(`TextareaField.tsx:67`), `imageUrls` 는 DS 가 자동 배선(`ImageGalleryField.tsx:187,305`) — 짝 없는 `aria-invalid` 0건. ② hint 연결 ✅: hint 를 가진 필드는 `imageUrls` 하나이며 DS 가 `describedBy = invalid ? errorIdOf : hint ? hintIdOf : undefined`(`ImageGalleryField.tsx:187`)로 valid 일 때 hint 를 물린다. `category`·`date`·`title` 은 hint 가 없어 해당 없음. ③ **required 노출 ✅ — F3a 가 이 마지막 절을 닫았다(gap → pass).** 직전 판정은 '스크린리더에 필수 여부가 닿는 경로가 0개' 였다. 지금은 필수 4필드가 **전부 닿는다**: **`category`** → `<FormField … required>` 의 자식이 DS `SelectField`(`EsgFormPage.tsx:101-116`)이고, `FormField` 의 `withAriaRequired()` 가 `aria-required` 를 `cloneElement` 로 주입하면(`FormField.tsx:50-56`, 주입 지점 `:107`) `SelectField` 가 native 속성을 마지막에 spread 해 `<select>` 로 흘린다(`SelectField.tsx:42,55-63`) — `isRequirableChild` 가 `SelectField` 를 명시 허용한다(`FormField.tsx:38-41`). **`date`·`title`** → 자식이 네이티브 `<input>`(`:118-129` · `:132-145`) 이므로 같은 주입 대상. **`summary`** → `TextareaField` 가 자기 `<textarea>` 에 native `required` + `aria-required` 를 **직접** 낸다(`TextareaField.tsx:64-65`) — 직전 판정이 지적한 '`required` 를 FormField 로만 넘긴다' 는 더 이상 사실이 아니다. **`imageUrls`(`ImageGalleryField`)는 `required` 를 주지 않으므로**(`EsgFormPage.tsx:161-169`) **이 절의 대상이 아니다** — 직전 판정은 그 몰리큘에 'required 를 받아도 `aria-hidden` 마커로만 그리는 잔여 결함' 이 있다고 적었고 이 화면은 그것을 밟지 않는다고 했다. **그 결함은 이후 PR #30 이 닫았고, 기록해 둘 만큼 사유가 흥미롭다**: `ImageGalleryField` 는 이제 **빈 드롭존의 접근성 이름**에 `requiredNameSuffix(required)` 로 '(필수)' 를 싣는다(`ImageGalleryField.tsx:237`, import `:14`). `aria-required` 를 쓰지 않은 것은 누락이 아니라 설계 판단이다 — `role=button` 이 지원하지 않는 속성이기 때문이다(근거 `ImageUploadField.tsx:44-54`). **갤러리 고유의 뉘앙스**: 꼬리표는 **빈 드롭존에만** 붙고 '이미지 추가' 버튼은 생략한다(`:275-277` 주석) — 그 시점엔 required('최소 한 장')가 **이미 충족**돼 있어 필수라고 말하면 거짓말이 되기 때문이다. NFR-019·NFR-020·NFR-021 이 이 수정으로 A11Y-11 을 pass 로 뒤집었으나 **이 화면의 판정에는 영향이 없다**(애초에 밟지 않았다). `*` 마커는 여전히 `aria-hidden` 장식이지만(`FormField.tsx:96-100` · 갤러리 쪽 `ImageGalleryField.tsx:214`) **이제 그것이 유일한 경로가 아니다** | RTL: `getByLabelText('제목')`·`getByLabelText('일자')`·`getByLabelText('내용')`·`getByLabelText('분류')` 의 `aria-required` 조회 → **전부 `"true"`**. (①②도 통과: 빈 제목 제출 → `aria-describedby="esg-title-error"` === `role="alert"` `<p>` 의 id) **⚠ grep 으로 판정하지 말 것**: `grep -rn "aria-required" apps/admin/src` 는 여전히 1건(수동 override)뿐 — 주입이 **런타임**이라 소스에 나타나지 않는다 | pass |
| A11Y-12 | A11Y | 직접 | **`aria-pressed` 단일 규약을 지킨다** — 이 화면이 소비하는 공용 `FilterPanel.tsx:146` `aria-pressed={active}`(**적용이 `직접` 인 이유는 §1.1 그대로다 — 어느 공용 컴포넌트를 소비할지 고르는 것이 이 화면의 결정이다**). `aria-current` 를 쓰지 않으며, 앱 전체에서도 `apps/admin/src` 의 `aria-current` **사용 0건**(잔여 매치는 `a11y-guard.test.ts` 의 검사 코드·주석, `ProductFilterPanel.tsx:5`·`StatsFilterBar.tsx:7`·`FilterPanel.tsx:9,18` 의 주석, 그리고 `ui.css:76` 의 `.tds-ui-page…[aria-current='page']` — 마지막 것은 **@tds/ui Pagination 의 진짜 navigation** 이라 정당하다). `filterItemStyle(active)`(`styles.ts:266`)을 쓰는 4버튼 전부가 `aria-pressed` 다(`FilterPanel.tsx:145-146`). **회귀를 막는 그 기록은 이제 공용 컴포넌트의 머리말에 산다 — `FilterPanel.tsx:9-10` 이 같은 골격이 아홉 벌로 갈라져 한쪽은 `aria-current` 를 쓰던 사실과 두 가지 피해(① A11Y-12 위반 ② 공유 hover 규칙 `.tds-ui-listitem[aria-pressed='false']` 를 비껴가 미선택 항목에 hover 표시가 없던 시각 버그)를 적고, `:17-19` 가 '이 버튼은 토글 필터이지 현재 위치가 아니다' 를 규약으로 못 박는다.** 회귀 가드가 테스트로도 고정돼 있다 — `a11y-guard.test.ts:74-86`(Filter/Panel 이름의 파일에 current 계열 속성 0건) · `:88-106`(**`filterItemStyle(` 을 부르는 파일 전부**가 `aria-pressed` 를 낸다 — 파일 이름이 아니라 규칙을 스캔하므로 `EsgListPage` 가 그 스타일을 직접 쓰지 않아도 소비 경로가 덮인다) | grep `apps/admin/src` (주석·테스트 제외): `aria-current` = 0건. RTL: '환경' 버튼 클릭 → `getByRole('button', { name: /환경/ })` 의 `aria-pressed === 'true'`, 나머지 3개는 `'false'`. `a11y-guard.test.ts` 통과 | **pass** |
| MOTION-01 | MOTION | 상속 | 표면: 삭제·일괄삭제·충돌·discard Modal 의 enter/exit. 이 화면은 소비만 한다. **PR #26 이후의 사실 갱신**(판정 주체는 그대로 DS): 오버레이 모션이 **구현됐고 CSS-only 다** — backdrop fade + dialog scale(`Modal.css:20-21,35-38,58-59`, keyframes `:126-168`), reduced-motion 게이트 `:173-180`. AnimatePresence 는 없으나 'exit 완료 후 unmount' 는 `onAnimationEnd`(`Modal.tsx:216-218`)가 동등 달성한다. **다만 이 화면의 다이얼로그 4종은 전부 `ConfirmDialog` 이고 그 footer 버튼(취소 `ConfirmDialog.tsx:145`·확인 `:153`)은 `Modal` 을 거치지 않아 즉시 언마운트된다**(`Modal.tsx:27-31`) — 애니메이션되는 것은 Esc/딤/× 뿐이다 | DS Modal 소유 문서의 판정을 따른다 | 종속 |
| MOTION-02 | MOTION | 상속 | 표면: 삭제·저장·취소 토스트의 exit. 이 화면은 `toast.*` 를 호출만 한다. **PR #26 이후의 사실 갱신**(판정 주체는 그대로 DS): **exit 가 완전 구현됐다** — `Toast.css:32-37`(`tds-toast-out … forwards` + `pointer-events:none`) · keyframes `:121-131`(opacity 1→0 · `translateY(0)→translateY(var(--tds-space-3))`) · reduced-motion 게이트 `:136-141`. `Toast.tsx:186-187` 이 `onAnimationEnd` 로 큐 제거(`ToastProvider.tsx:99-100`)를 퇴장 애니메이션 뒤로 미룬다. `component.overlay` recipe 로 exit = fast(150ms)/accelerate — 요구 문구를 정확히 충족 | ToastProvider 소유 문서의 판정을 따른다 | 종속 |
| MOTION-03 | MOTION | 상속 | 표면: 위 Modal·Toast 의 reduced-motion 게이트 — **PR #26 이 더한 애니메이션은 각자 게이트를 갖고 태어났다**(`Modal.css:173-180` · `Toast.css:136-141`). **이 화면 자체에 transition/transform 선언이 0건**(ToggleSwitch 도 없다 — DS 전역으로는 그 게이트도 이제 실재한다: `ToggleSwitch.css:79-84`). 행 추가/삭제 FLIP 도 없다(MOTION-04 P1 — `CrudTable` 소관) | 전역 Motion config/DS 소유 문서의 판정을 따른다 | 종속 |
| IA-01 | IA | 직접 | 세 라우트 전부 AppShell 레이아웃 라우트 **안에** 있다 — `App.tsx:192-194`(`/company/esg` · `/company/esg/new` · `/company/esg/:id/edit`)가 `APP_ROUTES` 에 있고 `:334-336` 이 그것을 `RequireAuth > AppShell`(`:324-330`) 하위에 렌더한다. 세 화면 모두 자체 sidebar/top bar/outer frame 을 도입하지 않는다 — 최상위가 `EsgListPage` 의 2열 grid(`:95`)와 `FormPageShell` 의 flex column(`:146`)뿐이다. **좌측 분류 필터는 화면 안의 필터 패널이지 앱 nav 가 아니다**(`EsgListPage.tsx:98` 이 `navLabel="ESG 분류 필터"` 를 넘기고 공용 `FilterPanel.tsx:122` 가 `<nav style={filterNavStyle} aria-label={navLabel}>` 로 그린다 — 자체 라벨을 가진 보조 nav 이며 AppShell 의 '주 내비게이션'을 대체하지 않는다) | 세 경로 진입 → 사이드바 1개 · AppHeader 1개 · padded `<main>` 1개. `pages/company/esg/**` grep: `<aside`/`<header>` = 0. (필터 레일의 `<aside>` 는 공용 `FilterRail`(`FilterPanel.tsx:188`)의 것이며 **화면 안 좌측 열**이지 앱 프레임이 아니다) | pass |
| IA-02 | IA | 직접 | **하위 라우트가 branch 라벨로 폴백하고, 폼에 `<h1>` 이 2개다.** `/company/esg` 는 nav leaf 라(`nav-config.ts:128`) AppHeader 가 'ESG' 를 옳게 보인다. **그러나 `/company/esg/new`·`/company/esg/:id/edit` 는 leaf 가 아니다** → `findNavLabel`(`nav-config.ts:253-264`)의 exact 매치(`:254`)가 실패하고 branch 폴백(`:257-262` `pathname.startsWith('/company')`)이 **'기업 관리'** 를 돌려준다 → `AppHeader.tsx:101` 이 `<h1>기업 관리</h1>` 를 그린다. 동시에 `FormPageShell.tsx:160` 가 `<h1>ESG 활동 등록</h1>` 를 그린다 — **한 화면에 `<h1>` 2개, 상위 것이 화면을 지목하지 못한다**(quality-bar 의 `/content/notices/new` 예시와 정확히 같은 결함). 게다가 **title 소스가 화면 타입마다 다르다**: 목록은 AppHeader 만('ESG', in-content 제목 없음), 폼은 AppHeader + in-content h1 둘 다. F2 가 `AppHeader.tsx` 를 수정했으나(+15줄) 그 변경은 `titleStyle` 을 `pageTitleStyle` 로 바꾼 TOKEN-05 건이고 **`findNavLabel` 폴백은 그대로다** | `/company/esg/new` 진입 → 화면 상단 title = **'기업 관리'**(기대: 'ESG 활동 등록'). `document.querySelectorAll('h1').length === 2`, `[0].textContent === '기업 관리'`. `/company/esg` → h1 1개('ESG'), 본문에 제목 없음 | **gap** |
...
| EXC-03 | EXC | 직접 | **읽기 게이팅은 성립하나 쓰기 게이팅이 전무하다.** ① read ✅: `AppShell.tsx:490` `<RequirePermission>` 이 `<Outlet>` 을 감싸 read 권한이 없으면 `ForbiddenScreen` 을 그린다(`RequirePermission.tsx:61-64`). 리소스는 `route-resource.ts:36-46` 이 경로에서 파생하므로 **`/company/esg/new`·`/:id/edit` 같은 비-leaf 라우트도 덮인다**(`:26-28` 이 세그먼트 경계로 감싸는 가장 구체적인 잎을 찾는다). ② **write ❌**: 쓰기 컨트롤 **전부**가 `can(resource, action)` 을 보지 않는다 — 'ESG 활동 등록'(`EsgListPage.tsx:87`) · 일괄 삭제(`CrudListShell.tsx:126-132`) · 행 수정/삭제(`CrudTable.tsx:192-197`) · 폼 저장(`FormPageShell.tsx:189`). `useRouteWritePermissions`(`RequirePermission.tsx:45-52`)가 존재하고 **소비자는 F3b 이후 7곳이지만(products 3 · settings 4) `pages/company/**` 는 그 목록에 없다** (`grep -rn "useRouteWritePermissions\|useRouteCan" pages/company/` → **0건**) — '앱 전역 미구현'이 아니라 **이 섹션의 미적용**이며 배선 선례가 이미 앱 안에 있다(`settings/site/SiteSettingsPage`). ③ **강등 reconcile ❌**: 버튼이 애초에 권한을 구독하지 않으므로 강등해도 사라지지 않는다 | 권한 스토어에서 `company-esg` 의 `remove` 를 끈 역할로 진입 → **행 삭제·일괄 삭제 버튼이 그대로 보이고 눌린다**(기대: 미렌더 또는 disable). `read` 를 끄면 403 화면(이쪽은 통과). grep: `pages/company/**` 에서 `useRouteWritePermissions` = **0건**(단 `pages/**` 전체로는 7곳 — products 3 · settings 4) | **gap** |
...
| EXC-08 | EXC | 직접 | 이 화면이 **`useCrudForm` 을 소비한다** — `EsgFormPage.tsx:48-69`. 그 훅이 세 겹을 모두 갖춘다: ① **동기 잠금** `useCrudForm.ts:103` `const submitLockRef = useRef(false)` → `:195-196` `if (submitLockRef.current) return; submitLockRef.current = true;` 가 **RHF 비동기 검증 뒤 `disabled` 가 켜지기 전의 틈**을 닫는다(`:94-101` 주석이 그 틈을 명시). `onSettled`(`:207-209`)가 잠금을 풀고 `onInvalid`(`:239-241`)도 푼다 ② **`disabled`** `FormPageShell.tsx:189` `disabled={saving \|\| loadingDetail}` ③ **제출 시도 단위 멱등키** `:112-117` `idempotencyKeyRef` 를 **mutationFn 밖**(ref)에서 만들어 재시도가 같은 키를 재사용하고, 성공하면 폐기해 다음 제출이 새 거래가 된다(`:214`). ⚠ 키가 **어댑터로 전달되지는 않는다**(`CrudAdapter.create/update` 시그니처에 자리가 없다 — BE-022 §7.10) — 그러나 요구의 acceptanceCheck 는 '키가 mutationFn 밖에 존재' 이며 그것은 성립한다. 전달 배선은 BE-022 §7.10 · §5 로 이관 | 제목을 고치고 저장 버튼에 포커스 → Enter 를 빠르게 2회 → `esgAdapter.create/update` 가 **1회만** 호출된다. `?fail=esg:save` 로 실패 후 재클릭 → 같은 `Idempotency-Key`(현재는 생성만 되고 미전송) | pass |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| pass | **13** | STATE-01 · STATE-02 · STATE-04 · TOKEN-01 · **A11Y-11** · FEEDBACK-02 · FEEDBACK-04 · A11Y-12 · IA-01 · IA-05 · EXC-04 · EXC-08 · EXC-09 |
...
| gap | **4** | IA-02 · IA-04 · IA-13 · EXC-03 |

**합계 검산: 13 + 11 + 2 + 4 = 30 ✓** (quality-bar P0 총계 30건과 일치)

> **직전 판정(`3cd3078`) 대비 A11Y-11 이 gap → pass 로 뒤집혔다** — 직전 문서가 '스크린리더에 필수 여부가 닿는 경로가 0개' 라 적은 그 자리를 F3a 가 닫았다(`FormField.withAriaRequired` 주입 + `TextareaField` 자체 배선). **이 화면은 `ImageGalleryField` 에 `required` 를 주지 않아** 그 몰리큘의 잔여 결함(required 미주입 — `ImageGalleryField.tsx:213-215`)을 밟지 않는다 — NFR-019(인증서)·NFR-020(파트너사)는 `ImageUploadField required` 를 써서 밟는다. **판정이 갈리는 지점이 '어느 몰리큘에 required 를 주는가' 다.**
>
> ⚠ **P0 gap 4건** — quality-bar §How to use 는 'P0 하나라도 미충족이면 배치 실패' 로 규정한다. 범위: **IA-13 은 이 화면 고유**(`useListState` 가 존재하고 **같은 섹션의 `company/logo-list` 가 이미 소비하는데** 이 화면은 하지 않는다 — 가장 값싸게 닫히는 gap). **IA-02 는 절반 해소**(가지 라벨 폴백은 통합이 닫았고 h1 이중 + 행위 미반영이 남았다 — §2 참조). **EXC-03 은 범위가 바뀌었다**(앱 전역 0건 → products·settings 7곳이 이미 소비, 기업 관리 섹션만 미적용). **IA-04 는 여전히 앱/계약 결손**이다. 상세는 §5 참조.
...
| COMP-05 | P2 | **필터 스타일을 이 화면이 전혀 선언하지 않는다** — 공유 컴포넌트를 통째로 소비하기 때문이다. `EsgListPage.tsx:8` 이 `FilterPanel`·`FilterRail` 을 `shared/ui` 에서 가져와 `:96-105` 에서 props 만 넘긴다. `filterPanelStyle`·`filterNavStyle`·`filterListStyle`·`filterHeadingStyle`·`filterItemStyle`·`badgeStyle` 을 import 하는 쪽은 이제 그 공용 컴포넌트다(`FilterPanel.tsx:23-31`). 이 화면의 로컬 `CSSProperties` 는 필터와 무관한 3개뿐이다(`EsgListPage.tsx:25,32,40`). **⚠ 정정**: 직전 판정은 '필터 파일에 로컬 `CSSProperties` 선언이 0건' 이라 적었는데, 지금 `FilterPanel.tsx` 는 `scrollListStyle`·`footerStyle`·`retryRowStyle`·`retryButtonStyle` 4개를 선언한다(`:42-75`) — **다만 전부 토큰 전용**이며(TOKEN-01 스캔 통과) 이 화면이 쓰지 않는 곁가지 축(스크롤·footer·재시도 줄)의 것이다. 판정은 그대로 **pass** 다 — 요구가 묻는 것은 '이 화면이 필터 스타일을 재선언하는가' 이고 답은 0건이다. quality-bar 가 `(ESG는 이미 공유 사용)` 이라 적은 그대로다 | grep `pages/company/esg/**`: `filterPanelStyle`/`filterItemStyle` 로컬 선언·import = 0 · `CSSProperties` = `EsgListPage` 3 + `EsgFormPage` 1(전부 필터 밖). `EsgListPage.tsx:8` 의 `FilterPanel, FilterRail` import 존재 | **pass** |
...
| A11Y-13 | P1 | ① submit 검증 실패 → 첫 invalid 필드 포커스 ✅ — `useCrudForm.ts:246-248-241,253` 이 `onInvalid` 를 **명시해 계약으로 고정**한다(`:233-238` 주석: RHF `shouldFocusError` 기본값이 바뀌어도 동작 유지). 서버 422 도 첫 위반 필드로 포커스(`:184`). ② **폼 진입 시 첫 필드 자동 포커스 ❌** — 그 코드가 없다 | 빈 제목 제출 → `document.activeElement` = 제목 입력 ✅. `/company/esg/new` 진입 → activeElement = `<main>`(라우트 포커스) ❌ | 부분 gap |
| A11Y-14 | P2 | `ImageGalleryField` 의 업로드 완료/실패 announce — DS 소유. **이 화면은 업로드가 실재하지 않아**(blob: 생성뿐 — FS-022 §7 #9) 비동기 결과 자체가 없다 | DS 소유 문서 | 종속 |
| A11Y-16 | P1 | 이 화면이 도입한 인터랙티브 표면 = 분류 필터 버튼 4개 — **직접 만든 것이 아니라 공용 `FilterPanel` 을 소비해 얻는다**(`EsgListPage.tsx:96-105`). 계약 충족: semantic role(`FilterPanel.tsx:142-143` `<button type="button">`) · keyboard(네이티브) · focus ring(`:144` `tds-ui-focusable`) · **이중(비색상) state 인코딩**(`:145-146` `aria-pressed` + `filterItemStyle(active)` 의 배경/색) · 비동기 status announce(`CrudListShell.tsx:107-109` 의 지속 live region — `:99-105` 주석이 '내용과 함께 생성된 region 은 announce 되지 않는다' 는 이유로 껍데기가 소유하게 한 판정을 기록) | RTL: 필터 버튼 4개의 role/`aria-pressed`. 필터 변경 → live region 에 'ESG 활동 N건을 찾았습니다.' 주입 | pass |
...
| ERP-06 | P1 | **뒤집혔다(gap → pass).** 사용자 대상 문구가 존댓말로 통일돼 있고, **검증 문구의 조사도 이제 파생이다**(ERP-13 참조 — 통합에서 해소). 직전 판정의 '조사 fallback 을 출하한다' 는 폐기 | 빈 제목 제출 → **'제목을 입력하세요.'** ('제목'은 종성 ㄱ 이라 `을`) | pass |
| ERP-08 | P2 | **일자 셀이 `shared/format` 을 경유하지 않는다** — `EsgListPage.tsx:82` `render: (item) => item.date`(원문 'YYYY-MM-DD'). 건수는 `formatNumber` 경유 ✅(공용 `FilterPanel.tsx:22,154` · `CrudListShell.tsx:119`) | 일자 셀이 raw 문자열 | 부분 gap |
| ERP-12 | P1 | 목록 export(CSV/xlsx) 없음 — 툴바에 등록 버튼뿐 | export 버튼 0개 | gap (앱 전역) |
| ERP-13 | P1 | **통합에서 뒤집혔다(gap → pass).** 조사 헬퍼가 **`shared/format.ts:269+` 로 승격**됐다 — 이전엔 사본이 셋이었고(`logs/josa.ts` · `notifications/_shared/notification.ts` · `@tds/ui` 의 `Empty`) 앱 shared 에는 없었다. 지금 `requiredText` 가 그 헬퍼로 조사를 조립한다 — `shared/crud/validation.ts:17`(`${label}${objectParticle(label)} 입력하세요.`) · `:21,24`(`${label}${topicParticle(label)} …자를 넘을 수 없습니다.`). 받침 판정은 한글 음절의 종성 코드로 한다(`format.ts:281-295`), 한글이 아니면 관용대로 받침 없음('API를'). 직전 판정이 지목한 세 자리가 전부 그것을 소비한다 — 저장 성공 토스트 `useCrudForm.ts:222`(`${config.entityLabel}${objectParticle(config.entityLabel)} ${verb}했습니다.`) → **'ESG 활동을 등록했습니다.'**('활동'은 종성 ㅇ) · 삭제 토스트 `useCrudList.tsx:108` · 삭제 확인 `:158`. `CrudTable.tsx:59-72` 의 빈 상태도 `emptyLabel` 한 문자열이 아니라 `{createVerb}된 {label}이(가) 없습니다` 를 **조사 파생으로 조립**하고 검색/필터 분기를 갖는다 | **`grep -rn "을(를)\|이(가)\|은(는)" pages/company/` → 0건**(사용자 대상 리터럴). 앱 전역 grep 히트 12건이 남지만 전부 ① 주석 ② '이 리터럴을 내지 않는다'를 단언하는 테스트 ③ 헬퍼 자신의 설명문이다 | pass |
...
| 24 | — | — | **공용 `FilterPanel`/`FilterRail` 에 렌더 테스트가 없다** — quality-bar 축이 아니라 검증 결손(§6.2 · §7). 좌측 필터 표면이 화면 전용 파일에서 `shared/ui/FilterPanel.tsx` 로 수렴하면서 회귀 반경이 아홉 화면으로 커졌는데, 덮는 것은 `a11y-guard.test.ts:74-106` 의 정적 스캔뿐이다. 필요한 것: 선택 전이(`aria-pressed` 가 하나만 `true`) · 배지(`counts` 객체 vs `null` → `'—'`) · `failed`/`onRetry` 줄 · `scroll`/`footer` 곁가지 | 공유(`shared/ui/FilterPanel.tsx`) | 프론트 구현 |
...
| 축 | 도구 | 비고 |
|---|---|---|
| A11Y-12 | **`shared/a11y-guard.test.ts:74-86`** — Filter/Panel 이름의 파일에 `aria-current` 가 없는지 검사(주석 제거 후 — `:120-125`). **`:88-106`** 은 파일 이름이 아니라 규칙을 스캔한다 — `filterItemStyle(` 을 부르는 파일 전부가 `aria-pressed` 를 내야 하며, `:98` 이 `FilterPanel.tsx` 가 그 목록에 실제로 있는지부터 못 박는다(`:88-93` 주석이 '이름을 세지 않는 이유' 로 공유 수렴을 기록). Pagination 의 `aria-current="page"` 는 진짜 navigation 이라 정당하다고 기록(`:70-72`) | §2 A11Y-12 pass 를 CI 가 고정한다 — **단 정적 스캔이며 렌더 전이는 확인하지 않는다**(§6.2 '순수 규칙' 행) |
...
| 순수 규칙 | **`esg.test.ts`** — `sortEsg`(일자 내림차순) · `filterEsg`(전체/환경/지배구조) · `countEsgByCategory` · `esgCategoryLabel` · `esgSchema` 6케이스(정상·분류 union·제목/내용 빈값·일자 형식·이미지 다중·이미지 상한) | **화면 렌더 테스트는 없다** — `EsgListPage`/`EsgFormPage` 의 RTL 테스트가 존재하지 않는다. **좌측 필터도 마찬가지다**: 그 표면은 공용 `shared/ui/FilterPanel.tsx` 로 옮겨 갔는데 **그 파일에도 전용 테스트가 없다**(`shared/ui/` 의 테스트는 `ToastProvider.test.tsx`·`useModalDirtyGuard.test.tsx` 둘뿐). 필터를 덮는 자동 검증은 `a11y-guard.test.ts:74-106` 의 **정적 스캔**(`aria-current` 부재 + `filterItemStyle(` 소비 파일의 `aria-pressed` 존재)뿐이며 **렌더 후 상태 전이를 확인하지 않는다** — 예: '환경' 클릭 후 나머지 3개가 `aria-pressed="false"` 인지, 배지 건수가 `counts` 를 따라가는지. §2 의 A11Y-11·A11Y-12·IA-02 판정은 코드 대조로만 이뤄졌다(§7 참조) |
...
- [x] **'공용 모듈이 있어도 이 화면이 쓰는지' 를 코드로 확인했다** — `useCrudList` ✅ 소비(STATE-01 pass) · `useCrudForm` ✅ 소비(EXC-08·EXC-04 pass) · `useListState` ❌ 미소비(IA-13 gap) · `useDebouncedSearch` ❌ 미소비(COMP-10 은 표면 부재로 n-a) · `useModalDirtyGuard` ❌ 미소비(FEEDBACK-06 n-a) · `useRouteWritePermissions` ❌ 미소비(EXC-03 gap) — **F3b 이후 앱의 다른 7곳(products 3 · settings 4)은 소비한다. 이 섹션만 미적용이다**
- [x] **A11Y-12 를 직접 재확인했다** — 이 화면이 소비하는 공용 `FilterPanel.tsx:146` `aria-pressed={active}` · `apps/admin/src` 의 `aria-current` 사용 0건(잔여 매치는 전부 주석·검사 코드) · 회귀 기록은 `FilterPanel.tsx:9-10,17-19` → **pass**
- [x] **COMP-05 를 직접 확인했다** — `EsgListPage.tsx:8` 이 `FilterPanel`/`FilterRail` 을 소비하고 **이 화면에 필터 스타일 선언이 0건** → **pass**(§3). 공용 컴포넌트 자신은 토큰 전용 `CSSProperties` 4개를 갖는다(`FilterPanel.tsx:42-75`) — 판정에 영향 없음
- [x] **인용 경로를 실파일로 재대조했다(v1.1)** — 직전 판정(v1.0)은 `apps/admin/src/pages/company/esg/EsgCategoryFilter.tsx` 를 이 화면의 구현 파일로 인용했으나 **그 파일은 존재하지 않는다**. 실제 디렉터리는 `EsgFormPage.tsx` · `EsgListPage.tsx` · `data-source.ts` · `esg.test.ts` · `types.ts` · `validation.ts` **6파일뿐이며 components 하위 디렉터리가 없다**. 그 이름으로 적혀 있던 동작은 전부 **공용 `shared/ui/FilterPanel.tsx`(`FilterPanel`/`FilterRail`)** 에 실재하며, 이번 개정에서 TOKEN-01 · TOKEN-02 · COMP-10 · A11Y-12 · A11Y-16 · IA-01 · COMP-05 · ERP-08 · §6.2 를 그쪽으로 재고정했다. **뒤집힌 판정은 없다** — 근거 위치만 옮겼다
- [ ] **⚠ 남은 결손(이번에 새로 드러남)**: 좌측 필터의 **렌더 테스트가 앱 어디에도 없다** — `pages/company/esg/**` 에도, 표면이 옮겨 간 `shared/ui/` 에도 없다(`shared/ui/` 의 테스트는 `ToastProvider.test.tsx`·`useModalDirtyGuard.test.tsx` 둘뿐). 아홉 화면이 이 한 벌을 공유하므로(`FilterPanel.tsx:3-7`) 회귀 반경이 ESG 한 화면이 아니라 **아홉 화면**이다. `a11y-guard.test.ts:74-106` 의 정적 스캔은 속성의 **존재**만 보고 `aria-pressed` 가 선택 항목만 `true` 인지, 배지가 `counts`/`null` 을 옳게 가르는지는 보지 않는다 → **§5 #24 로 이관**
