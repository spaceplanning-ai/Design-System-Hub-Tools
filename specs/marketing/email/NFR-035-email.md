...
| COMP-10 | COMP | **직접** | **충족하지 않는다.** 검색 입력이 `useState` 에 직결돼 있다 — `<SearchField value={keyword} onChange={setKeyword} />`(`EmailListPage.tsx:124-129`, 상태는 `:102`). 디바운스도, `isComposing`/`compositionstart`/`compositionend` 처리도 없다. 공용 `useDebouncedSearch`(250ms + IME 조합 판정 + 조합 중 Enter 차단)가 앱에 존재하나 **소비처가 `MembersPage` 한 곳뿐**이라 이 화면은 쓰지 않는다 | 검색창에 '뉴스레터'를 IME 로 입력 → 자모 단위 change 마다 `searchEmailCampaigns` 가 재실행되고 `visible` 이 매번 다시 계산된다. 조합 중 Enter 가 그대로 상위로 전파된다. **완화 요인**: 검색이 클라이언트 필터라 네트워크 요청·응답 경합은 발생하지 않는다 — 요구의 3개 절 중 'stale 응답이 최신을 덮지 않는다'는 표면 부재로 성립하고, **'조합 중 커밋 금지'·'디바운스+최소 길이'가 미충족**이다 | **gap** |
...
| IA-02 | IA | **직접** | **충족하지 않는다.** title 소스가 라우트마다 모순된다. 목록(`/marketing/email`)은 AppHeader 의 `<h1>` 만 갖고(잎 정확 일치 → '이메일 발송') 본문에 제목이 없다. **폼(`/new`·`/:id/edit`)은 `<h1>` 이 둘이다**: `findNavLabel` 이 잎에 정확히 일치하는 것을 못 찾아 브랜치 라벨로 폴백해(`nav-config.ts:257-262`) AppHeader 가 **'마케팅 관리'** 를 `<h1>` 으로 그리고(`AppHeader.tsx:92,101`), 폼 본문이 '이메일 발송 등록'을 또 `<h1>` 으로 그린다(`EmailFormPage.tsx:252`). 요구가 명시적으로 금지한 'sub-route 의 branch label 노출'이 그대로 성립한다 | `/marketing/email/new` 진입 → 가시 primary title 이 **'마케팅 관리'**(AppHeader). `document.querySelectorAll('h1')` → **2개**('마케팅 관리', '이메일 발송 등록'). `findNavLabel('/marketing/email/new')` 를 직접 호출 → `'마케팅 관리'` | **gap** |
| IA-04 | IA | **직접** | **부분 충족.** 템플릿의 앞 4단은 지킨다 — toolbar(좌측 검색+필터 / **우상단 '이메일 발송 등록'** `EmailListPage.tsx:121-150`) → 결과 count 요약(`CrudListShell.tsx:115-123`) → SelectionBar(`:125-133`) → table(`:135-154`). **마지막 단인 Pagination 이 없다**: `CrudListShell` 이 페이지네이션을 렌더하지 않고 `marketing/**` 전역에 `Pagination` 참조가 0건이다. 캠페인 수에 상한이 없어 'page size 초과 가능'은 참이다 | `grep -rn "Pagination" apps/admin/src/pages/marketing` → **0건**. 픽스처를 100건으로 늘리면 100행이 한 표에 그려진다(page 컨트롤 없음). 우상단 primary action 과 count 요약은 존재 | **gap** |
| IA-05 | IA | 직접 | `/marketing/email/new` 와 `/marketing/email/:id/edit` 가 **같은 `EmailFormPage` 컴포넌트**로 해석된다(`App.tsx:264-265`). `useCrudForm` 이 `useParams().id` 유무로 `isEdit` 를 파생하고(`useCrudForm.ts:73-74`) 레이아웃은 동일한 채 title(`EmailFormPage.tsx:252`)·prefill(`useCrudForm.ts:125-128` reset)·제출 라벨(`:428`)만 갈린다. create 전용/edit 전용 페이지가 없다 | `App.tsx:264-265` 의 두 라우트 `element` 가 동일 컴포넌트 참조. `/new` 와 `/em-1/edit` 를 나란히 열어 레이아웃 동일 · 제목만 '등록'/'수정' 확인 | pass |
| IA-13 | IA | **직접** | **충족하지 않는다.** 목록 조회 상태가 컴포넌트 `useState` 에만 있다 — `const [filter, setFilter] = useState(EMAIL_FILTER_ALL)` · `const [keyword, setKeyword] = useState('')`(`EmailListPage.tsx:101-102`). URL 쿼리스트링에 아무것도 직렬화되지 않는다(`useSearchParams` 미사용). 공용 `useListState`(URL 단일 원천 + page clamp + 선택 해제)가 존재하나 **소비처가 `MembersPage` 한 곳뿐**이다 | 상태 필터를 '예약'으로 → URL 이 `/marketing/email` 그대로. F5 → 필터가 '전체 상태'로 초기화. 행을 열어 수정 화면 진입 후 Back → 검색어·필터가 사라진 초기 목록. 필터 걸린 view 의 링크를 만들 방법이 없다 | **gap** |
...
| EXC-03 | EXC | **직접** | **부분 충족.** (a) read 게이팅은 상속으로 충족된다 — `AppShell` 이 `<Outlet>` 을 `RequirePermission` 으로 감싸(`AppShell.tsx:490-492`) 권한 없는 deep-link 에 `ForbiddenScreen` 을 그리고, 리소스는 라우트에서 파생된다(`route-resource.ts:36-46` — `/marketing/email/1/edit` → 잎 `/marketing/email`). (b) **write-action 게이팅이 이 화면에 없다**: '이메일 발송 등록' 버튼(`EmailListPage.tsx:145-148`)·행 액션 수정/삭제(`CrudTable.tsx:192-197`)·일괄 삭제(`CrudListShell.tsx:126-132`)가 권한과 무관하게 렌더된다. 공용 `useRouteWritePermissions` 가 존재하나(`RequirePermission.tsx:45-52`) **앱 전체에서 소비처가 0건**이다 | ① 권한 없는 역할로 `/marketing/email` deep-link → 403 화면(pass). ② `grep -rln "useRouteCan\|useRouteWritePermissions" apps/admin/src` → `RequirePermission.tsx` **한 파일뿐**(자기 정의). remove 권한 없는 역할로 목록 진입 → 행 삭제 버튼과 '선택 N건 삭제'가 **그대로 보이고 눌린다** → **요구의 'create/update/delete 컨트롤은 can()이 false면 렌더 안 함(또는 disable)' 절 미충족** | **gap** |
| EXC-04 | EXC | 직접 | 폼이 `useCrudForm` 을 소비하고(`EmailFormPage.tsx:168-177`) 그것이 409/412 를 `isConflict` 로 잡아 **입력을 보존한 채** 충돌 다이얼로그를 연다 — 성공 토스트도 목록 이동도 없다(`useCrudForm.ts:158-172`). 화면이 `FormConflictDialog` 를 렌더한다(`EmailFormPage.tsx:433`). 어댑터도 없는 id 의 update/remove 를 조용히 통과시키지 않고 409 를 던져 '유령 저장'을 막는다(`crud.ts:71-74,82-84`) | `?status=marketing-email:save:409` → `/marketing/email/em-1/edit` 에서 저장 → 충돌 다이얼로그가 뜨고 입력이 그대로 살아 있으며 성공 토스트·이동이 없다. '이어서 편집' → 다이얼로그만 닫히고 입력 유지. **주의**: `EmailCampaign` 에 `updatedAt`/`version` 이 없어 `If-Match` 를 보내지 못한다 — **삭제 충돌은 잡히나 동시 수정 충돌은 잡히지 않는다**(BE-035 §7.2 · §3 EXC-04 보강) | pass |
| EXC-08 | EXC | 직접 | 폼 제출이 `useCrudForm` 의 **동기 제출 락**을 탄다 — `submitLockRef` 가 렌더를 기다리지 않고 두 번째 제출을 즉시 반환시킨다(`useCrudForm.ts:102,195-197`). `disabled={saving \|\| loadingDetail}`(`EmailFormPage.tsx:427`)와 이중이다. **멱등키가 `mutationFn` 밖(ref)에서 생성**돼(`useCrudForm.ts:112-117`) 재시도가 같은 키를 재사용하고 성공 시 폐기된다(`:214`). 뮤테이션 자동 재시도는 꺼져 있다(`queryClient.ts` — `mutations.retry: false`) | 제출 버튼 더블클릭 / 응답 전 Enter 연타 → 정확히 1개 요청. `useCrudForm.ts:112` 의 키가 `create.mutate` 호출 **밖**에서 생성됨을 코드로 확인. **주의**: 이 화면에서 EXC-08 이 진짜로 중요해지는 지점(발송)은 **UI 가 없어 아직 성립하지 않는다** — FS-035-EL-035 · §3 | pass |
...
> **⚠ P0 gap 9건 — quality-bar §How to use 기준 '배치 실패' 사유다.** **NFR-034(SMS)와 판정이 완전히 동일하다** — 같은 프레임워크·같은 `_shared` 를 쓰는 형제 화면이므로 gap 도 형제다. 성격이 갈린다: **이 화면 소유 6건**(COMP-10 · A11Y-11 · IA-02 · IA-04 · IA-13 · EXC-03)과 **앱 전역/DS 소유 3건**(MOTION-01/02/03 — Motion 라이브러리 미도입). 후자는 이 화면만 고쳐서 닫을 수 없다.
...
|---|---|---|---|---|
| STATE-03 | P1 | `useCrudListQuery` 가 `placeholderData: (previous) => previous`(`crud.ts:151`)로 이전 행을 유지하고 `refreshing` 이 가벼운 인디케이터('· 새로고침 중…')만 띄운다(`CrudListShell.tsx:118-122`). `staleTime: 30_000` | 목록이 있는 상태에서 재조회 → 이전 행 유지 + 요약에 '새로고침 중…' | pass |
...
| **ERP-09** | **P2** | **이메일은 SMS 와 갈린다 — 야간 판정 자체가 없다.** `emailSchema` 에 `isNightAt` 호출이 0건이다(`validation.ts:85-105`) — SMS 는 있다(`sms/validation.ts:106-114`). 따라서 '야간 판정의 TZ 가 브라우저 로컬이라 위험하다'는 ERP-09 의 SMS 판정이 여기서는 **표면 부재로 성립하지 않는다**. **그러나 과거 예약 판정은 남는다** — `Date.now()` 기준(`validation.ts:17-21`)이고 `scheduledAt` 이 오프셋 없는 `datetime-local` 문자열로 저장된다(`types.ts:24`). 서버가 어느 TZ 로 해석할지 계약에 없다. **더 중요한 것은 야간 제한 부재 자체가 미결이라는 사실이다** — 의도인지 누락인지 코드에도 문서에도 근거가 없다(FS-035 §7 #7 · BE-035 §7.5) | `grep -n "isNightAt" apps/admin/src/pages/marketing/email/validation.ts` → **0건**. 광고성 + 23시 예약 → **통과한다**. `scheduledAt` 저장값이 `'2026-07-22T10:00'`(오프셋 없음)임을 픽스처에서 확인 | **gap**(TZ 계약 부재 · 야간 정책 미결) |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | **IA-13** | **P0** | 검색어·상태 필터가 `useState` 에만 있다(`EmailListPage.tsx:101-102`) — URL 미직렬화. F5·Back·링크 공유로 복원 불가. 공용 `useListState` 소비처가 `MembersPage` 뿐 | 이 화면(+앱의 다른 목록 다수) | A11 (FS-035 §7 #2) |
| 2 | **COMP-10** | **P0** | 검색 입력에 IME 조합 판정·디바운스가 없다(`EmailListPage.tsx:124-129`). 공용 `useDebouncedSearch` 미소비 | 이 화면 | A11 (FS-035 §7 #2) |
| 3 | **A11Y-11** | **P0** | `SegmentPicker` 의 required 표식이 `aria-hidden` `<span>` 이고 `aria-required` 가 없으며 오류가 `aria-describedby` 로 연결되지 않는다(`SegmentPicker.tsx:109-113,137-140`). **수신거부 토글도 같다**(`EmailBodyCard.tsx:117-120`) — `ToggleSwitch` 가 `aria-describedby` prop 을 노출하지 않는다. **이메일은 SMS 보다 한 곳 더 걸린다** | 이 화면 + `_shared`(SMS·뉴스레터 공유) + DS | A11 · A40 (FS-035 §7 #3) |
| 4 | **IA-02** | **P0** | 폼 라우트에 `<h1>` 이 둘 — AppHeader 가 브랜치 라벨 '마케팅 관리'를 그리고(`AppHeader.tsx:92,101` → `nav-config.ts:257-262`) 폼이 '이메일 발송 등록'을 또 그린다 | 앱 전역(모든 하위 라우트) | A40 · A11 (FS-035 §7 #1) |
| 5 | **IA-04** | **P0** | Pagination 이 없다 — `marketing/**` 전역 `Pagination` 참조 0건 | 이 화면 + `CrudListShell` | A11 (FS-035 §7 #12) |
| 6 | **EXC-03** | **P0** | 쓰기 권한 게이팅이 없다 — 등록·수정·삭제 버튼이 권한과 무관하게 렌더. `useRouteWritePermissions` 소비처 0건 | 앱 전역 | A11 · A63 (FS-035 §7 #13 · BE-035 §7.11 #11) |
...
| 13 | ERP-09 | P2 | 과거 예약 판정이 브라우저 로컬 TZ(`validation.ts:17-21`)이고 `scheduledAt` 이 오프셋 없는 문자열이다 — 서버 해석 TZ 가 계약에 없다. **더불어 야간 광고 제한 자체가 이메일에 없고 그 근거가 어디에도 없다**(`validation.ts:85-105` — `isNightAt` 0건) | 이 화면 + `_shared` | **A01 · A63** (FS-035 §7 #7 · BE-035 §7.5) |
