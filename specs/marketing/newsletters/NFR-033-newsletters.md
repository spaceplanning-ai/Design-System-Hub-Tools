...
| COMP-10 | COMP | 직접 | **미충족.** 검색 입력이 `SearchField` 에 `onChange={setKeyword}` 를 직결한다(`NewsletterListPage.tsx:136-141`) — 키 입력마다 즉시 `keyword` state 가 바뀐다. IME 조합 처리(`isComposing`/`compositionstart`)·디바운스·최소 길이 정책이 **없다**. `shared/crud/useDebouncedSearch.ts`(이 셋을 전부 소유)가 존재하지만 **이 화면은 소비하지 않는다** — 앱 전체에서 이 훅의 소비자는 `MembersPage`/`MembersToolbar` 2파일뿐이다(grep 확인). 오늘은 클라이언트 필터라 네트워크 요청이 나가지 않아 **out-of-order race 는 발생하지 않지만**, 조합 중 매 자모마다 전 배열 재필터가 돌고, BE-033 §7.5 의 서버 검색 계약이 붙는 순간 이 입력이 바로 그 seam 이 된다 | 검색창에 '뉴스레터' 를 IME 로 입력 → `searchNewsletters` 가 'ㄴ','뉴','뉴ㅅ','뉴스'… 매 조합 단계마다 실행된다(React DevTools Profiler 또는 `searchNewsletters` 에 임시 카운터). 네 파일에 `isComposing`/`onCompositionStart` grep = 0. `useDebouncedSearch` import = 0 | **gap** |
...
| IA-02 | IA | 직접 | **미충족.** 두 곳이 각각 `<h1>` 을 그린다: ① `AppHeader` 가 `findNavLabel(pathname)` 로(`AppHeader.tsx:92` · `:101`) ② 폼이 '뉴스레터 등록'/'뉴스레터 수정' 으로(`NewsletterFormPage.tsx:241`). `findNavLabel` 은 정확히 일치하는 nav 잎이 없으면 **basePath 접두 일치로 branch 라벨을 반환**한다(`nav-config.ts:253-263`) — `/marketing/newsletters/new` 는 잎이 아니므로 **'마케팅 관리'** 가 된다. 결과: 등록 화면 상단에 '마케팅 관리', 본문에 '뉴스레터 등록' — **h1 2개 · 상위 제목이 모호한 branch 라벨**. F2 는 `AppHeader.tsx` 의 **스타일만** 고쳤고(`titleStyle` → `pageTitleStyle`) 라벨 유도 로직은 그대로다(F1→F2 diff 확인). 목록 라우트는 잎과 정확히 일치해 '뉴스레터' 가 나오지만, 목록에는 in-content 제목이 없다 — **list 는 헤더 제목, form 은 헤더+본문 제목**으로 소스가 모순 | `/marketing/newsletters/new` 진입 → `document.querySelectorAll('h1')` 이 **2개**를 반환하고 첫 번째 텍스트가 **'마케팅 관리'**(‘뉴스레터 등록’ 아님). `/marketing/newsletters` → h1 1개('뉴스레터'), 본문 제목 없음 | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** 목록 조회 상태(상태 필터·검색어)가 컴포넌트 `useState` 에만 있다(`NewsletterListPage.tsx:113-114`). URL 직렬화가 없다 — `useSearchParams` 미사용. `shared/crud/useListState.ts`(URL 직렬화 + page clamp + 선택 해제를 전부 소유)가 존재하지만 **이 화면은 소비하지 않는다** — 앱 전체 소비자는 `MembersPage`/`MembersToolbar` 2파일뿐이다(grep 확인). page/sort 는 애초에 존재하지 않는다(IA-04 gap) | '발송완료' 필터 + '7월' 검색 적용 → URL 이 `/marketing/newsletters` 그대로(쿼리스트링 없음). 행을 열어 수정 폼 진입 후 브라우저 Back → **필터·검색어가 초기화된 전체 목록**에 착지. URL 을 새 탭에 복사 → 필터 없는 기본 목록. `NewsletterListPage.tsx` 에 `useSearchParams`/`useListState` import = 0 | **gap** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| pass | **11** | STATE-01 · STATE-02 · STATE-04 · TOKEN-01 · FEEDBACK-02 · FEEDBACK-04 · IA-01 · IA-05 · EXC-04 · EXC-08 · EXC-09 |
...
| gap | **6** | COMP-10 · A11Y-11 · IA-02 · IA-04 · IA-13 · EXC-03 |
| **합계** | **30** | 11 + 11 + 2 + 6 = **30** ✅ |

> **P0 gap 6건 = quality-bar '배치 실패' 사유.** 이 중 **IA-02 · IA-04 · IA-13 · EXC-03 은 앱 전역 문제**이고(다른 마케팅·콘텐츠 화면이 같은 구조를 공유한다), **COMP-10 · A11Y-11 은 이 화면에서 해소 가능**하다. §5 참조.
...
| ERP-13 | P1 | **미충족.** 이 화면이 출하하는 조사 폴백형: 삭제 토스트·확인 문구 `'<이름>' 을(를)`(`useCrudList.tsx:107` · `:157`), 저장 토스트 `뉴스레터을(를) 등록했습니다`(`useCrudForm.ts:215`), 검증 문구 `제목을(를) 입력하세요` · `본문은(는) 5000자를…`(`shared/crud/validation.ts:14` · `:16`). **추가로 이 화면 고유 비문**: 로드 실패가 `'뉴스레터 찾을 수 없습니다'` · `'뉴스레터 불러오지 못했습니다'` — 목적격 조사가 **아예 빠졌다**(`NewsletterFormPage.tsx:211` · `:213`). `Empty` 만 josa 헬퍼를 갖는다(`Empty.tsx:17-27`, 주격 이/가 전용). **회귀 테스트가 폴백 문구를 고정 중**이라(`newsletters.test.ts:79` 등은 `toContain('입력')` 이라 안전하나, 앱 전역에는 문구 고정 테스트가 있다) josa 헬퍼 도입 시 함께 고쳐야 한다 | 사용자 대상 문자열에 `을(를)` grep — 이 화면 경로에서 4건(`useCrudList` 2 · `useCrudForm` 1 · `validation` 1), `은(는)` 1건. `/:id/edit` 를 없는 id 로 열면 '뉴스레터 찾을 수 없습니다' 비문 | gap |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | IA-13 | **P0** | 목록 상태(필터·검색어)가 URL 에 없다 — 뒤로가기·새로고침·링크 공유로 조건이 복원되지 않는다. `useListState` 미소비(앱 전역 소비자 2파일) | 앱 전역(대부분 `*ListPage`) | A11 · A63(BE-033 §7.5 와 한 배치) |
| 2 | IA-04 | **P0** | Pagination 이 없다 — 필터 결과 전량 렌더. BE-033 EP-01 도 페이징 파라미터가 없다 | 앱 전역(`CrudListShell` 소비 화면 전부) | A11 · A63 |
| 3 | IA-02 | **P0** | 하위 라우트가 branch 라벨('마케팅 관리')을 상위 제목으로 보이고 h1 이 2개다 — title 소스 모순 | 앱 전역(`AppHeader`·`findNavLabel`) | A11 · A40 |
| 4 | EXC-03 | **P0** | 쓰기 권한 게이팅 미배선 — `useRouteWritePermissions` 소비자 0건. 등록·수정·삭제 버튼이 권한 무관하게 렌더 | 앱 전역 | A11 · A40 |
| 5 | COMP-10 | **P0** | 검색 입력에 IME 조합 처리·디바운스가 없다(`useDebouncedSearch` 미소비). 서버 검색 도입 시 즉시 race 표면이 된다 | **이 화면**(+ 대부분의 `*ListPage`) | A11 |
| 6 | A11Y-11 | **P0** | 발신자 select 가 `aria-invalid` 만 있고 `aria-describedby` 가 없다 — 스크린리더가 사유를 못 읽는다 | **이 화면**(1건) | A11 |
| 7 | ERP-13 | P1 | `을(를)`·`은(는)` 폴백형 출하 + **이 화면 고유 비문**('뉴스레터 찾을 수 없습니다') | 앱 전역 + 이 화면 2건 | A11 |
...
- [x] **브리핑의 사실 주장을 그대로 쓰지 않고 재확인했다** — `useListState`/`useDebouncedSearch` 소비자는 브리핑이 말한 10/8파일이 아니라 **`MembersPage`/`MembersToolbar` 2파일뿐**이다(나머지는 훅 자신·테스트·`index.ts` 재export). 이 화면은 어느 쪽도 쓰지 않는다
