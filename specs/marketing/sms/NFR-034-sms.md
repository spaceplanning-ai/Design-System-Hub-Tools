...
|---|---|---|---|---|---|
| STATE-01 | STATE | 직접 | 목록이 `useCrudList` 를 소비하고(`SmsListPage.tsx:106-111`) 스켈레톤 조건이 `firstLoading = isFetching && data === undefined` 로 파생된다(`useCrudList.tsx:71`). `CrudListShell` 이 그 값만 `CrudTable.loading` 으로 넘긴다(`CrudListShell.tsx:137`). 재조회는 `refreshing`(`useCrudList.tsx:72`)으로 갈려 표를 비우지 않고 요약에 '새로고침 중…'만 붙는다(`CrudListShell.tsx:118-122`). 0행 empty·조회 실패 배너도 서로 배타적이다(`CrudListShell.tsx:113,153-165`) | `/marketing/sms` 진입 → 최초 400ms 동안 5행 스켈레톤만(빈 상태 문구 없음). 상태 필터를 바꿔 0행 → Empty 만. `?fail=marketing-sms:list` → danger Alert 만. 목록이 있는 상태에서 `refetch` → 표가 스켈레톤/공백으로 바뀌지 않는다 | pass |
...
| STATE-04 | STATE | 직접 | (b) 선택 해제: 검색어·상태 필터가 바뀌면 `useEffect` 가 `clear()` 한다(`SmsListPage.tsx:114-116`). (a) page clamp: **이 화면에 페이지네이션이 없어**(FS-034-EL-007 · IA-04 gap) 범위를 벗어날 page 자체가 존재하지 않는다 — 요구의 그 절은 표면 부재로 성립하지 않는다 | 행 3건 선택 → 상태 필터를 '초안'으로 변경 → SelectionBar 가 사라진다(선택 0). 검색어 입력에도 동일 | pass |
...
| TOKEN-05 | TOKEN | 상속 | 폼 제목이 DS 의 `pageTitleStyle` 을 소비한다(`SmsFormPage.tsx:263`) — 값을 손으로 재현하지 않는다. AppHeader 의 `<h1>` 도 같은 토큰(`AppHeader.tsx:52-55`) | `styles.ts`/tokens 소유 문서의 판정을 따른다. 이 화면에서는 'titleStyle 재선언 0건'만 확인한다 | 종속 |
| COMP-10 | COMP | **직접** | **충족하지 않는다.** 검색 입력이 `useState` 에 직결돼 있다 — `<SearchField value={keyword} onChange={setKeyword} />`(`SmsListPage.tsx:126-131`, 상태는 `:104`). 디바운스도, `isComposing`/`compositionstart`/`compositionend` 처리도 없다. 공용 `useDebouncedSearch`(250ms + IME 조합 판정 + 조합 중 Enter 차단)가 앱에 존재하나 **소비처가 `MembersPage` 한 곳뿐**이라 이 화면은 쓰지 않는다 | 검색창에 '홍길동'을 IME 로 입력 → 자모 단위 change 마다 `searchSmsCampaigns` 가 재실행되고 `visible` 이 매번 다시 계산된다('ㅎ'→'호'→'홍'…). 조합 중 Enter 가 그대로 상위로 전파된다. **완화 요인**: 검색이 클라이언트 필터라 네트워크 요청·응답 경합은 발생하지 않는다 — 요구의 3개 절 중 'stale 응답이 최신을 덮지 않는다'는 표면 부재로 성립하고, **'조합 중 커밋 금지'·'디바운스+최소 길이'가 미충족**이다 | **gap** |
| FEEDBACK-02 | FEEDBACK | 상속 | 파괴적 액션(단건·일괄 삭제)이 `useCrudList` 의 `ConfirmDialog intent="delete"` 로 게이트된다(`useCrudList.tsx:151-178`). busy 중 확인 버튼 `disabled + aria-busy`(`ConfirmDialog.tsx:151`), 실패 시 다이얼로그 유지 + danger 배너(`useCrudList.tsx:109-113`), 취소·Esc·dim 은 in-flight 를 abort 하고 pending 을 리셋한다(`useCrudList.tsx:86-92,117-123`) | `?fail=marketing-sms:delete` → 행 삭제 확인 → 다이얼로그가 열린 채 배너, 재클릭이 재시도. 요청 중 취소 → 토스트 없이 버튼 상태 복원. 판정 정본은 `ConfirmDialog`/`useCrudList` 소유 문서 | 종속 |
...
| A11Y-01 | A11Y | 상속 | 이 화면이 토스트를 띄운다 — 삭제 성공(`useCrudList.tsx:107,145`) · 저장 성공(`useCrudForm.ts:215`). `ToastProvider` 가 **항상 마운트된** polite/assertive 리전을 소유하고 텍스트만 주입한다(`ToastProvider.tsx:165,168`) — Toast 자신은 `role`/`aria-live` 를 갖지 않는다 | `ToastProvider` 소유 문서의 판정을 따른다. 이 화면에서는 '토스트 표면이 실재한다'는 사실만 못 박는다 | 종속 |
...
| A11Y-11 | A11Y | **직접** | **부분 충족.** 짝 없는 `aria-invalid` 는 0건이다 — 발송명(`SmsFormPage.tsx:286-287`)·예약 일시(`:382-385`)가 `aria-invalid` + `aria-describedby={errorIdOf(...)}` 를 함께 싣고, 본문은 `TextareaField` 가 자동 배선하며(`TextareaField.tsx`), 발신번호는 `SelectField isInvalid`(`:301`)가 담당한다. **그러나 `SegmentPicker` 가 계약을 지키지 않는다**: required 표식이 `aria-hidden` 된 `<span>` 이고(`SegmentPicker.tsx:109-113`) 그룹·체크박스에 `aria-required` 가 없으며, 오류 `<p role="alert">`(`:137-140`)가 어느 컨트롤과도 `aria-describedby` 로 연결되지 않는다. 같은 결로 `ToggleSwitch` 는 `aria-describedby` prop 자체를 노출하지 않는다(`ToggleSwitch.tsx:19-38`) | ① `grep -rn "aria-invalid" apps/admin/src/pages/marketing/sms` → 3건 전부 `aria-describedby` 동반(pass). ② `/marketing/sms/new` 에서 세그먼트 미선택으로 제출 → 오류가 뜨지만 RTL 로 체크박스의 `aria-describedby` 를 조회하면 `null`, `aria-required` 도 없다 → **요구의 'required 는 native required/aria-required 로 노출' 절 미충족** | **gap** |
| A11Y-12 | A11Y | **N/A** | **좌측 필터 list item 이 이 화면에 없다.** 상태 필터가 `SelectField` 드롭다운이고(`SmsListPage.tsx:132-145`) 필터 사이드바·토글 버튼 목록이 없다. `aria-pressed`/`aria-current` 를 쓸 toggle 필터 표면이 존재하지 않는다 | `grep -rn "aria-current\|aria-pressed" apps/admin/src/pages/marketing/sms` → 0건(표면 부재) | N/A |
...
| IA-01 | IA | 상속 | 이 화면의 3개 라우트가 전부 `APP_ROUTES` 에 등재돼(`App.tsx:260-262`) `RequireAuth` → `AppShell` 레이아웃 라우트 아래에서 렌더된다(`App.tsx:326-328`). 자체 sidebar/top bar/outer frame 를 도입하지 않는다 — 화면은 `<div>` 스택으로만 시작한다(`SmsListPage.tsx:154` · `SmsFormPage.tsx:251`) | `App.tsx` 소유 문서의 판정을 따른다. 이 화면에서는 '자체 frame 0건'만 확인한다 | 종속 |
| IA-02 | IA | **직접** | **충족하지 않는다.** title 소스가 라우트마다 모순된다. 목록(`/marketing/sms`)은 AppHeader 의 `<h1>` 만 갖고(잎 정확 일치 → 'SMS 발송') 본문에 제목이 없다. **폼(`/new`·`/:id/edit`)은 `<h1>` 이 둘이다**: `findNavLabel` 이 잎에 정확히 일치하는 것을 못 찾아 브랜치 라벨로 폴백해(`nav-config.ts:257-262`) AppHeader 가 **'마케팅 관리'** 를 `<h1>` 으로 그리고(`AppHeader.tsx:92,101`), 폼 본문이 'SMS 발송 등록'을 또 `<h1>` 으로 그린다(`SmsFormPage.tsx:263`). 요구가 명시적으로 금지한 'sub-route 의 branch label 노출'이 그대로 성립한다 | `/marketing/sms/new` 진입 → 가시 primary title 이 **'마케팅 관리'**(AppHeader). `document.querySelectorAll('h1')` → **2개**('마케팅 관리', 'SMS 발송 등록'). `findNavLabel('/marketing/sms/new')` 를 직접 호출 → `'마케팅 관리'` | **gap** |
| IA-04 | IA | **직접** | **부분 충족.** 템플릿의 앞 4단은 지킨다 — toolbar(좌측 검색+필터 / **우상단 'SMS 발송 등록'** `SmsListPage.tsx:123-152`) → 결과 count 요약(`CrudListShell.tsx:115-123`) → SelectionBar(`:125-133`) → table(`:135-154`). **마지막 단인 Pagination 이 없다**: `CrudListShell` 이 페이지네이션을 렌더하지 않고 `marketing/**` 전역에 `Pagination` 참조가 0건이다. 캠페인 수에 상한이 없어 'page size 초과 가능'은 참이다 — 요구는 그 경우 Pagination 을 요구한다 | `grep -rn "Pagination" apps/admin/src/pages/marketing` → **0건**. 픽스처를 100건으로 늘리면 100행이 한 표에 그려진다(page 컨트롤 없음). 우상단 primary action 과 count 요약은 존재 | **gap** |
| IA-05 | IA | 직접 | `/marketing/sms/new` 와 `/marketing/sms/:id/edit` 가 **같은 `SmsFormPage` 컴포넌트**로 해석된다(`App.tsx:261-262`). `useCrudForm` 이 `useParams().id` 유무로 `isEdit` 를 파생하고(`useCrudForm.ts:73-74`) 레이아웃은 동일한 채 title(`SmsFormPage.tsx:263`)·prefill(`:125-128` reset)·제출 라벨(`:426`)만 갈린다. create 전용/edit 전용 페이지가 없다 | `App.tsx:261-262` 의 두 라우트 `element` 가 동일 컴포넌트 참조. `/new` 와 `/sms-1/edit` 를 나란히 열어 레이아웃 동일 · 제목만 '등록'/'수정' 확인 | pass |
| IA-13 | IA | **직접** | **충족하지 않는다.** 목록 조회 상태가 컴포넌트 `useState` 에만 있다 — `const [filter, setFilter] = useState(SMS_FILTER_ALL)` · `const [keyword, setKeyword] = useState('')`(`SmsListPage.tsx:103-104`). URL 쿼리스트링에 아무것도 직렬화되지 않는다(`useSearchParams` 미사용). 공용 `useListState`(URL 단일 원천 + page clamp + 선택 해제)가 존재하나 **소비처가 `MembersPage` 한 곳뿐**이다 | `?status=예약` 으로 필터 → URL 이 `/marketing/sms` 그대로. F5 → 필터가 '전체 상태'로 초기화. 행을 열어 수정 화면 진입 후 Back → 검색어·필터가 사라진 초기 목록. 필터 걸린 view 의 링크를 만들 방법이 없다 | **gap** |
...
| EXC-03 | EXC | **직접** | **부분 충족.** (a) read 게이팅은 상속으로 충족된다 — `AppShell` 이 `<Outlet>` 을 `RequirePermission` 으로 감싸(`AppShell.tsx:490-492`) 권한 없는 deep-link 에 `ForbiddenScreen` 을 그리고, 리소스는 라우트에서 파생된다(`route-resource.ts:36-46` — `/marketing/sms/1/edit` → 잎 `/marketing/sms`). (b) **write-action 게이팅이 이 화면에 없다**: 'SMS 발송 등록' 버튼(`SmsListPage.tsx:147-150`)·행 액션 수정/삭제(`CrudTable.tsx:192-197`)·일괄 삭제(`CrudListShell.tsx:126-132`)가 권한과 무관하게 렌더된다. 공용 `useRouteWritePermissions` 가 존재하나(`RequirePermission.tsx:45-52`) **앱 전체에서 소비처가 0건**이다 | ① 권한 없는 역할로 `/marketing/sms` deep-link → 403 화면(pass). ② `grep -rln "useRouteCan\|useRouteWritePermissions" apps/admin/src` → `RequirePermission.tsx` **한 파일뿐**(자기 정의). remove 권한 없는 역할로 목록 진입 → 행 삭제 버튼과 '선택 N건 삭제'가 **그대로 보이고 눌린다** → **요구의 'create/update/delete 컨트롤은 can()이 false면 렌더 안 함(또는 disable)' 절 미충족** | **gap** |
| EXC-04 | EXC | 직접 | 폼이 `useCrudForm` 을 소비하고(`SmsFormPage.tsx:175-184`) 그것이 409/412 를 `isConflict` 로 잡아 **입력을 보존한 채** 충돌 다이얼로그를 연다 — 성공 토스트도 목록 이동도 없다(`useCrudForm.ts:158-172`). 화면이 `FormConflictDialog` 를 렌더한다(`SmsFormPage.tsx:431`). 어댑터도 없는 id 의 update/remove 를 조용히 통과시키지 않고 409 를 던져 '유령 저장'을 막는다(`crud.ts:71-74,82-84`) | `?status=marketing-sms:save:409` → `/marketing/sms/sms-1/edit` 에서 저장 → 충돌 다이얼로그가 뜨고 입력이 그대로 살아 있으며 성공 토스트·이동이 없다. '이어서 편집' → 다이얼로그만 닫히고 입력 유지. **주의**: `SmsCampaign` 에 `updatedAt`/`version` 이 없어 `If-Match` 를 보내지 못한다 — **삭제 충돌은 잡히나 동시 수정 충돌은 잡히지 않는다**(BE-034 §7.2 · §3 EXC-04 보강) | pass |
| EXC-08 | EXC | 직접 | 폼 제출이 `useCrudForm` 의 **동기 제출 락**을 탄다 — `submitLockRef` 가 렌더를 기다리지 않고 두 번째 제출을 즉시 반환시킨다(`useCrudForm.ts:102,195-197`). `disabled={saving \|\| loadingDetail}`(`SmsFormPage.tsx:425`)와 이중이다. **멱등키가 `mutationFn` 밖(ref)에서 생성**돼(`useCrudForm.ts:112-117`) 재시도가 같은 키를 재사용하고 성공 시 폐기된다(`:214`). 뮤테이션 자동 재시도는 꺼져 있다(`queryClient.ts:'mutations.retry: false'`) | 제출 버튼 더블클릭 / 응답 전 Enter 연타 → 정확히 1개 요청. `useCrudForm.ts:112` 의 키가 `create.mutate` 호출 **밖**에서 생성됨을 코드로 확인. **주의**: 이 화면에서 EXC-08 이 진짜로 중요해지는 지점(발송)은 **UI 가 없어 아직 성립하지 않는다** — FS-034-EL-036 · §3 | pass |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| `pass` | **9** | STATE-01 · STATE-02 · STATE-04 · TOKEN-01 · FEEDBACK-04 · IA-05 · EXC-04 · EXC-08 · EXC-09 |
| `종속` | **10** | TOKEN-02 · TOKEN-03 · TOKEN-04 · TOKEN-05 · FEEDBACK-02 · A11Y-01 · A11Y-02 · IA-01 · EXC-01 · EXC-02 |
| `n-a` | **2** | FEEDBACK-06 · A11Y-12 |
| `gap` | **9** | COMP-10 · A11Y-11 · MOTION-01 · MOTION-02 · MOTION-03 · IA-02 · IA-04 · IA-13 · EXC-03 |

**검산**: pass 9 + 종속 10 + n-a 2 + gap 9 = **30** ✓

> **⚠ P0 gap 9건 — quality-bar §How to use 기준 '배치 실패' 사유다.** 다만 성격이 갈린다: **이 화면 소유 6건**(COMP-10 · A11Y-11 · IA-02 · IA-04 · IA-13 · EXC-03)과 **앱 전역/DS 소유 3건**(MOTION-01/02/03 — Motion 라이브러리 미도입). 후자는 이 화면만 고쳐서 닫을 수 없다. §5 가 범위를 갈라 이관한다.
...
|---|---|---|---|---|
| STATE-03 | P1 | `useCrudListQuery` 가 `placeholderData: (previous) => previous`(`crud.ts:151`)로 이전 행을 유지하고 `refreshing` 이 가벼운 인디케이터('· 새로고침 중…')만 띄운다(`CrudListShell.tsx:118-122`). `staleTime: 30_000`(`queryClient.ts`) | 목록이 있는 상태에서 재조회 → 이전 행 유지 + 요약에 '새로고침 중…' | pass |
...
| ERP-09 | P2 | **이 화면에서 실질 위험이다** — 야간 광고 판정이 `getHours()` = **브라우저 로컬 TZ**(`messaging.ts:263-267`), 과거 판정이 `Date.now()`(`validation.ts:17-21`). 법정 기준(Asia/Seoul)이 계약에 없다. `scheduledAt` 은 오프셋 없는 `datetime-local` 문자열로 저장된다 | 브라우저 TZ 를 UTC 로 바꾸고 한국 23시에 해당하는 값으로 광고 예약 → 야간 검사를 통과한다 | **gap**(→ BE-034 §7.5) |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | **IA-13** | **P0** | 검색어·상태 필터가 `useState` 에만 있다(`SmsListPage.tsx:103-104`) — URL 미직렬화. F5·Back·링크 공유로 복원 불가. 공용 `useListState` 소비처가 `MembersPage` 뿐 | 이 화면(+앱의 다른 목록 다수) | A11 (FS-034 §7 #2) |
| 2 | **COMP-10** | **P0** | 검색 입력에 IME 조합 판정·디바운스가 없다(`SmsListPage.tsx:126-131`). 공용 `useDebouncedSearch` 미소비. 클라이언트 필터라 응답 경합은 없으나 조합 중 커밋·자모 단위 재계산이 발생 | 이 화면 | A11 (FS-034 §7 #2) |
| 3 | **A11Y-11** | **P0** | `SegmentPicker` 의 required 표식이 `aria-hidden` `<span>` 이고 `aria-required` 가 없으며 오류 `<p role="alert">` 가 체크박스와 `aria-describedby` 로 연결되지 않는다(`SegmentPicker.tsx:109-113,137-140`). `ToggleSwitch` 도 `aria-describedby` prop 미노출 | 이 화면 + `_shared`(이메일·뉴스레터 공유) + DS | A11 · A40 (FS-034 §7 #3) |
| 4 | **IA-02** | **P0** | 폼 라우트에 `<h1>` 이 둘 — AppHeader 가 브랜치 라벨 '마케팅 관리'를 그리고(`AppHeader.tsx:92,101` → `nav-config.ts:257-262`) 폼이 'SMS 발송 등록'을 또 그린다 | 앱 전역(모든 하위 라우트) | A40 · A11 (FS-034 §7 #1) |
| 5 | **IA-04** | **P0** | Pagination 이 없다 — `marketing/**` 전역 `Pagination` 참조 0건. 캠페인 전량이 한 표에 그려진다 | 이 화면 + `CrudListShell` | A11 (FS-034 §7 #9) |
| 6 | **EXC-03** | **P0** | 쓰기 권한 게이팅이 없다 — 등록·수정·삭제 버튼이 권한과 무관하게 렌더. `useRouteWritePermissions` 소비처 0건(`RequirePermission.tsx:45-52`) | 앱 전역(모든 목록·폼) | A11 · A63 (FS-034 §7 #10 · BE-034 §7.12 #7) |
...
| 12 | ERP-09 | P2 | 야간·과거 판정이 브라우저 로컬 TZ 기준(`messaging.ts:263-267` · `validation.ts:17-21`). 법정 기준 Asia/Seoul 이 계약에 없다 | 이 화면 + `_shared`(이메일·뉴스레터 공유) | A63 · A11 (FS-034 §7 #7 · BE-034 §7.5) |
...
- [x] §2.1 산수를 검산했다 — pass 9 + 종속 10 + n-a 2 + gap 9 = **30** ✓
