...
> **§7.5 판정 필수** — 프론트 어댑터가 `filter` 라 없는 id 에 조용히 성공한다(유령 삭제). 서버는 존재한 적 없는 id 에 **404**, 이미 삭제된 id 에 **204**(멱등)로 답한다.
...
| EP-07 삭제 | id 형식 → 400 | 401 → 다이얼로그 안 배너 | `operator` → **403** / 읽기 없음 → **404** | 404 = 존재한 적 없는 id. **이미 삭제 → 204(멱등)**. **현재 프론트는 없는 id 에 조용히 성공**(유령 삭제 · §7.5) | N/A — 멱등 삭제 | **422 `TEMPLATE_LOCKED`**(심사 중 템플릿 삭제 — §7.6) | 429 분당 60(일괄이 항목당 요청) | 500 → 단건은 다이얼로그 배너 / 일괄은 부분 실패 건수 | 5초 → 504 → 같은 자리 |
...
### 7.5 유령 저장·유령 삭제·죽은 404 갈래 — 어댑터 계약이 갈려 있다

이 화면의 어댑터는 `createStoreAdapter`(`crud.ts:107-137`)로 **존재 검사를 하지 않는다**:

| 함수 | 저장소 구현 | 결과 |
|---|---|---|
| `getOne` | `store.ts:168-172` — 없으면 **generic `Error`**(status 없음) | 폼의 404 갈래(`isNotFound`)가 **죽어 있다**. 삭제된 템플릿에 '다시 시도'(영원히 실패)를 권한다 — FS-036-EL-029 |
| `update` | `store.ts:191-206` — `map`(없으면 무변화) | **유령 저장**: '저장했습니다' 토스트 + 목록 이동, 저장된 것 없음 |
| `remove` | `store.ts:208-210` — `filter`(없으면 무변화) | **유령 삭제**: 성공 토스트. 일괄 삭제에서 없는 id 가 **성공으로 집계** |
| — | 409 를 던지는 경로 없음 | **충돌 다이얼로그(FS-036-EL-030)가 도달 불가** — `useCrudForm` 의 `isConflict` 분기가 죽어 있다 |

**뉴스레터의 `createCrudAdapter` 는 같은 자리에서 409 를 던진다**(`crud.ts:71-73` · `:82-84`, 주석이 '유령 저장' 을 명시적으로 지목한다) — **두 어댑터의 계약이 갈려 있고, 이 화면은 갈린 쪽에 있다.**

**판정**:
1. **서버 계약**: EP-02 는 **404 status** 를, EP-04/EP-07 은 없는 id 에 **404** 를, 동시 수정에 **409/412** 를 반환한다. 어댑터는 이를 **`HttpError` 로 승격**해 그대로 던진다 — 그 순간 폼의 not-found 갈래·충돌 다이얼로그가 **코드 변경 없이 살아난다**(이미 배선돼 있다).
2. **백엔드 연결 전 임시 조치**: `createStoreAdapter` 에 `createCrudAdapter` 와 같은 존재 검사를 넣어 두 어댑터의 계약을 맞춘다 — 그러지 않으면 `createStoreAdapter` 를 쓰는 **모든 화면**(상품·포트폴리오·카테고리·답변템플릿·고객센터 유형)이 같은 유령 저장을 공유한다. §7.12 이관 — **이 화면만의 문제가 아니다.**
...
프론트가 제출 **시도** 단위로 `Idempotency-Key` 를 만들고 성공하면 버린다(`useCrudForm.ts:112-117`). 동기 제출 락도 있다.

**판정**: `POST`(EP-03)는 `Idempotency-Key` 를 **필수**로 받는다 — 같은 키의 재요청은 **최초 응답을 재생**한다(중복 템플릿을 만들지 않는다). 보존 기간 24시간. `PUT`(EP-04)은 본래 멱등이지만 키를 받아 **재시도 시 승인 무효화 부수효과(§7.1-4)가 두 번 적용되지 않게** 한다 — 이것이 이 화면에서 PUT 에도 키가 필요한 이유다.
...
| 2 | **`createStoreAdapter` 가 존재 검사를 하지 않는다** — 유령 저장·유령 삭제·죽은 404 갈래·도달 불가능한 충돌 다이얼로그. **`createCrudAdapter` 와 계약이 갈려 있고, 이 어댑터를 쓰는 모든 화면이 영향받는다**(§7.5) | **A63 · A11(앱 전역)** |
...
| 11 | 쓰기 권한 게이팅 미배선 — **승인상태 select 도 권한 축으로 막히지 않는다**(§2 · §7.1) | A11 · A40 |
