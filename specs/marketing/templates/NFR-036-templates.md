...
| 판정 시점 | 2026-07-17 · HEAD `3cd3078`(F2 포함) 코드 대조 |
...
| COMP-10 | COMP | 직접 | **미충족.** 검색 입력이 `SearchField` 에 `onChange={setKeyword}` 를 직결한다(`TemplateListPage.tsx:132-137`) — 키 입력마다 즉시 `keyword` state 가 바뀐다. IME 조합 처리·디바운스·최소 길이 정책이 **없다**. `shared/crud/useDebouncedSearch.ts` 가 이 셋을 전부 소유하지만 **이 화면은 소비하지 않는다**(앱 전체 소비자는 `MembersPage`/`MembersToolbar` 2파일 — grep 확인). 오늘은 클라이언트 필터라 네트워크 race 는 없으나 **`searchTemplates` 가 이름 + 본문(최대 2000자)을 훑으므로** 매 자모마다 전 배열 × 본문 스캔이 돈다. BE-036 §7.8 의 서버 검색이 붙는 순간 이 입력이 그 seam 이 된다 | 검색창에 '주문' 을 IME 로 입력 → `searchTemplates` 가 'ㅈ','주','주ㅁ','주문' 매 조합 단계마다 실행(Profiler 또는 임시 카운터). 세 파일에 `isComposing`/`onCompositionStart` grep = 0. `useDebouncedSearch` import = 0 | **gap** |
...
| IA-02 | IA | 직접 | **미충족.** 두 곳이 각각 `<h1>` 을 그린다: ① `AppHeader` 가 `findNavLabel(pathname)` 로(`AppHeader.tsx:92` · `:101`) ② `FormPageShell` 이 '발송 템플릿 등록'/'발송 템플릿 수정' 으로(`FormPageShell.tsx:159`). `findNavLabel` 은 정확히 일치하는 nav 잎이 없으면 **basePath 접두 일치로 branch 라벨을 반환**한다(`nav-config.ts:253-263`) — `/marketing/templates/new` 는 잎이 아니므로 **'마케팅 관리'**. 목록 라우트는 잎과 정확히 일치해 **'발송 템플릿 관리'**(nav 라벨)가 나오는데, 이는 폼의 entityLabel('발송 템플릿')과도 다르다. F2 는 `AppHeader.tsx` 의 **스타일만** 고쳤다(F1→F2 diff 확인) | `/marketing/templates/new` 진입 → `document.querySelectorAll('h1')` 이 **2개**를 반환하고 첫 번째 텍스트가 **'마케팅 관리'**(‘발송 템플릿 등록’ 아님). `/marketing/templates` → h1 1개('발송 템플릿 관리'), 본문 제목 없음 | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** 목록 조회 상태(채널 필터·검색어)가 컴포넌트 `useState` 에만 있다(`TemplateListPage.tsx:109-110`). URL 직렬화가 없다 — `useSearchParams` 미사용. `shared/crud/useListState.ts` 가 URL 직렬화 + page clamp + 선택 해제를 전부 소유하지만 **이 화면은 소비하지 않는다**(앱 전체 소비자 2파일 — grep 확인). page/sort 는 애초에 존재하지 않는다(IA-04 gap · FS-036 §7 #4) | '알림톡' 필터 + '주문' 검색 적용 → URL 이 `/marketing/templates` 그대로(쿼리스트링 없음). 행을 열어 수정 폼 진입 후 브라우저 Back → **필터·검색어가 초기화된 전체 목록**에 착지. URL 을 새 탭에 복사 → 필터 없는 기본 목록. `TemplateListPage.tsx` 에 `useSearchParams`/`useListState` import = 0 | **gap** |
...
| EXC-03 | EXC | 직접 | **부분 미충족(쓰기 게이팅 축).** **읽기**는 상속으로 충족 — `RequirePermission` 이 `<Outlet>` 을 감싸(`AppShell.tsx:490`) read 권한 없는 딥링크에 403 화면을 렌더하고 리소스는 라우트에서 파생된다(`route-resource.ts`). **쓰기 게이팅이 없다**: '템플릿 등록' 버튼(`TemplateListPage.tsx:155-158`) · 행 `RowActions` 수정/삭제(`CrudTable.tsx:192-197`) · `SelectionBar` 일괄 삭제(`CrudListShell.tsx:126-132`) **그리고 승인상태 select**(`TemplateFormPage.tsx:208`)가 권한을 조회하지 않는다. `useRouteCan`/`useRouteWritePermissions` 가 정의돼 있으나 **앱 전체 소비자 0건**(grep: 정의 파일 자신 외 없음). **이 화면에서는 이것이 단순 UX 문제가 아니다** — 승인상태 select 가 권한 축으로도 막히지 않아 어떤 역할이든 알림톡을 '승인' 으로 세팅할 수 있다(BE-036 §7.1) | remove=false·update=false 인 역할로 로그인 → 행 삭제·일괄 삭제·'템플릿 등록'·**승인상태 select** 가 **여전히 보이고 조작된다**. read=false 로 바꾸면 403 화면은 정상 렌더(읽기 축은 pass). 앱 전체 `useRouteWritePermissions` grep = 1(정의부만) | **gap** |
| EXC-04 | EXC | 직접 | **미충족.** 이 화면의 어댑터는 **`createStoreAdapter`** 로 존재 검사를 하지 않는다(`crud.ts:126-135`) — `update` 는 저장소 `updateTemplate` 에 위임하는데 그것이 `map` 이라 **없는 id 를 조용히 지나치고 성공을 반환**한다(`store.ts:191-206`). `remove` 도 `filter` 라 동일(`:208-210`). 그 결과 다른 관리자가 방금 지운 템플릿을 편집하면 **'저장했습니다' 토스트 + 목록 이동**이 일어나지만 저장된 것이 없다 — quality-bar EXC-04 의 _근거_ 문단이 지목하는 **ghost 'saved' 그 자체**다. 409 를 던지는 경로가 없어 `useCrudForm` 의 `isConflict` 분기와 충돌 다이얼로그(FS-036-EL-030)가 **도달 불가능**하다. 낙관적 동시성 토큰(`If-Match`)도 없다. **뉴스레터의 `createCrudAdapter` 는 같은 자리에서 409 를 던진다**(`crud.ts:71-73` · `:82-84`) — 두 어댑터의 계약이 갈렸고 이 화면은 갈린 쪽이다 | `/marketing/templates/tpl-sms-1/edit` 진입 → 다른 탭(또는 DevTools 로 `removeTemplate('tpl-sms-1')`)에서 그 템플릿 삭제 → 폼에서 저장 → **'발송 템플릿을(를) 저장했습니다' 토스트 + 목록 이동**, 그러나 목록에 그 템플릿이 없다(저장되지 않았다). 충돌 다이얼로그는 뜨지 않는다. `?status=save:409` 로 강제하면 다이얼로그가 뜨므로 **UI 는 준비돼 있고 어댑터가 409 를 만들지 않는 것**이 원인이다 | **gap** |
| EXC-08 | EXC | 직접 | `useCrudForm` 이 **동기 제출 락**(`submitLockRef`)으로 disabled 렌더 이전의 두 번째 제출을 막고(`useCrudForm.ts:102` · `:195-197`), `FormPageShell` 이 `disabled={saving \|\| loadingDetail}` 로 이중 차단한다(`FormPageShell.tsx:188`). **멱등키는 mutationFn 밖 ref** 에서 제출 **시도** 단위로 만들어(`useCrudForm.ts:112-117` · `:205`) 재시도가 같은 키를 재사용하고 성공 시 버린다(`:214`). 삭제·일괄 삭제는 `ConfirmDialog` 의 `busy` 잠금. quality-bar 가 지목한 3대 작업 중 이 화면은 **'생성'** 에 해당한다(발송·심사 제출 트리거는 이 화면에 없다 — FS-036 §7 #3) | 제출 버튼 더블클릭 / 응답 전 Enter 연타 → 정확히 1개 요청(어댑터 `create` 호출 카운터). `?status=save:500` 으로 실패시킨 뒤 재제출 → `idempotencyKeyRef` 가 같은 키를 유지 | pass |
...
> **P0 gap 6건 = quality-bar '배치 실패' 사유.** **EXC-04 는 이 화면 고유**다 — 자매 화면인 뉴스레터(NFR-033)는 `createCrudAdapter` 를 써서 같은 항목이 pass 다. 그 차이가 `createStoreAdapter` 의 존재 검사 부재 하나에서 나온다(BE-036 §7.5). **IA-02 · IA-04 · IA-13 · EXC-03 은 앱 전역 문제**이고 **COMP-10** 은 이 화면에서 해소 가능하다. §5 참조.
>
...
| ERP-13 | P1 | **미충족.** 이 화면이 출하하는 조사 폴백형: 삭제 토스트·확인 문구 `'<이름>' 을(를)`(`useCrudList.tsx:107` · `:157`), 저장 토스트 `발송 템플릿을(를) 등록했습니다`(`useCrudForm.ts:215`), 로드 실패 `발송 템플릿을(를) 찾을 수 없습니다`(`FormPageShell.tsx:128-129`), 검증 문구 `템플릿명을(를) 입력하세요` · `본문은(는) 2000자를…`(`shared/crud/validation.ts:14` · `:16`). `Empty` 만 josa 헬퍼를 갖는다(`Empty.tsx:17-27`). **이 화면 고유 스키마 문구는 폴백형이 아니다** — `templateSchema` 가 '제목을 입력하세요.' 를 직접 쓴다(`validation.ts:34`). **회귀 테스트는 느슨해 안전하다** — `templates.test.ts` 가 `toContain('입력')`·`toContain('변수만')` 으로 검사하므로 josa 헬퍼 도입 시 이 파일은 고칠 필요가 없다. 단 앱 전역에 문구를 문자열로 고정한 테스트가 있으면 함께 고쳐야 한다 | 사용자 대상 문자열에 `을(를)` grep — 이 화면 경로에서 5건(`useCrudList` 2 · `useCrudForm` 1 · `FormPageShell` 2), `은(는)` 1건(`validation`). `/marketing/templates/nope/edit` → '발송 템플릿을(를) 찾을 수 없습니다' | gap |
...
| EXC-12 | P1 | **미충족.** `useCrudForm` 이 `isNotFound` 로 `'not-found'`/`'error'` 를 가르고(`useCrudForm.ts:137-143`) `FormPageShell` 이 404 에는 '다시 시도' 를 주지 않는다(`FormPageShell.tsx:120-142`) — **UI 는 완전히 배선돼 있다**. 그러나 저장소가 **status 없는 generic `Error`** 를 던져(`store.ts:168-172`) `isNotFound` 가 항상 false 다 → 삭제된 템플릿에 **영원히 실패하는 '다시 시도' 를 권한다**. **뉴스레터의 `createCrudAdapter` 는 `HttpError(404)` 를 던져 이 갈래가 산다**(`crud.ts:54-56`) — 어댑터 하나 차이 | `/marketing/templates/nope/edit` → '발송 템플릿을(를) **불러오지 못했습니다**' + **'다시 시도' 버튼**(404 갈래가 아니다). 뉴스레터에서 같은 조작 → '찾을 수 없습니다' + '목록으로' 만. `?status=detail:404` 로 강제하면 이 화면도 404 갈래를 보인다 — **UI 가 아니라 어댑터가 원인임을 증명** | gap |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | **EXC-04** | **P0** | **유령 저장·유령 삭제** — `createStoreAdapter` 가 존재 검사를 하지 않아 없는 id 에 성공을 반환한다. 충돌 다이얼로그가 도달 불가. **`createCrudAdapter`(뉴스레터)와 계약이 갈렸다** | **이 화면 + `createStoreAdapter` 를 쓰는 모든 화면**(상품·포트폴리오·카테고리·답변템플릿·고객센터 유형) | **A63(BE-036 §7.5) · A11** |
| 2 | IA-13 | **P0** | 목록 상태(필터·검색어)가 URL 에 없다 — `useListState` 미소비(앱 전역 소비자 2파일) | 앱 전역 | A11 · A63(BE-036 §7.8 과 한 배치) |
| 3 | IA-04 | **P0** | Pagination 이 없다 — 필터 결과 전량 렌더. BE-036 EP-01 도 페이징 파라미터가 없다 | 앱 전역 | A11 · A63 |
| 4 | IA-02 | **P0** | 하위 라우트가 branch 라벨('마케팅 관리')을 상위 제목으로 보이고 h1 이 2개다. 목록 제목('발송 템플릿 관리')과 폼 entityLabel('발송 템플릿')도 갈린다 | 앱 전역(`AppHeader`·`findNavLabel`) | A11 · A40 |
| 5 | EXC-03 | **P0** | 쓰기 권한 게이팅 미배선 — **승인상태 select 도 권한 축으로 막히지 않는다** | 앱 전역 | A11 · A40 |
| 6 | COMP-10 | **P0** | 검색 입력에 IME 조합 처리·디바운스가 없다. **본문까지 훑어 자모당 비용이 크다** | 이 화면(+ 대부분의 `*ListPage`) | A11 |
| 7 | EXC-12 · EXC-06 | P1 | **404 갈래가 죽어 있다** — 저장소가 status 없는 generic Error 를 던져 삭제된 템플릿에 영원히 실패하는 '다시 시도' 를 권한다. #1 과 같은 뿌리 | 이 화면 + `createStoreAdapter` 화면 | **A63(BE-036 §7.5)** |
...
| 10 | ERP-13 | P1 | `을(를)`·`은(는)` 폴백형 출하(공용 6건). **이 화면 테스트는 느슨해 josa 헬퍼 도입 시 수정 불필요** | 앱 전역 | A11 |
...
- [x] §2.1 산수 검산 — 11(pass) + 11(종속) + 2(n-a) + 6(gap) = **30** ✅
...
- [x] **자매 화면(NFR-033 뉴스레터)과의 차이를 정확히 짚었다** — 같은 프레임워크를 쓰면서 EXC-04 만 갈리는 이유가 `createStoreAdapter` vs `createCrudAdapter` 임을 코드로 확인했다
- [x] **quality-bar 에 행이 없는 최대 위험(승인 상태 클라이언트 지정)을 §2 표에 억지로 끼워 넣지 않고 §2.1 주석 · §4.2 · §4.3 · §5 #12 로 명시했다**
- [x] **브리핑의 사실 주장을 그대로 쓰지 않고 재확인했다** — `useListState`/`useDebouncedSearch` 소비자는 브리핑이 말한 10/8파일이 아니라 **`MembersPage`/`MembersToolbar` 2파일뿐**이다(나머지는 훅 자신·테스트·`index.ts` 재export)
