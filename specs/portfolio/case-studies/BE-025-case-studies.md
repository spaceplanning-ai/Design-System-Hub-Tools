...
> **BE-023 과의 관계**: 두 도메인은 같은 프론트 프레임워크 위에 있어 엔드포인트 모양이 거의 같다. **이 문서는 BE-023 과 다른 것을 온전히 기술하고**(§1.1), 같은 판정은 BE-023 을 참조한다. **결정적 차이는 어댑터다** — BE-023 의 화면은 존재 검사를 하지 않는 `createStoreAdapter` 를 쓰지만, 이 화면은 **`createCrudAdapter` 를 써 없는 id 에 409/404 를 던진다**. 그래서 BE-023 §7.3 이 '유령 저장'을 판정해야 했던 자리에서 이 문서는 **계약이 이미 지켜지고 있음**을 확인만 한다(§7.3).
...
| 어댑터 존재 검사 | **없다**(`createStoreAdapter`) → 유령 저장(BE-023 §7.3) | **있다**(`createCrudAdapter`) → 409/404 정상 |
...
| 멱등성 | **비멱등.** 멱등키가 어댑터로 전달되지 않는다(FS-025 §7 #16) — 현재 계약에 `Idempotency-Key` 헤더가 없다. §7.8 |
| 레이트리밋 | 분당 30회 |
...
**BE-023 §7.3 과 갈리는 지점이다. 절반은 이미 지켜지고 있다.**

**지켜지는 것(확인)**: 이 화면은 `createCrudAdapter` 를 쓰므로 `update`(`crud.ts:71-73`)·`remove`(`:82-84`)가 **트랜잭션 전에 대상 존재를 확인하고 없으면 409 를 던진다**. 그래서 —
...
**BE-023 이 §7.3 에서 '고쳐야 한다' 고 판정한 것이 여기서는 '이미 그렇다' 이므로, 백엔드는 이 동작을 계약으로 유지한다** — EP-04 는 대상이 없으면 409 `CONFLICT`(`message`: '다른 사용자가 먼저 삭제한 항목입니다.'), EP-02 는 404.
...
| 6 | 멱등키 전달 경로(`Idempotency-Key`) — `CrudAdapter` 시그니처 확장 필요(§7.8). BE-023 §7.10 #5 와 동일 사안 | A63 · A11 |
...
BE-023 §7.8 과 동일. `useCrudForm.ts:206` 이 `takeIdempotencyKey()` 반환값을 버리고 `CrudAdapter.create/update` 시그니처에 키 파라미터가 없어 **`Idempotency-Key` 헤더가 나갈 경로가 없다**. EP-03 은 비멱등이며 프론트의 동기 제출 락이 **한 탭 안에서만** 중복을 막는다. 성공 사례 등록은 금액·발송이 아니므로 중복의 대가가 '중복 사례 1건' 이라 **비멱등으로 확정**하고 §7.7 #6 으로 이관한다.
...
- [x] 멱등성 판정 — 조회 GET / 수정·삭제 멱등 / 등록 비멱등(§7.8). **픽스처의 409(이미 삭제)와 서버 계약의 204(멱등)가 다른 이유를 EP-05 에 명시했다**
- [x] **보안 판정 2건 이상**: 저장형 XSS(§7.1 — 표면이 BE-023 보다 넓은 이유 포함) · 이미지 스킴 화이트리스트(§7.2) · 403/404 은닉(§7.5 — 참조 무결성이 왜 없는지 포함) · 정합 판정 동시성(§7.3)
- [x] **BE-023 과의 차이를 §1.1 에 전수 표로 못 박았다** — 특히 어댑터 차이가 계약 판정을 실제로 가르는 지점(§7.3)을 명시했다
