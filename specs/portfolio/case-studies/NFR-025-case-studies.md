...
>
> **직전 `4b805ad` 기준 기록(보존)**: 뒤집힌 P0 0건. **A11Y-11 의 범위를 8→1 로 좁혔을 뿐**이었고(남은 것은 `ImageUploadField` 하나) 그때 '좁혀진 gap 도 gap' 으로 유지한 판단은 옳았다 — **이번에 그 마지막 하나가 실제로 닫혀서** pass 가 됐다. **P1 2건은 그때 pass 가 됐다**(ERP-13 · EXC-08 의 멱등키 절 — §3 참조). EXC-03·IA-13 은 사유가 '앱 전역 미비' → **'이 화면의 미소비'** 로 바뀌었다(공용 훅과 선례가 각각 6곳·34곳에 실재한다).
>
...
| 4 | EXC-03 | **P0** | 쓰기 액션 권한 게이팅 부재 — **`useRouteWritePermissions` 는 이제 6곳이 소비하지만**(products 3 · settings 3) `pages/portfolio/**` 는 그 밖이다. 최근접 선례는 같은 `useCrudList`+`CrudListShell` 목록인 `products/items/ProductListPage.tsx:119`. read 게이팅은 성립 | **이 화면**(공용 훅 + 6곳 선례 존재) | UI 기획 쪽 변경 요청 (FS-025 §7 #2) |
