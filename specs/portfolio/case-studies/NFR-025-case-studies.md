...
| 검증 방법 | **E2E 미실행. 모든 판정 근거는 F2(`3cd3078`) 코드 대조다.** §6 의 재현 스위치는 판정을 *확인할 수 있는* 절차이며 이 문서가 그것을 실행했다는 뜻이 아니다 |
...
**EXC-04 는 양쪽 다 gap 이지만 심각도가 다르다** — 포트폴리오는 유령 저장이 실재하고(`createStoreAdapter` 가 존재 검사를 하지 않는다), 이 화면은 **동시 삭제를 409 로 잡지만 동시 수정을 못 잡는다**(토큰 부재). §2 EXC-04 행이 이를 구분한다. **EXC-12 도 같은 뿌리로 갈린다**(P1 — §3).
...
| A11Y-11 | A11Y | 직접(+앱 전역) | **절반은 충족**: `aria-invalid` 를 쓰는 이 화면의 모든 컨트롤이 `aria-describedby` 로 이유를 연결한다 — `CaseStudyFormPage.tsx:130-133`(업종) · `:151-152`(일자) · `:167-168`(제목) · `:182-183`(고객사), `TextareaField.tsx:62-63`(과제·해결·성과 3개, 자체 배선), `ImageUploadField`/`ImageGalleryField`(`aria-invalid` 를 쓰지 않고 describedBy 만 — 짝 없는 위반이 아니다). 회귀는 `shared/a11y-guard.test.ts` 가 고정한다. **나머지 절반은 미충족**: `FormField.tsx:58-62` 의 `required` 는 **시각 `*`(aria-hidden)만** 그리고, 폼의 어떤 컨트롤도 `required`/`aria-required` 를 갖지 않는다 | ① `aria-describedby` 없는 `aria-invalid` grep = 0 → **통과**. ② 빈 폼 제출 후 입력의 `aria-describedby` === `role=alert <p>` id → **통과**. ③ **`apps/admin/src` 전체에서 `aria-required` grep = 0, 이 폼의 필수 컨트롤 8개(업종·일자·제목·고객사·과제·해결·성과·대표이미지) 중 `required` 속성을 가진 것 0개 → 미충족**. 범위: 앱 전역(DS `FormField`) | **gap** |
...
| IA-02 | IA | 직접 | **미충족.** `nav-config.ts:253-264` `findNavLabel('/portfolio/case-studies/new')` → leaf 정확 일치 실패 → `:260` branch `basePath: '/portfolio'` 매치 → **'포트폴리오 관리'**. `AppHeader.tsx:92`·`:101` 이 그것을 `<h1>` 으로 그린다. 동시에 `FormPageShell.tsx:159` 가 본문에 `<h1>성공 사례 등록</h1>` 을 또 그린다 → **한 화면에 `<h1>` 2개, 서로 다른 문구**. 목록은 in-content title 이 없어 AppHeader 만 제목을 갖는다 — 목록/폼의 title 소스가 다르다. F2 가 `AppHeader.tsx` 를 손봤지만 바뀐 것은 `pageTitleStyle` 소비(TOKEN-05)뿐이고 **제목 소스는 여전히 `findNavLabel(pathname)`** 이다 | `/portfolio/case-studies/new` 를 연다 → 헤더 `<h1>` = '포트폴리오 관리'(구체 title '성공 사례 등록' 이 아니다). `document.querySelectorAll('h1').length` = 2, 두 텍스트가 다르다 | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** list query state(업종 필터)가 **컴포넌트 `useState` 에만 있다** — `CaseStudyListPage.tsx:56` `const [filter, setFilter] = useState<CaseFilter>(CASE_FILTER_ALL)`. 파일에 `useSearchParams`·`useListState` import 0건. `parseFilter`(`:106`)로 값을 타입 안전하게 좁히기는 하지만 **그 값이 URL 로 나가지 않는다** — 안전한 값을 URL 없이 들고 있을 뿐이다. 앱에는 이미 URL 단일 원천 훅(`shared/crud/useListState.ts`, 10개 화면이 소비)이 있는데도 소비하지 않는다. 정렬은 서버 고정이라 URL 대상이 아니고, page·keyword 는 표면 자체가 없다 | 업종을 '금융'으로 바꾼 뒤 ① F5 → **'전체 업종'으로 되돌아간다** ② 행을 눌러 수정 폼에 갔다가 Back → **필터가 풀린 목록에 착지한다** ③ 주소창 URL 을 복사해 새 탭에 붙여넣기 → **필터가 재현되지 않는다**(URL 에 `?` 가 아예 붙지 않는다). `grep -n 'useSearchParams\|useListState' apps/admin/src/pages/portfolio/case-studies/CaseStudyListPage.tsx` = 0 | **gap** |
...
| EXC-03 | EXC | 직접 | **부분 충족 → 미충족.** ① **route-level authorization 은 성립한다**: `AppShell.tsx:490-492` 가 `<Outlet>` 을 `RequirePermission` 으로 감싸 read 권한이 없으면 `ForbiddenScreen` 을 렌더한다. `route-resource.ts:36-46` 이 경로에서 리소스를 파생하므로(`/portfolio/case-studies/:id/edit` → leaf `/portfolio/case-studies`) **폼 라우트까지 자동으로 덮인다**. 강등 reconcile 도 권한 스토어 구독으로 성립한다. ② **write-action 게이팅은 전무하다**: `useRouteWritePermissions`(`RequirePermission.tsx:45`)의 **소비처가 앱 전체에서 0건**이다 — '성공 사례 등록'(`CaseStudyListPage.tsx:118`) · 행 액션(`CrudTable.tsx:192-197`) · 일괄 삭제(`CrudListShell.tsx:126-132`) · 노출 토글(`publishColumn.tsx:20`)이 `can()` 을 보지 않고 무조건 렌더된다 | ① read-forbidden 역할로 딥링크 → 403 화면 = **통과**. ② delete 권한이 없는 역할로 목록을 연다 → **행 삭제·일괄 삭제 버튼이 그대로 보이고 눌린다** = 미충족. `grep -rn 'useRouteWritePermissions\|useRouteCan' apps/admin/src/pages/` = 0. 범위: 앱 전역 | **gap** |
...
| EXC-08 | EXC | 직접 | ① **동기 제출 락**: `useCrudForm.ts:102` `submitLockRef` + `:195-197` `if (submitLockRef.current) return; submitLockRef.current = true` — RHF 비동기 검증과 `disabled={saving}` 렌더 사이의 틈을 ref 가 닫는다. `:207-209` `onSettled` 와 `:239-241` `onInvalid` 가 해제한다. ② **disable**: `FormPageShell.tsx:188` `disabled={saving || loadingDetail}`. ③ **멱등키가 mutationFn 밖에 있다**: `:112-117` `idempotencyKeyRef` + `takeIdempotencyKey()` 가 **제출 시도 단위**로 키를 잡아 재클릭이 같은 키를 재사용하고 `:214` 성공 시에만 버린다 — 요구가 명시한 구조를 만족한다. 삭제·일괄 삭제·토글도 요청 중 컨트롤을 잠근다 | 등록 폼을 채우고 **Enter 를 연타**한다(응답 400ms 전) → 요청 1건, 사례 1건만 생성. `?status=save:500` 후 재제출 → `idempotencyKeyRef` 가 같은 키를 유지한다. **주의(§5 #14)**: 키가 어댑터로 **전달되지는 않는다**(`:206` 반환값 폐기) — 현재 네트워크가 없어 관측 차이가 없고, 요구의 두 검사(1요청 발사 · 키가 mutationFn 밖)는 충족한다 | pass |
...
> **NFR-023 대비**: pass 11 → **12**, gap 8 → **7**. 차이는 **STATE-02 한 건**(§1.2) — 업종이 소스 상수라 표면 없는 read query 가 존재하지 않는다.
>
> **P0 gap 7건 = quality-bar '배치 실패' 사유.** 7건 중 **이 화면 고유는 2건**(IA-13 · EXC-04)이고, **5건은 앱 전역/공용 프레임워크 범위**(A11Y-11 · MOTION-03 · IA-02 · IA-04 · EXC-03)로 이 화면만 고쳐서는 해소되지 않는다 — §5 의 '범위' 열이 이를 가른다.
...
| **ERP-13** | P1 | 조사 폴백형 리터럴을 실제로 출력하며 **이 화면이 더 눈에 띈다** — **'성공 사례'는 받침이 없어 '를'·'는' 이 옳은데 '을(를)'·'은(는)' 이 나온다**: `useCrudForm.ts:215`('성공 사례을(를) 등록했습니다.'), `FormPageShell.tsx:128-129`('성공 사례을(를) 불러오지 못했습니다.'), `useCrudList.tsx:107`·`:145`, `validation.ts:15-17` `requiredText`('과제을(를) 입력하세요.'). **대조**: `@tds/ui` `Empty.tsx:17-27` 은 받침 계산으로 '성공 사례**가**' 를 옳게 낸다 — 같은 화면 안에서 옳은 조사와 폴백형이 공존한다 | 사례를 등록 → '성공 사례을(를) 등록했습니다.' 토스트. 같은 화면의 빈 상태는 '등록된 성공 사례가 없습니다'(옳다). 사용자 대상 문자열에 `'을(를)'` grep > 0 | gap |
...
| 4 | EXC-03 | **P0** | 쓰기 액션 권한 게이팅 전무 — `useRouteWritePermissions` 소비처 0건. read 게이팅은 성립 | **앱 전역**(모든 `*ListPage`) | A11 change_request (FS-025 §7 #2) |
| 5 | IA-02 | **P0** | 하위 라우트가 브랜치 라벨로 폴백 + `<h1>` 2개. 목록/폼의 title 소스 불일치 | **앱 전역**(AppHeader · nav-config) | A11 change_request · A40 (FS-025 §7 #11) |
| 6 | A11Y-11 | **P0** | 필수 필드가 `required`/`aria-required` 를 노출하지 않는다(앱 전역 grep = 0). describedby 절반은 충족 | **앱 전역**(@tds/ui `FormField`) | A11 change_request (FS-025 §7 #14) |
...
| 14 | EXC-08(부분) | P1 | 멱등키가 생성되나 어댑터로 전달되지 않는다. 중복 제출 차단은 pass | 공용(shared/crud) | A63 (BE-025 §7.8 · §7.7 #6) · A11 |
| 15 | A11Y-08 | P1 | 행 안에 keyboard-focusable 이름 링크 없음(제목이 평문, 행 클릭은 마우스 전용) | 공용(shared/crud `CrudTable`) | A11 (FS-025 §7 #5) |
| 16 | ERP-13 | P1 | 조사 폴백형 리터럴 출력 — **'성공 사례'는 받침이 없어 '를'·'는' 이 옳은데 '을(를)'·'은(는)' 이 나온다**. 같은 화면의 `Empty` 는 옳게 낸다 | 공용(shared/crud · shared/format) | A11 (FS-025 §7 #12) |
...
**E2E 미실행 — 이 문서의 모든 판정 근거는 F2(`3cd3078`) 코드 대조다.** 아래는 판정을 *확인할 수 있는* 절차다.
...
- [x] **NFR-023(자매 화면)과 갈리는 판정을 §1.2 에 못 박고**, 본문에서 그 근거(어댑터 차이 · 업종 상수)를 짚었다 — STATE-02(P0) · STATE-06 · EXC-12 가 여기서 pass 이고, EXC-04 는 양쪽 gap 이나 심각도가 다르다
