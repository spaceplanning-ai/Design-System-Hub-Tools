...
| EXC-03 | EXC | 직접 | **부분 미충족.** ① route-level read 게이팅은 **상속으로 충족** — `RequirePermission` 이 `<Outlet>` 을 감싸(`AppShell.tsx:490`) read 권한 없는 deep-link 에 `ForbiddenScreen` 을 렌더하고, 리소스는 `resourceIdForPath` 가 nav 리프(`/portfolio/categories`)에서 파생한다. ② **write-action 게이팅이 이 화면에 없다** — `useRouteWritePermissions()`(`RequirePermission.tsx:45`, `canCreate`/`canUpdate`/`canRemove` 제공)는 **더 이상 소비자 0 이 아니다: 6곳이 쓴다**(`products/{categories/ProductCategoriesPage.tsx:179, items/ProductListPage.tsx:119, returns/ReturnDetailPage.tsx:110}` · `settings/{api-keys/ApiKeysPage.tsx:97, oauth/OAuthPage.tsx:100, site/SiteSettingsPage.tsx:142}`). **가장 가까운 선례가 `products/categories/ProductCategoriesPage` — 이 화면과 같은 taxonomy+모달 패턴인데 그쪽은 `canCreate` 를 본다.** 그런데도 이 화면의 '카테고리 추가'(`:227`)·행 '수정'(`:127`)·행 '삭제'(`:136`)는 쓰기 권한과 무관하게 렌더·클릭된다 | `remove` 권한이 꺼진 역할로 `/portfolio/categories` 진입 → 삭제 버튼이 **여전히 보이고 눌린다**(현재 fail). 기대: 버튼 미렌더/비활성 — `ProductCategoriesPage.tsx:179` 를 그대로 따르면 된다. read 권한을 끄면 `ForbiddenScreen` → **통과**(①). `grep -rn 'useRouteWritePermissions' apps/admin/src/pages/portfolio/` = **0** | **gap** |
...
| 7 | (사각지대) | — | 모달 dirty 상태에서 **브라우저 뒤로가기·새로고침** 시 입력이 조용히 사라진다 — FEEDBACK-06(모달 4경로)과 FEEDBACK-04(페이지 폼 3경로) **어느 쪽도 이 경로를 정의하지 않는다.** quality-bar 자체의 공백 | 앱 전역(폼 모달 전부) | 명세 리뷰(quality-bar 개정 검토) · UI 기획 |
| 8 | COMP-01 | P1 | 행 수정·삭제가 raw `<button>` + `buttonStyle('ghost')` 로 조립(`:96`, `:127-148`) — DS `<Button>` 미사용 | 이 화면(+ COMP-01 appliesTo 목록) | UI 기획 |
| 9 | IA-03 | P1 | breadcrumb 부재로 `/portfolio/categories` 와 `/products/categories` 의 AppHeader 제목이 **둘 다 '카테고리'** — 제목만으로 구분 불가 | 앱 전역(AppHeader) | UI 기획 · 프론트 구현 |
| 10 | ERP-13 | P1 | 삭제 차단 aria-label 의 조사 파손 — '3개 사용 중**라** 삭제할 수 없습니다'(`:141`). **조사 헬퍼는 이제 `shared/format.ts:269+` 에 있지만 계사 '이라/라' 는 없다**(`objectParticle`·`topicParticle`·`directionParticle` 3종뿐) — 헬퍼 확장이 선행돼야 한다. 앱 전역 사용자 대상 `'을(를)'` 리터럴은 0건이므로 ERP-13 의 나머지 절은 충족 | `shared/format`(헬퍼 확장) + 이 화면 · `support/categories/CategoriesPage.tsx:142`(같은 문구 복제) | 프론트 구현 · UI 기획 |
| 11 | ERP-06 | P1 | 0-usage 배지 문구가 화면 간 불일치 — 이 화면 '미사용'(`types.ts:15`) vs 문의 유형 '사용 안 함'(`support/_shared/domain.ts:43`) | 두 화면 | UI 기획 |
| 12 | EXC-06 · EXC-07 · EXC-20 | P1 | 실패가 status 로 갈리지 않는다 — 403/404/409/422/429/500 이 전부 같은 문구. 422 field error 매핑 없음. 5xx 참조 코드 없음 | 앱 전역(+ 이 화면 3개 실패 표면) | UI 기획 · 백엔드 명세 |
| 13 | EXC-05 · EXC-11 · IA-14 | P1 | 클라이언트 타임아웃 상한(`AbortSignal.timeout` 0건)·오프라인 감지(`navigator.onLine` 0건)·반응형 선언 부재 | **앱 전역** | 프론트 구현 · UI 기획 |
| 14 | COMP-12 | P2 | 이름 입력에 글자 수 카운터 없음 — `maxLength=40`(`:136`)이 예고 없이 입력을 멈춘다. `FormField.counter` prop 미사용 | 이 화면 | UI 기획 |
...
- grep 근거: `buttonStyle(` (COMP-01) · `useRouteWritePermissions` (EXC-03 — 이 화면 0 / 앱 전역 6) · `submitLockRef` (EXC-08 — 이 화면 0)
