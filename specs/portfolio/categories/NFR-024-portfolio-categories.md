...
| 4 | EXC-03 | **P0** | 쓰기 액션 게이팅 부재 — **`useRouteWritePermissions`(`RequirePermission.tsx:45`)는 이제 7곳이 소비하지만**(products 3 · settings 4) 이 화면은 그 밖이다. 읽기 전용 역할이 추가·수정·삭제 버튼을 그대로 보고 누른다. **최근접 선례가 같은 taxonomy+모달 패턴인 `products/categories/ProductCategoriesPage.tsx:181`** 이다. route read 게이팅(①)은 충족 | **이 화면**(공용 훅 + 7곳 선례 존재 — 앱 전역 미비가 아니다) | UI 기획 쪽 변경 요청 |
