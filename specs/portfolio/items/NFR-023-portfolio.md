...
| 검증 방법 | **E2E 미실행. 모든 판정 근거는 F2(`3cd3078`) 코드 대조다.** §6 의 재현 스위치는 판정을 *확인할 수 있는 절차*이며 이 문서가 그것을 실행했다는 뜻이 아니다 |
...
| A11Y-11 | A11Y | 직접(+앱 전역) | **절반은 충족**: `aria-invalid` 를 쓰는 이 화면의 모든 컨트롤이 `aria-describedby` 로 이유를 연결한다 — `PortfolioFormPage.tsx:134-137`(분류) · `:156-157`(일자) · `:172-173`(제목) · `:187-188`(고객사), `TextareaField.tsx:62-63`(소개, 자체 배선), `ImageUploadField.tsx:219`·`ImageGalleryField.tsx:231`(`aria-invalid` 를 쓰지 않고 describedBy 만 — 짝 없는 위반이 아니다). 회귀는 `shared/a11y-guard.test.ts` 가 소스 전수 스캔으로 고정한다. **나머지 절반은 미충족**: 요구의 세 번째 절('required 는 native required/aria-required 로 노출')이 성립하지 않는다 — `FormField.tsx:58-62` 의 `required` 는 **시각 `*`(aria-hidden)만** 그리고, 폼의 어떤 컨트롤도 `required`/`aria-required` 를 갖지 않는다 | ① `aria-describedby` 없는 `aria-invalid` grep = 0 → **통과**(가드 테스트가 강제). ② 빈 폼 제출 후 RTL/AT 로 입력의 `aria-describedby` === `role=alert <p>` id → **통과**. ③ **`apps/admin/src` 전체에서 `aria-required` grep = 0, 이 폼의 필수 컨트롤 6개(분류·일자·제목·고객사·소개·대표이미지) 중 `required` 속성을 가진 것 0개 → 미충족**. 범위: 앱 전역(DS `FormField`) | **gap** |
...
| IA-02 | IA | 직접 | **미충족.** 하위 라우트가 브랜치 라벨로 폴백한다: `nav-config.ts:253-264` `findNavLabel('/portfolio/items/new')` → leaf 정확 일치 실패(`/portfolio/items` ≠ `/portfolio/items/new`) → `:260` branch `basePath: '/portfolio'` 가 매치 → **'포트폴리오 관리'** 를 돌려준다. `AppHeader.tsx:92`·`:101` 이 그것을 `<h1>` 으로 그린다. 동시에 `FormPageShell.tsx:159` 가 본문에 `<h1>포트폴리오 등록</h1>` 을 또 그린다 → **한 화면에 `<h1>` 2개, 서로 다른 문구**. 더해 목록(`/portfolio/items`)은 in-content title 이 없어 AppHeader 만 제목을 갖는다 — 목록/폼의 title 소스가 서로 다르다. F2 가 `AppHeader.tsx` 를 손봤지만 바뀐 것은 `pageTitleStyle` 소비(TOKEN-05)뿐이고 **제목 소스는 여전히 `findNavLabel(pathname)`** 이다 | `/portfolio/items/new` 를 연다 → 헤더 `<h1>` = '포트폴리오 관리'(구체 title '포트폴리오 등록' 이 아니다). `document.querySelectorAll('h1').length` = 2, 두 텍스트가 다르다. 요구의 acceptanceCheck('`/content/notices/new` 의 가시 primary title 이 '공지 등록'')의 포트폴리오 대응 | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** 이 화면의 list query state(분류 필터)가 **컴포넌트 `useState` 에만 있다** — `PortfolioListPage.tsx:47` `const [filter, setFilter] = useState(PORTFOLIO_FILTER_ALL)`. 파일에 `useSearchParams`·`useListState` import 0건. 앱에는 이미 URL 단일 원천 훅이 있고(`shared/crud/useListState.ts`, 배럴로 export, 10개 화면이 소비) 그 헤더가 정확히 이 실패를 설명하는데도 이 화면이 소비하지 않는다. 정렬은 서버 고정이라 URL 대상이 아니고, page·keyword 는 표면 자체가 없다 | 분류를 '오피스'로 바꾼 뒤 ① F5 → **'전체 분류'로 되돌아간다** ② 행을 눌러 수정 폼에 갔다가 브라우저 Back → **필터가 풀린 목록에 착지한다** ③ 주소창 URL 을 복사해 새 탭에 붙여넣기 → **필터가 재현되지 않는다**(URL 에 `?` 가 아예 붙지 않는다). `grep -n 'useSearchParams\|useListState' apps/admin/src/pages/portfolio/items/PortfolioListPage.tsx` = 0 | **gap** |
...
| EXC-03 | EXC | 직접 | **부분 충족 → 미충족.** ① **route-level authorization 은 성립한다**: `AppShell.tsx:490-492` 가 `<Outlet>` 을 `RequirePermission` 으로 감싸 read 권한이 없으면 `ForbiddenScreen` 을 렌더한다. 리소스는 `route-resource.ts:36-46` 이 경로에서 파생하므로(`/portfolio/items/:id/edit` → leaf `/portfolio/items`) **폼·상세 라우트까지 자동으로 덮인다**. 강등 reconcile 도 권한 스토어 구독으로 성립한다. ② **write-action 게이팅은 전무하다**: `useRouteWritePermissions`(`RequirePermission.tsx:45`)의 **소비처가 앱 전체에서 0건**이다 — 이 화면의 '포트폴리오 등록'(`PortfolioListPage.tsx:113`) · 행 액션 수정/삭제(`CrudTable.tsx:192-197`) · 일괄 삭제(`CrudListShell.tsx:126-132`) · 노출 토글(`publishColumn.tsx:20`)이 `can(resource, action)` 을 보지 않고 무조건 렌더된다. 요구의 두 번째 절('create/update/delete 컨트롤은 `can()` 이 false 면 렌더 안 함/disable')이 미충족 | ① read-forbidden 역할로 `/portfolio/items` 딥링크 → 403 화면 = **통과**. ② delete 권한이 없는 역할로 목록을 연다 → **행 삭제·일괄 삭제 버튼이 그대로 보이고 눌린다** = 미충족. `grep -rn 'useRouteWritePermissions\|useRouteCan' apps/admin/src/pages/` = 0. 범위: 앱 전역(모든 `*ListPage`) | **gap** |
| EXC-04 | EXC | 직접 | **미충족.** ① **토큰이 없다**: `PortfolioItem`(`_shared/store.ts:23-40`)에 `updatedAt`·`version` 필드가 없어 If-Match/ETag 로 보낼 값 자체가 존재하지 않는다. ② **어댑터가 409 를 던지지 않는다**: 이 화면은 `createStoreAdapter`(`crud.ts:107-137`)를 쓰는데 그 `update`(`:126`)·`remove`(`:131`)에는 **존재 검사가 없다** — `store.updateItem`(`:182` `items.map`)·`store.removeItem`(`:188` `items.filter`)이 없는 id 를 조용히 지나치고 성공을 반환한다. **요구의 근거 문장('`createCrudAdapter.update/remove` 가 id 부재 시 조용히 no-op 하고 success 를 반환')이 지목한 바로 그 버그이며, F2 는 `createCrudAdapter`(`:71`·`:82`)만 고치고 `createStoreAdapter` 는 고치지 않았다.** ③ 소비자 측은 준비돼 있다: `useCrudForm.ts:160-172` 가 `isConflict` 로 분기해 입력을 보존한 채 충돌 다이얼로그를 띄운다(`FormFeedback.tsx:58-75`) — 생산자만 없다 | ① 탭 A 에서 `/portfolio/items/pf-1/edit` 를 연다 → 탭 B(같은 탭 세션)에서 `pf-1` 을 삭제 → 탭 A 에서 저장 → **'포트폴리오을(를) 저장했습니다' 유령 토스트가 뜨고 목록으로 이동한다**(저장된 것은 없다). conflict dialog 없음 = 미충족. ② 대조: `?status=save:409` 로는 충돌 다이얼로그가 정상적으로 열린다 — **UI 는 있고 트리거만 없다**는 증거. ③ `grep -n 'updatedAt\|version\|If-Match' apps/admin/src/pages/portfolio/` = 0 | **gap** |
| EXC-08 | EXC | 직접 | ① **동기 제출 락**: `useCrudForm.ts:102` `submitLockRef` + `:195-197` `if (submitLockRef.current) return; submitLockRef.current = true` — RHF 비동기 검증과 `disabled={saving}` 렌더 사이의 틈을 ref 가 닫는다(렌더를 기다리지 않는다). `:207-209` `onSettled` 와 `:239-241` `onInvalid` 가 락을 해제한다. ② **disable**: `FormPageShell.tsx:188` `disabled={saving || loadingDetail}`. ③ **멱등키가 mutationFn 밖에 있다**: `:112-117` `idempotencyKeyRef` + `takeIdempotencyKey()` 가 **제출 시도 단위**로 키를 잡아 재클릭이 같은 키를 재사용하고, `:214` 성공 시에만 버린다 — 요구가 명시한 구조('key 가 mutationFn 밖에 존재')를 만족한다. 삭제·일괄 삭제·토글도 요청 중 컨트롤을 잠근다 | 등록 폼을 채우고 **Enter 를 연타**한다(응답 400ms 전) → 요청 1건, 항목 1건만 생성. `?status=save:500` 후 재제출 → `idempotencyKeyRef` 가 같은 키를 유지한다(성공 전까지 리셋되지 않는다). **주의(§5 #17)**: 키가 어댑터로 **전달되지는 않는다**(`:206` 반환값 폐기, `CrudAdapter.create/update` 에 키 파라미터 없음) — 현재 네트워크가 없어 관측 차이가 없고, 요구의 두 검사(1요청 발사 · 키가 mutationFn 밖)는 충족한다 | pass |
...
> **P0 gap 8건 = quality-bar '배치 실패' 사유.** 다만 8건 중 **이 화면 고유는 3건**(STATE-02 · IA-13 · EXC-04)이고, **5건은 앱 전역/공용 프레임워크 범위**(A11Y-11 · MOTION-03 · IA-02 · IA-04 · EXC-03)로 이 화면만 고쳐서는 해소되지 않는다 — §5 의 '범위' 열이 이를 가른다.
...
| ERP-13 | P1 | 조사 폴백형 리터럴을 **이 화면이 실제로 출력한다** — `useCrudList.tsx:107`·`:145`(`'<제목>' 을(를) 삭제했습니다.`), `useCrudForm.ts:215`('포트폴리오을(를) 등록했습니다.'), `validation.ts:15-17` `requiredText`('제목을(를) 입력하세요.'), `FormPageShell.tsx:128-129`. **대조**: `@tds/ui` `Empty.tsx:17-27` 은 이미 받침 계산으로 옳게 쓴다 — 앱 shared 에 그 헬퍼가 없을 뿐이다 | 항목을 삭제 → 토스트가 `'…' 을(를) 삭제했습니다.` 로 뜬다. 사용자 대상 문자열에 `'을(를)'` grep > 0 | gap |
...
| EXC-12 | P1 | **셸은 갈랐으나 어댑터가 못 실어 준다** — `useCrudForm.ts:138-143` 이 `isNotFound(loadError)` 로 `'not-found' \| 'error'` 를 만들고 `FormPageShell.tsx:115-143` 이 두 문구·복구 수단을 가른다(F2 개선). 그러나 이 화면의 `store.getItem`(`store.ts:170`)이 status 없는 generic Error 를 던져 **항상 'error' 로 떨어진다** | 없는 `:id` 로 `/portfolio/items/zzz/edit` 진입 → '찾을 수 없습니다 + 목록으로' 가 아니라 **'불러오지 못했습니다' + '다시 시도' + '목록으로'** 가 뜬다(재시도해도 영원히 없다). 무한 loading 은 없다 | gap |
...
| EXC-15 | P1 | 업로드 전 client 검증은 성립한다 — `ImageUploadField.tsx:19-25` `imageFileError(file, 5)` 가 type/size 를 막고 인라인 거절(`:154-157`), 요청 미발사. 로드 실패 fallback 은 갤러리만 `role="img"`(`ImageGalleryField.tsx:79`)이고 단일 필드는 드롭존 문구 전환(`:83-87`). **progress/cancel 경로 없음**(업로드 자체가 없으므로 — §5 #15) | 비이미지/6MB 파일 선택 → 인라인 거절, 값 미변경. progress 표시 없음 | gap(부분) |
...
| 3 | EXC-04 | **P0** | 동시성 토큰 부재(`updatedAt`/`version` 없음) + `createStoreAdapter` 가 없는 id 를 조용히 성공 처리 → 유령 저장. 충돌 다이얼로그는 배선돼 있으나 트리거가 없다 | 이 화면 + 공용(`shared/crud/crud.ts` `createStoreAdapter`) | A63 (BE-023 §7.3 · §7.10 #2) · A11 |
| 4 | EXC-03 | **P0** | 쓰기 액션 권한 게이팅 전무 — `useRouteWritePermissions` 소비처 0건. read 게이팅은 성립 | **앱 전역**(모든 `*ListPage`) | A11 change_request (FS-023 §7 #2) |
| 5 | IA-04 | **P0** | Pagination 부재 — `CrudListShell` 이 전량을 렌더한다. 항목 수 상한 없음 | **앱 전역**(shared/crud) | A11 change_request (FS-023 §7 #3) |
| 6 | IA-02 | **P0** | 하위 라우트가 브랜치 라벨로 폴백 + `<h1>` 2개(AppHeader `findNavLabel` ↔ FormPageShell). 목록/폼의 title 소스 불일치 | **앱 전역**(AppHeader · nav-config) | A11 change_request · A40 (FS-023 §7 #12) |
| 7 | A11Y-11 | **P0** | 필수 필드가 `required`/`aria-required` 를 노출하지 않는다(`aria-required` 앱 전역 grep = 0). describedby 절반은 충족 | **앱 전역**(@tds/ui `FormField`) | A11 change_request (FS-023 §7 #14) |
...
| 9 | EXC-12 | P1 | 셸은 404/500 을 가르지만 `store.getItem` 이 status 없는 Error 를 던져 404 분기가 죽어 있다 | 이 화면 | A63 (BE-023 §7.10 #7) |
...
| 15 | EXC-15(부분) | P1 | 업로드 progress/cancel 경로 없음 — **업로드 자체가 없기 때문**. `blob:` 값이 저장된다. client 검증은 성립 | 이 화면 + 계약 미정 | A63 (BE-023 §7.2 · §7.10 #1) · A11 |
| 16 | A11Y-08 | P1 | 행 안에 keyboard-focusable 이름 링크 없음(제목이 평문, 행 클릭은 마우스 전용) | 공용(shared/crud `CrudTable`) | A11 (FS-023 §7 #4) |
| 17 | EXC-08(부분) | P1 | 멱등키가 생성되나 어댑터로 전달되지 않는다(반환값 폐기, 시그니처에 파라미터 없음). 중복 제출 차단은 pass | 공용(shared/crud) | A63 (BE-023 §7.8 · §7.10 #5) · A11 |
| 18 | ERP-13 | P1 | 조사 폴백형 리터럴('을(를)'·'은(는)') 출력 — 토스트·검증 문구·로드 실패 문구 | 공용(shared/crud · shared/format) | A11 (FS-023 §7 #13) |
...
**E2E 미실행 — 이 문서의 모든 판정 근거는 F2(`3cd3078`) 코드 대조다.** 아래는 판정을 *확인할 수 있는* 절차이며, 이 문서가 그것을 실행했다는 뜻이 아니다.
...
| 예시 | 확인 대상 |
|---|---|
| `?status=save:409` | FS-023-EL-029 충돌 다이얼로그(입력 보존·유령 저장 금지) — **UI 는 동작한다**. §5 #3 은 '실사용 트리거 부재'가 문제다 |
| `?status=save:422` | EXC-07 필드 인라인 경로. **단 `dev.ts` 가 `violations` 를 싣지 않아 배너로 폴백한다**(§5 #12) |
| `?status=save:500` | EXC-20 오류 참조 코드(`TDS-…`) |
| `?status=detail:404` | EXC-12 not-found 분기 — 셸이 '찾을 수 없습니다 + 목록으로'(재시도 없음)를 그린다. **실사용 경로는 이 status 를 못 만든다**(§5 #9) |
...
| 도구 | 고정하는 것 |
|---|---|
| `apps/admin/src/shared/a11y-guard.test.ts` | A11Y-11 의 describedby 절(짝 없는 `aria-invalid` = 0) · A11Y-12(필터에 `aria-current` = 0). **`aria-required` 는 이 가드가 보지 않는다** — §5 #7 |
