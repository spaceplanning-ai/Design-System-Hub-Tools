...
| A11Y-12 | A11Y | 직접 | **충족 — 그리고 이제 이 화면이 직접 그리지 않는다.** 좌측 필터 골격이 공용 `shared/ui/FilterPanel.tsx` 한 벌로 수렴했고, `aria-pressed={active}` 는 그 정본이 소유한다(`FilterPanel.tsx:146`). 이 화면은 `FilterPanel`/`FilterRail` 을 합성만 한다(`CategoryUsageFilter.tsx:18-27`) — 자기 버튼이 없으므로 표기가 갈릴 자리 자체가 없다. 파일 헤더가 그 결정을 적는다(`CategoryUsageFilter.tsx:3-4` '여기서 다시 만들지 않는다 … 선택 상태는 aria-pressed 하나로만 말한다(A11Y-12)'). **`pages/products/categories/**` 에 `aria-current` 0건**(grep 확인) | `grep -rn "aria-current" apps/admin/src/pages/products/categories` → **0건이어야 한다.** 회귀는 이제 자동으로 잡힌다 — `shared/a11y-guard.test.ts:67-88` | **pass** |
