...
| A11Y-12 | A11Y | N/A | **표면이 없다.** 요구의 appliesTo 는 '**좌측 필터 list item**' 이며, 그 골격의 정본은 공용 `shared/ui/FilterPanel.tsx` 다(요구가 원래 나열했던 개별 필터들은 그 한 벌로 흡수됐다 — `quality-bar.md` A11Y-12 '해소 경위'). **이 화면에 좌측 필터도 toggle 필터 list 도 없다** — 단일 문서 폼이라 필터할 대상이 없다. `FilterPanel`·`FilterRail` 소비 0건, **`aria-current` 0건**(grep 확인). 묶음배송 `ToggleSwitch` 는 **폼 입력**이지 필터 toggle 이 아니며 `role="switch"` + `aria-checked` 를 쓴다(`packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.tsx:26` — 그것이 옳은 속성이다) | 좌측 토글 필터가 도입되면 이 판정을 다시 매긴다 | **n-a** |
