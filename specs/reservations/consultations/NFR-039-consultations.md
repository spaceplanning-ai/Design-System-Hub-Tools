...
> **이 화면의 판정 지형**: 목록은 `useCrudList` + `CrudListShell`, 폼은 `useCrudForm` + `FormPageShell` 위에 있다. 그래서 STATE-01/02 · EXC-04/08/09 · STATE-05 · A11Y-16 · EXC-12 를 **프레임워크가 이미 충족시킨다** — 같은 섹션의 신청서(NFR-038)가 그 프레임워크를 우회해 같은 요구를 gap 으로 남긴 것과 대조된다. 이 화면의 gap 은 **프레임워크가 아직 안 하는 것**(URL state · IME · Pagination · 쓰기 권한 게이팅 · `aria-required`)과 **도메인 결정이 없는 것**(시간 충돌)에 몰려 있다.
...
| COMP-10 | COMP | 직접 | **미충족.** `ConsultationBookingListPage.tsx:116-121` 이 `SearchField` 에 `onChange={setKeyword}` 를 직결한다 — `isComposing` 처리도 디바운스도 없다. 공용 `useDebouncedSearch`(`shared/crud`, 8파일 소비, `isComposing` 처리 포함)를 **이 화면은 쓰지 않는다**(reservations 전체에서 import 0건). DS `SearchField` 도 조합을 다루지 않는다(`SearchField.tsx:66`). **정직한 한정**: 검색이 **클라이언트 필터**라(`:80-83`) 자모마다 요청이 나가지도, 순서 뒤바뀐 응답이 최신을 덮지도 않는다 — 요구가 막는 실패 중 **네트워크 축은 현재 재현 불가능**하다. 그러나 조합 계약(compositionend 에서만 commit) 자체가 미구현이라 조합 중 자모가 그대로 필터에 걸린다 | 검색창에 '윤아름' 을 IME 로 입력 → 조합 중 표가 자모 단위로 재필터되면 gap. `isComposing` grep = 0 으로도 확인 | **gap** |
...
| A11Y-11 | A11Y | 직접 | **미충족(required 노출 축).** 축을 갈라 보면 — ① **`aria-invalid` ↔ `aria-describedby` 짝: 충족.** 인라인 에러를 갖는 6개 필드 전부가 `aria-invalid={errors.X !== undefined}` 와 `aria-describedby={errors.X !== undefined ? errorIdOf('csb-X') : undefined}` 를 **쌍으로** 건다(`ConsultationBookingFormPage.tsx:150-153, 172-175, 210-212, 229-233, 248-253`) — 짝 없는 `aria-invalid` 0건이고, `FormField:72` 가 그 id 로 `role="alert"` `<p>` 를 그린다. ② **required 노출: 미충족.** `FormField required` 는 `*` 를 **`aria-hidden="true"`** 로만 그리고(`FormField.tsx:58-62`) 컨트롤에 `required`/`aria-required` 를 붙이지 않는다 — 그 배선은 **호출부 책임**인데 이 폼의 어느 입력에도 없다. **필수 6필드(고객명·연락처·상담유형·주제·희망날짜·희망시각)의 필수 여부가 스크린리더에 전달되지 않는다.** 요구는 세 축('aria-invalid↔describedby' · 'hint는 valid일 때만' · '**required는 native required/aria-required로 노출**')을 모두 요구한다 | 폼 파일에 `aria-required`·`required=` (JSX 속성) grep = 0 → gap. RTL 로 `getByLabelText('고객명')` 의 `aria-required` 가 없음을 assert. `aria-invalid` 짝 축은 pass | **gap** |
...
| IA-02 | IA | 직접 | **미충족.** `findNavLabel`(`shared/layout/nav-config.ts:253-264`)은 잎을 **정확 일치**로만 찾고, 못 찾으면 `pathname.startsWith(item.basePath)` 인 **브랜치 라벨로 폴백**한다. `/reservations/consultations/new` 와 `/:id/edit` 는 잎이 아니므로 → **'예약/신청 관리'**. 그런데 `FormPageShell:159` 가 본문에 `<h1>상담 예약 등록/수정</h1>` 를 또 그린다 — **한 화면에 `<h1>` 이 둘이고 상단의 것이 요구가 금지한 브랜치 라벨이다**. 목록(`/reservations/consultations`)은 잎과 정확히 일치해 '상담 예약' 으로 옳게 나오고 in-content h1 이 없다 — **list 는 AppHeader 가, form 은 본문이 제목을 대는 모순**. **F2 가 `AppHeader.tsx` 를 수정했으나(+15줄) 그것은 `titleStyle` 을 primitive 손복제에서 공유 `pageTitleStyle` 로 바꾼 TOKEN-05 작업이고 `findNavLabel(pathname)` 호출은 그대로다** — 현재 코드로 재확인함 | `/reservations/consultations/new` 진입 → 가시 primary title 이 '예약/신청 관리' 이고 `<h1>` 이 2개면 gap | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** 상태 필터와 키워드가 컴포넌트 `useState` 에만 있다 — `ConsultationBookingListPage.tsx:65-66`. `useSearchParams` 도, 공용 `useListState`(`shared/crud` — 실소비자는 `members/**` 뿐)도 쓰지 않는다(reservations 전체에서 import 0건). **이 화면은 행 클릭이 곧 수정 폼 진입이라(FS-039-EL-008.12) 목록→폼→Back 이 핵심 루프인데**, 그 Back 이 필터를 버린다. URL 공유도 불가능하다 | 상태='요청' + 키워드 입력 → 행 클릭으로 수정 폼 진입 → 브라우저 Back → **필터·키워드가 초기화돼 있으면 gap**. URL 을 새 탭에 붙여 넣어도 조건이 재현되지 않는다 | **gap** |
...
| EXC-03 | EXC | 직접 | **미충족(쓰기 게이팅 축).** read 가드는 상속으로 충족 — `AppShell.tsx:490-492` 가 `<Outlet>` 을 `RequirePermission` 으로 감싸 read 권한 없는 라우트에 `ForbiddenScreen` 을 그리고, 리소스는 `resourceIdForPath` 가 라우트에서 파생하므로 `/new`·`/:id/edit` 같은 하위 라우트도 잎 `/reservations/consultations` 의 리소스로 덮인다(`route-resource.ts` — '자기를 감싸는 가장 구체적인 잎이 곧 자기 리소스'). **그러나 쓰기 컨트롤 게이팅이 이 화면에 없다** — `useRouteWritePermissions`/`useRouteCan` 을 `pages/reservations` 전체가 import 0건이다. **'상담 예약 등록' 버튼 · RowActions 의 수정/삭제 · SelectionBar 의 일괄 삭제 · 폼의 '저장' 이 전부 권한과 무관하게 렌더·활성된다** — 요구가 'delete 없는 role 이 row/bulk delete 버튼을 못 봄' 을 명시적으로 요구한다. 강등 reconcile 도 없다 | remove 권한 없는 역할로 목록 진입 → **행 삭제·일괄 삭제 버튼이 그대로 보이고 눌리면 gap**. `pages/reservations` 에 `useRouteCan` grep = 0 | **gap** |
| EXC-04 | EXC | 직접 | 충족 — **acceptanceCheck 기준. 단 잔여가 있다.** 이 화면이 `useCrudForm` 을 소비하고, 그 훅의 `handleWriteError` 가 `isConflict(cause)` 를 보면 **덮어쓰지 않고** 충돌 state 를 세워(`useCrudForm.ts:160-173`) `FormConflictDialog` 를 띄운다 — 입력은 폼에 그대로 남고, **성공 토스트도 목록 이동도 없다**(유령 저장 금지). '최신 불러오기'(재조회 → `:125-128` effect 가 `reset`)와 '닫기'(입력 유지) 두 선택을 준다. 어댑터도 없는 id 의 update 에 409 를 던져 **ghost saved 를 막는다**(`crud.ts:71-73` — F2 작업). **잔여(요구의 '토큰을 보낸다' 축)**: `ConsultBooking` 에 `updatedAt`/`version` 이 없고 `If-Match` 는 앱 전체에서 회원 메모에만 있다 — 지금 감지되는 것은 '먼저 **삭제**됐다' 뿐이고 '먼저 **변경**됐다' 는 못 잡는다(BE-039 §7.6). **acceptanceCheck 의 두 항목('fixture 409 가 입력 유지 + 충돌 다이얼로그 + 토스트/이동 없음' · '제거된 record 편집이 conflict 배너')은 모두 관측 가능하므로 pass 로 판정하되, 토큰 부재를 §4.2 잔여로 남긴다** | `?status=save:409` 로 수정 저장 → **충돌 다이얼로그가 뜨고 입력이 남고 이동이 없으면 pass**. 다른 탭에서 그 항목을 삭제한 뒤 저장해도 동일 | **pass** |
| EXC-08 | EXC | 직접 | 충족. 이 화면이 `useCrudForm` 을 소비한다. ① **동기 락** — `submitLockRef`(`useCrudForm.ts:102`)를 `onValid` 첫 줄에서 검사·설정하고(`:195-196`) `onSettled` 에서 푼다(`:207-209`). **RHF 의 비동기 검증이 끝나 `saving` 이 렌더되기 전의 빠른 두 번째 Enter/클릭이 여기서 멈춘다** — `disabled={saving \|\| loadingDetail}`(`FormPageShell:188`)과 **이중**이다. ② **멱등키** — `idempotencyKeyRef`(`:112-117`)를 **mutationFn 밖**에서 만들어 재시도가 **같은 키**를 재사용하고, 성공 시 버려 다음 제출이 새 거래가 된다(`:214`). 검증 실패 시 락을 풀어(`:239-241`) 다시 제출할 수 있다 | '등록' 을 응답 전에 빠르게 두 번 클릭(또는 Enter 연타) → **정확히 1건만 발사되면 pass**. 실패 후 재클릭이 같은 키를 재사용하는지는 `Idempotency-Key` 헤더 배선 후 확인(BE-039 §2) | **pass** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| `pass` | **8** | STATE-01 · STATE-02 · TOKEN-01 · FEEDBACK-04 · IA-05 · EXC-04 · EXC-08 · EXC-09 |
...
| `gap` | **8** | STATE-04 · COMP-10 · FEEDBACK-02 · A11Y-11 · IA-02 · IA-04 · IA-13 · EXC-03 |

**검산**: 8 + 12 + 2 + 8 = **30** ✓ (STATE 3 · TOKEN 5 · COMP 1 · FEEDBACK 3 · A11Y 4 · MOTION 3 · IA 5 · EXC 6 = 30)

> **P0 gap 8건 — quality-bar '배치 실패' 사유다.** 소유를 갈라 보면: **이 화면 코드로 닫는 것 5건**(COMP-10 · FEEDBACK-02 · IA-04 · IA-13 · EXC-03), **공용 프레임워크가 닫는 것 2건**(STATE-04 — `useCrudList` 단건 삭제 경로에 `clear()` 한 줄, **10개 소비 화면 공통** · A11Y-11 — DS `FormField` 의 required 배선 또는 호출부 6곳), **앱 전역 1건**(IA-02 — `findNavLabel`). **가장 싼 것은 STATE-04 다** — `useCrudList.tsx:104-108` 에 `clear()` 한 줄이면 이 화면과 나머지 9개 화면이 함께 닫힌다.
...
| ERP-13 | P1 | **`useCrudList` 의 토스트·확인 문구가 리터럴 '을(를)' 을 렌더한다** — `'<고객명> (<번호>)' 을(를) 삭제했습니다.`(`:107`) · 같은 형식의 확인 본문(`:157`). `FormPageShell`/`useCrudForm` 도 '상담 예약을(를) 저장했습니다.'(`useCrudForm.ts:215`) · '상담 예약을(를) 찾을 수 없습니다.'(`FormPageShell:128`). **공용 `Empty` 만 조사 헬퍼를 갖는다** — 나머지는 리터럴 fallback 형이다 | 삭제 성공 토스트에 '을(를)' 이 보이면 gap | gap(공용) |
...
| 2 | COMP-10 | P0 | 검색에 IME 조합·디바운스 없음. `useDebouncedSearch` 미사용 | 이 화면 | A11 (FS-039 §7 #4) |
| 3 | FEEDBACK-02 | P0 | 비가역 종료 전이(취소·노쇼·상담완료)에 ConfirmDialog 게이트 없음. **삭제 축은 충족** | 이 화면 | A11 (FS-039 §7 #2) |
| 4 | A11Y-11 | P0 | 필수 6필드에 `required`/`aria-required` 없음 — `FormField` 의 `*` 가 `aria-hidden`(`FormField.tsx:58-62`). **`aria-invalid`↔`describedby` 짝은 충족** | DS `FormField` 또는 호출부 6곳 | A11 (FS-039 §7 #5) |
| 5 | IA-02 | P0 | 폼 화면에 `<h1>` 이 둘이고 상단이 브랜치 라벨 '예약/신청 관리' | 이 화면 + 앱 전역(`findNavLabel`) | A40 · A11 (FS-039 §7 #3) |
| 6 | IA-04 | P0 | Pagination 없음 — 전량 렌더. **템플릿의 다른 축은 `CrudListShell` 덕에 전부 충족** | 이 화면 + 계약(BE-039 §7.8 #9) | A11 · A63 |
| 7 | IA-13 | P0 | 필터·키워드가 `useState` 에만(`:65-66`). `useListState` 미사용. **목록→폼→Back 이 핵심 루프인데 그 Back 이 필터를 버린다** | 이 화면 | A11 (FS-039 §7 #19) |
| 8 | EXC-03 | P0 | 쓰기 권한 게이팅 없음 — 등록·수정·삭제·일괄 삭제 컨트롤이 권한 무관 렌더 | 이 화면 (read 가드는 상속으로 충족) | A11 (FS-039 §7 #9 · BE-039 §7.8 #6) |
...
| 21 | ERP-13 | P1 | `useCrudList`·`useCrudForm`·`FormPageShell` 의 토스트/확인/에러 문구가 리터럴 '을(를)' 렌더 | **공용 — 다수 화면 공통** | A11 |
...
| `?status=detail:404` | **'상담 예약을(를) 찾을 수 없습니다. 이미 삭제되었을 수 있습니다.' + 목록으로(재시도 없음)** | **EXC-12 pass 확증** |
...
- [x] §2.1 산수 검산 — pass 8 + 종속 12 + N/A 2 + gap 8 = **30** ✓
