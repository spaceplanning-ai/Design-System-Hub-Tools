...
**이름 주의**: 이 화면의 '상담' 은 **이미 끝난 상담의 이력**(과거 기록·읽기 전용)이지 **예약·일정이 아니다.** 위 `n-a` 14건은 **쓰기·폼·모달 표면이 없다**는 이 화면의 성격에서 나온 판정이므로, 쓰기가 있는 다른 화면(예약·접수·처리류)에 그대로 옮기면 **전부 틀린다.** (과거 배치에 존재했던 `/reservations/consultations`(FS-039 상담 예약)는 삭제됐다 — 이 문서의 판정은 그 화면과 무관하다.)
...
- [x] **이 화면의 '상담' 이 예약이 아니라 과거 이력(읽기 전용)임을 §1.2 에 명시**하고, **이 문서의 N/A 판정이 그 성격에서 나온 것이라 쓰기가 있는 화면에 옮기면 전부 틀린다**고 경고했다
