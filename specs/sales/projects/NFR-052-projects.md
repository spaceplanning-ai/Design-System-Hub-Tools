...
| A11Y-11 | A11Y | 직접 | **부분 미충족 — 한 폼 안에서 규칙이 갈렸다.** **required 절 — 충족(5/5)**: `project-name`(input ✔) · `project-account`(input ✔) · `project-stage`(**`SelectField` — `FormField.tsx:40` 이 허용 대상으로 인정** ✔) · `project-lost-reason`(input ✔) 이 `FormField required` 의 **런타임 `aria-required` 주입**(`FormField.tsx:50-56` `withAriaRequired`)을 받고, `DateRangeField required` 는 **컴포넌트가 두 입력에 직접** `required`+`aria-required` 를 준다(`DateRangeField.tsx:48`). **래퍼 `div`/`span` 자식으로 주입이 막히는 경우가 이 화면에 0건**이다 — `설정/OAuth` 의 Client Secret 이 입력과 '변경' 버튼을 나란히 놓는 래퍼 `<span>` 을 자식으로 둬 주입이 막히고 호출부가 `<input>` 에 직접 `aria-required` 를 주는 것으로 메워야 했던 것과 달리(`OAuthProviderCard.tsx:277-285`), 이 화면은 그 보정 자체가 필요 없다. **describedby 절 — 충족**: `aria-invalid` 를 세운 곳(`:307,329`)은 **전부 `aria-describedby` 와 짝**이고(`:308-310,330-332`) 그 id 가 `FormField` 의 `<p id={errorIdOf(htmlFor)} role="alert">`(`FormField.tsx:124`)와 일치한다. `DateRangeField` 도 invalid 시 두 입력에 짝으로 준다(`:45`). `TextareaField` 는 내부 배선(`:62-67`). **'describedby 없는 aria-invalid' grep = 0 ✔**. **미충족 절**: **오류가 있는데 `aria-invalid` 를 세우지 않는 필드가 4개**다 — 확률(`:367-382`) · 예상매출(`:383-398`) · 진척률(`:433-448`) · 실주 사유(`:401-418`). 이들은 `FormField error={errors.X?.message}` 로 `<p role="alert" id="…-error">` 를 **렌더하면서도** 입력에 `aria-invalid` 도 `aria-describedby` 도 주지 않는다 → **AT 가 그 입력을 유효한 것으로 읽고, 오류 문구와의 연결이 없다.** 시각 신호는 `controlStyle(errors.X !== undefined)` 의 **붉은 테두리뿐 — 색만의 오류 인코딩**이다. 마일스톤 오류(`ProjectMilestonesField.tsx:157-159`)는 **id 조차 없어** 이을 수도 없다. **요구의 acceptanceCheck 두 절 중 grep 절은 통과하고 RTL 절('input 의 aria-describedby === role=alert `<p>` id')은 4개 필드에서 실패한다** | `grep -n "aria-invalid" apps/admin/src/pages/sales/projects -r` → 2건(`:307,329`), **각각 같은 요소에 `aria-describedby` 가 있으므로 그 절은 pass.** RTL: 확률에 '150' 을 넣고 제출 → `screen.getByLabelText('확률 (%)').getAttribute('aria-invalid')` 가 **null 이면 gap**, `getAttribute('aria-describedby')` 가 **null 이면 gap**. required: `getByLabelText('프로젝트명').getAttribute('aria-required') === 'true'` 확인(pass 예상) | **gap** |
