...
| **평문이 사는 유일한 자리는 노출 모달의 지역 state 다** | 발급 응답(`ApiKeyIssued`)에만 실리고 `ApiKeysPage.tsx:75` `const [issued, setIssued] = useState<ApiKeyIssued \| null>(null)` 에만 산다. **react-query 캐시에 넣지 않는다**(`queries.ts:31-34` — `onSuccess` 가 목록만 invalidate). 모달을 닫으면 `setIssued(null)`(`:140-142`) — 그 순간 평문은 앱 어디에도 없다 |
| **폐기는 되돌릴 수 없다 — 확인을 거친다** | `ConfirmDialog intent="delete"` + 결과를 말하는 문구(`:53-55`). 상태가 `active`/`revoked` 2개뿐이라 '일시정지'가 없다(`types.ts:36-37`) |
| **폐기는 지우지 않는다 — 감사다** | `revoked` 로 남긴다(`data-source.ts:117-119` '이 키가 언제까지 살아 있었나 는 사고 조사에서 반드시 필요한 기록이다') |
...
| FS-069-EL-009 | FS-069-SEC-02 | 발급 실행 규칙 | 텍스트 | `submitCreate(draft)`(`:99-128`): ① `createLock.acquire()` 가 false 면 중단(EXC-08 — '두 번 발급되지 않는다') ② `setCreateError(null)` ③ 새 `AbortController` 로 `create.mutate({ draft, signal })` | O | 비표시 규칙. **멱등키가 없다**(§7 #8) |
...
| 중복 제출 | 발급·폐기 각각 `disabled` + **동기 잠금 `useSubmitLock` 2개**(EL-016). **멱등키가 없다** — 사이트·언어·OAuth 설정과 달리 **`If-Match` 같은 토큰도 없어** 응답 유실 후 재시도가 **키를 2개 만든다**(§7 #8) |
...
| **평문의 수명** | 발급 응답 → `ApiKeysPage` 지역 state(`:75`) → `RevealKeyModal` prop → 모달 닫기 시 소멸. **react-query 캐시·목록·전역 상태 어디에도 들어가지 않는다**(`queries.ts:6-8`). 회귀 테스트가 못박는다(`api-keys.test.ts:61` — 목록 응답 전체를 `JSON.stringify` 해도 평문이 없다) |
...
| FS-069-EL-007.6 / EL-008 / EL-009 / EL-009.1 | 키 발급 | W | 요청 `{ name, scopes[] }` · 응답 `{ key, plaintext }` | `createApiKey(draft, signal)` (`:89-115`) | `useCreateApiKey`. **평문은 이 응답에만 실린다**(`:81-84`). `onSuccess` 는 **목록만 invalidate**(`queries.ts:31-34`) |
...
- [x] **평문이 사는 자리를 코드로 추적**했다: 발급 응답 → `ApiKeysPage.tsx:75` 지역 state → `RevealKeyModal` prop → 소멸. **캐시에 넣지 않음을 `queries.ts:31-34` 로 확인**하고 §4.1 '평문의 수명' 에 고정했다
...
| 8 | **발급에 멱등키가 없다 — 이 화면 최대 위험.** 동기 잠금(EL-016)은 연타를 막지만 **응답 유실 후 재시도는 새 키를 만든다.** 사용자는 첫 키를 받지 못했고 서버엔 **폐기되지 않은 유령 키**가 남는다 — 아무도 그 존재를 모른다. **`POST` 라 `If-Match` 같은 토큰도 쓸 수 없다**(사이트·언어·OAuth 는 PUT + revision 이라 재적용이 구조적으로 막힌다 — 이 화면만 무방비다). 앱에 선례가 있다: `members/components/PointsCard.tsx:103,162-173`(`idempotencyKeyRef`, 성공해야 키를 버린다) · `members/data-source.ts:248`(심 `Idempotency-Key: <uuid>`, 24h)(quality-bar EXC-08 P0) | **A11 · A63 (최우선 — BE-069 §7.3)** |
...
| 19 | 이탈·취소 시 abort 는 **클라이언트만 결과를 버릴 뿐** 서버 도달 여부를 보장하지 않는다 — **발급에서는 이것이 #8 과 같은 뿌리다**: abort 된 발급이 서버에 도달했으면 **평문을 못 본 유령 키**가 생긴다 | **A63 (BE-069 §7.3)** |
