...
| FS-070-SEC-03 | 제공자 카드 (×3) | **브랜드 글리프 + 제목(펼침/접힘 버튼) + 사용 스위치** + (펼침 시) 발급 위치 안내 · Client ID(제공자별 라벨) · Client Secret · Redirect URI(제공자별 라벨) + 연결 테스트 행. **설정은 접힘/펼침**(disclosure) — 켜면 펼치고 끄면 접는다 |
...
| FS-070-EL-005 | FS-070-SEC-03 | 제공자 카드 제목 (펼침/접힘 버튼) | 버튼 | `CardTitle action={<ToggleSwitch/>}`. 제목은 **disclosure 버튼**이다 — `ProviderBrandIcon`(모노크롬 브랜드 글리프) + `providerLabel`('Google' · '카카오' · '네이버') + chevron(펼치면 90° 회전). `aria-expanded` + `aria-controls`→설정 영역(EL-005.2). 클릭 시 펼침/접힘 토글 | O | **브랜드 색은 토큰에 없어** 하드코딩 금지 → 모양(글리프)으로 구분한다(`currentColor`). `providerLabel`이 못 찾으면 id 를 그대로 돌려준다 |
| FS-070-EL-005.1 | FS-070-SEC-03 | 발급 위치 안내 | 텍스트 | 각 제공자 설정 맨 위 muted 박스 '발급 위치' + `providerConsoleHint(provider)` — 'Google Cloud Console → API 및 서비스 → 사용자 인증 정보' · 'Kakao Developers → 내 애플리케이션 → 카카오 로그인' · 'NAVER Developers → 내 애플리케이션 → API 설정' | — | 두 창(콘솔·이 화면)을 오가는 운영자가 어디를 열어야 하는지 짚어 준다 |
| FS-070-EL-005.2 | FS-070-SEC-03 | 설정 접힘/펼침 영역 | 표시 | 발급 위치·Client ID·Client Secret·Redirect URI·연결 테스트를 감싼 영역(`id=oauth-<provider>-config`). 접힘은 `display:none` — **`hidden` 속성 대신 display 로 토글**(내부 `display:flex` 가 `hidden` 을 덮으므로). 기본 펼침 = `value.enabled`, 켜짐이 바뀌면 동기화하고 제목 버튼으로 수동 여닫기도 가능 | — | 비표시 규칙. **접혀도 DOM 에 남는다**(display:none) — 시크릿 필수 검증 회귀 테스트가 querySelector 로 찾으므로 그대로 통과. 켜짐이 아닌 제공자는 검증도 안 되므로 접힌 필수 필드가 숨은 오류를 만들지 않는다 |
...
| FS-070-EL-007 | FS-070-SEC-03 | Client ID 입력 | 입력 | `FormField htmlFor="oauth-<provider>-client-id"` **라벨이 제공자별로 갈린다**(`providerClientIdLabel`): Google·네이버 'Client ID', **카카오 'REST API 키'**(콘솔 용어에 맞춘다 — 'Client ID' 라고만 적으면 카카오 콘솔에서 못 찾는다). **`required={value.enabled}`**, 힌트 `'<라벨> 콘솔의 '<필드라벨>' 값을 붙여넣으세요.'`. 자식이 네이티브 `<input type="text">` 라 `FormField` 가 `aria-required` 를 주입한다. `maxLength=200`(`CLIENT_ID_MAX`). `aria-invalid`+`aria-describedby` 짝 | O | 켠 경우에만 검증: 비면 `'<라벨> Client ID를 입력하세요.'` · 200자 초과 시 `'Client ID는 200자를 넘을 수 없습니다.'`(`validation.ts` — **검증 문구는 여전히 'Client ID' 이나 필드 라벨은 콘솔 용어**) |
...
| FS-070-EL-009 | FS-070-SEC-03 | Redirect URI 입력 | 입력 | `FormField htmlFor="oauth-<provider>-redirect"` **라벨이 제공자별로 갈린다**(`providerRedirectLabel`): Google '승인된 리디렉션 URI' · 카카오 'Redirect URI' · **네이버 'Callback URL'**. **`required={value.enabled}`**, 힌트 `'이 주소를 <라벨> 콘솔의 '<필드라벨>'에도 똑같이 등록해야 합니다.'`. 자식이 네이티브 `<input type="url" inputMode="url">` 라 `aria-required` 가 주입된다. `aria-invalid`+`aria-describedby` 짝 | O | **`maxLength` 가 없다**(§7 #11). 검증은 EL-009.1 |
