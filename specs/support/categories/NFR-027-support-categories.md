...
> **`4b805ad` 재판정: P0 gap 이 5 → 3 으로 줄었다.** 뒤집힌 2건: **STATE-01**(F3b 가 `isFetching` 직결을 끊고 `firstLoading`/`refreshing` 파생을 넣었다 — `CategoriesPage.tsx:176-178`) · **A11Y-11**(F3a 의 `withAriaRequired` 런타임 주입 — `FormField.tsx:50-56`, 이 모달의 유일한 required 컨트롤이 네이티브 `<input>` 이라 주입 대상이다). **EXC-04 는 범위가 좁아졌을 뿐**(유령 저장 해소, 동시성 토큰·conflict UI 미해소) **이라 gap 을 유지했다.**
>
> **P0 gap 3건 — quality-bar §How to use 기준 '배치 실패' 사유다.** 판정 집합이 NFR-024(포트폴리오 카테고리)와 **여전히 완전히 동일하다** — 두 화면이 같은 골격(`useCrudListQuery` + 저수준 훅 모달 + `createStoreAdapter`)을 공유하기 때문이다. 다만 3건의 성격이 F2 때와 달라졌다: 예전엔 '공용 계층에 그 기능이 아예 없다'였지만, 이제 **셋 다 앱에 기능·선례가 실재하는데 이 화면이 소비하지 않는 것**이다 — EXC-03 은 `useRouteWritePermissions`(7곳 소비, 최근접 선례 `products/categories/ProductCategoriesPage.tsx:181`), EXC-08 은 `submitLockRef`+`WriteContext.idempotencyKey`(정본 `useCrudForm`), EXC-04 는 `useCrudForm` 의 충돌 다이얼로그. **동시성 토큰만이 도메인 타입 변경(BE-027)을 요구하는 진짜 앱 밖 항목이다.** 한 곳을 고치면 두 화면이 함께 낫는다 — §5 의 '범위' 열 참조.
...
- grep 근거: `buttonStyle(` (COMP-01) · `useRouteWritePermissions` (EXC-03 — 이 화면 0 / 앱 전역 7) · `submitLockRef` (EXC-08 — 이 화면 0) · `prefers-reduced-motion` in `ToggleSwitch.css` (MOTION-03 — 0건)
...
- [x] **판정 기준일을 `2026-07-17 · HEAD = 4b805ad` 로 갱신하고 F3a·F3b·통합 이후 코드로 전수 재확인했다.** 뒤집힌 P0 **2건**(STATE-01 · A11Y-11)을 pass 로 올리고 §2.1·§5 를 함께 갱신했다. **EXC-04 는 범위만 좁아졌으므로 gap 을 유지했다**(유령 저장 해소 ≠ 동시성 토큰 충족 — 이 구분을 흐리지 않았다). EXC-03·EXC-08 은 '앱에 기능이 없다' → '이 화면이 소비하지 않는다' 로 **사유만** 바꿨다. **NFR-024 와의 판정 집합 일치는 유지된다**
