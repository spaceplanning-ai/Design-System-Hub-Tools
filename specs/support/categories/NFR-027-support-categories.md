...
| EXC-03 | EXC | 직접 | **부분 미충족.** ① route-level read 게이팅은 **상속으로 충족** — `RequirePermission` 이 `<Outlet>` 을 감싸(`AppShell.tsx:526-528`) read 권한 없는 deep-link 에 `ForbiddenScreen` 을 렌더하고, 리소스는 `resourceIdForPath` 가 nav 리프(`/support/categories`)에서 파생한다. ② **write-action 게이팅이 없다** — `useRouteWritePermissions()`(`RequirePermission.tsx:45-52`)는 **더 이상 소비자 0 이 아니다 — 6곳이 소비하는데**(products 3 · settings 3) **이 화면은 그 밖이다**(`apps/admin/src/pages/support/**` 전수 grep = 0). 이 화면의 '유형 추가'(`:224`)·행 '수정'(`:126`)·행 '삭제'(`:135`)는 쓰기 권한과 무관하게 렌더·클릭된다 | `remove` 권한이 꺼진 역할로 `/support/categories` 진입 → 삭제 버튼이 **여전히 보이고 눌린다**(현재 fail). 기대: 버튼 미렌더/비활성. read 권한을 끄면 `ForbiddenScreen` → **통과**(①) | **gap** |
...
>
> **P0 gap 4건 — quality-bar §How to use 기준 '배치 실패' 사유다.** **판정 집합이 NFR-024(포트폴리오 카테고리)와 여전히 완전히 동일하다** — 두 화면이 같은 골격(`useCrudListQuery` + 저수준 훅 모달 + `createStoreAdapter`)을 공유하기 때문이며, **`PortfolioCategoryFormModal.tsx:104` 도 latch 폭발 반경 9곳에 함께 들어 있어 FEEDBACK-06 도 같이 뒤집힌다.** 4건의 성격: **FEEDBACK-06 은 DS(`packages/ui`) 소유의 신규 회귀**이고, 나머지 3건은 **앱에 기능·선례가 실재하는데 이 화면이 소비하지 않는 것**이다 — EXC-03 은 `useRouteWritePermissions`(최근접 선례 `products/categories/ProductCategoriesPage.tsx:179`), EXC-08 은 `submitLockRef`+`WriteContext.idempotencyKey`(정본 `useCrudForm`), EXC-04 는 `useCrudForm` 의 충돌 다이얼로그. **동시성 토큰만이 도메인 타입 변경(BE-027)을 요구하는 진짜 앱 밖 항목이다.** 한 곳을 고치면 두 화면이 함께 낫는다 — §5 의 '범위' 열 참조.
...
| 3 | EXC-03 | **P0** | 쓰기 액션 게이팅 부재 — **`useRouteWritePermissions`(`RequirePermission.tsx:45`)는 이제 6곳이 소비하지만**(products 3 · settings 3) 이 화면은 그 밖이다. 읽기 전용 역할이 추가·수정·삭제 버튼을 그대로 보고 누른다. **최근접 선례가 같은 taxonomy+모달 패턴인 `products/categories/ProductCategoriesPage.tsx:179`** 이다. route read 게이팅(①)은 충족 | **이 화면**(공용 훅 + 6곳 선례 존재) | UI 기획 쪽 변경 요청 |
...
- grep 근거: `buttonStyle(` (COMP-01) · `useRouteWritePermissions` (EXC-03 — 이 화면 0 / 앱 전역 6. 형제 훅 `useRouteCan` 직접 소비 1건(`logs/components/LogListShell.tsx:115`)까지 더하면 7) · `submitLockRef` (EXC-08 — 이 화면 0) · **`prefers-reduced-motion` in `ToggleSwitch.css` (MOTION-03 — ⚠ 이제 1건: `:79-84`. 이전 기준의 '0건' 은 낡았다)** · **`setClosing(false)`·`closingRef.current = false` in `Modal.tsx` (FEEDBACK-06 — 0건 = latch 리셋 부재의 근거, §5 #7)**
