...
| STATE-04 | STATE | 직접 | **미충족(선택 해제 조항).** clamp 조항은 페이지네이션이 없어 해당 없으나 **'page/filter/keyword 변경 시 이제 숨겨진 행의 선택은 해제한다' 를 위반**한다. `DownloadListPage.tsx:68-70` 의 `category`·`visibility`·`keyword` setter 가 `controller.clear()` 를 부르지 않는다(`onChange={setKeyword}` `:141`, `setCategory(...)` `:149`, `setVisibility(next)` `:165`). 공용 훅이 이 책임을 **호출부 계약으로 명시**했는데도(`shared/ui/useRowSelection.ts:7-8` '페이지/필터가 바뀌면 호출부가 clear() 로 비운다 — 안 보이는 행이 선택된 채 남지 않게') 부르지 않는다. 게다가 일괄 삭제가 `[...selectedIds]` **전량**을 대상으로 하고 `visibleItems` 와 교집합을 취하지 않는다(`useCrudList.tsx:126`) — **안 보이는 행이 그대로 삭제된다** | 자료 4건 전체선택 → 카테고리 '브로슈어' 로 필터(1건만 보임) → SelectionBar 가 여전히 '4건 선택됨' → '선택 4건 삭제' → **화면에 없던 3건까지 삭제된다**. 기대: 필터 변경 시 `selectedIds` 리셋 | **gap** |
| TOKEN-01 | TOKEN | 직접 | 이 화면의 style object 전량이 `var(--tds-*)` 만 참조한다 — `DownloadListPage.tsx:40-58` · `DownloadFormPage.tsx:39-90` · `FileUploadField.tsx:31-117` · `DownloadPreview.tsx:9-53`. 하드코딩 hex 0건 · px 리터럴 0건 · border/outline 키워드 0건. 폭 해킹은 `calc(var(--tds-space-6) * 3.5)` 형태로 토큰 경유(TOKEN-08 P1 대상이지 TOKEN-01 위반은 아니다) | `pages/support/downloads/**` 에 `#[0-9a-f]{3,6}` · `[1-9]px` · `(outline\|border): (thin\|medium\|thick)` grep = 0 | pass |
...
| TOKEN-05 | TOKEN | 상속 | display tier 표면: 폼의 `<h1 style={pageTitleStyle}>`(`DownloadFormPage.tsx:201`) · AppHeader 의 `<h1>`(`AppHeader.tsx:101`) · `CardTitle` ×2 · `DownloadPreview` 의 `titleStyle`(`title-sm` 토큰 경유, `:31-38`). 전부 semantic 토큰을 소비하며 primitive 를 손으로 재현하지 않는다 | tokens/AppHeader 판정에 따른다 | 종속 |
| COMP-10 | COMP | 직접 | **미충족.** 검색 입력이 실재하는데(`DownloadListPage.tsx:140-145`) `onChange={setKeyword}` 로 **매 키 입력을 즉시 커밋**한다. 디바운스 없음 · `isComposing`/`compositionend` 처리 없음. 공용 `useDebouncedSearch`(`shared/crud/useDebouncedSearch.ts` — 250ms 디바운스 + `isComposing` + `composingRef` 이중 판정)가 있는데 **이 화면이 쓰지 않는다**. DS `SearchField` 는 조합을 다루지 않는 dumb controlled input 이다(`packages/ui/.../SearchField.tsx` — `onChange` 값 콜백만). **완화**: 필터가 클라이언트 `useMemo`(`:81-84`)라 **query 발행이 0건** — 자모당 네트워크 요청·out-of-order 응답 문제는 발생하지 않는다. 남는 것은 ①(조합 중 부분 문자열 커밋)이며, 그 결과가 네트워크가 아니라 **표의 재필터**로 나타난다 | '홍길동' 을 IME 로 입력 → `keyword` 가 'ㅎ'→'호'→'홍'→'홍ㄱ'… 로 매 자모 커밋되어 **표가 자모마다 재필터되며 깜빡인다**(6회). 기대: 조합 완료 후 1회 | **gap** |
...
| A11Y-11 | A11Y | 직접 | **부분 미충족.** 같은 폼 안에서 배선 규칙이 갈린다 — **제목**(`:212-227`)은 `aria-invalid={errors.title !== undefined}` + `aria-describedby={errors.title !== undefined ? errorIdOf('download-title') : undefined}` 로 **정확히 짝지어져 있고**, **`FileUploadField`**(`:183`)도 `describedBy = invalid ? errorIdOf(id) : hasFile ? undefined : hintIdOf(id)` 로 오류/힌트를 정확히 가른다. **그러나 버전 입력**(`:249-258`)은 **`aria-invalid` 도 `aria-describedby` 도 세우지 않는다.** `FormField` 는 `hint` 와 `error` 를 받아 `<p id={hintIdOf('download-version')}>` / `<p id={errorIdOf('download-version')} role="alert">` 를 렌더하지만(`packages/ui/.../FormField.tsx`) **id 만 노출하고 배선은 호출부 책임이다** — 그래서 20자 초과 시 오류 `<p>` 가 화면에 뜨는데 **입력은 그것을 참조하지 않고 invalid 로 표시되지도 않는다**. hint 도 영영 연결되지 않는다. *(짝 없는 `aria-invalid` 는 0건이므로 grep 기반 acceptanceCheck 는 통과한다 — 이 결함은 반대 방향이다: 오류가 있는데 `aria-invalid` 가 없다)* | 버전에 21자 입력 후 제출 → 화면에 '버전은 20자를 넘을 수 없습니다.' 가 보이지만 RTL 로 `input.getAttribute('aria-describedby')` = `null`, `aria-invalid` = `null`. 제목에 같은 절차 → 둘 다 정상(대조군). 기대: 오류 시 `aria-invalid="true"` + `aria-describedby="download-version-error"`, 정상 시 `aria-describedby="download-version-hint"` | **gap** |
...
| MOTION-01 | MOTION | 상속 | Modal enter/exit 표면이 실재한다 — 삭제 `ConfirmDialog` ×2 · `FormConflictDialog` · 미저장 `ConfirmDialog`. 전부 DS Modal 을 상속한다 | DS Modal 판정에 따른다 | 종속 |
| MOTION-02 | MOTION | 상속 | toast exit 표면이 실재한다 — 저장·삭제·토글 결과가 전부 토스트다 | ToastProvider/Toast 판정에 따른다 | 종속 |
| MOTION-03 | MOTION | 상속 | reduced-motion 게이트 표면: `ToggleSwitch`(목록 행마다 1개 `:126` + 폼 1개 `:264`) · Modal · Toast · 스켈레톤. **DS 현황: `packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.css:56` 이 `transition: transform` 을 선언하고 그 파일에 `prefers-reduced-motion` 블록이 없다**(코드 확인) — quality-bar MOTION-03 이 지목한 위반이 남아 있다. 이 화면은 소비자 | DS ToggleSwitch.css 판정에 따른다. 재현: reduced-motion 활성 후 토글 → knob 이 여전히 이동 애니메이션 | 종속 |
...
| IA-02 | IA | 직접 | **미충족(하위 라우트).** 목록 `/support/downloads` 는 nav 잎이라(`nav-config.ts:170`) `findNavLabel` exact 분기가 '자료실' 을 준다 — 정상. **그러나 `/new`·`/:id/edit` 는 잎이 아니다** → exact 실패 → **branch 폴백**(`nav-config.ts:257-262` `pathname.startsWith(item.basePath)`)이 `/support` 브랜치를 잡아 AppHeader h1 에 **'고객센터'** 를 그린다(`AppHeader.tsx:92`·`:101`). 동시에 폼이 자체 `<h1>자료 등록</h1>`(`DownloadFormPage.tsx:201`)을 그린다. **결과: 한 화면에 h1 이 둘이고 상단(더 눈에 띄는)의 것이 '고객센터' 라는 모호한 브랜치 라벨**이다 — quality-bar IA-02 가 `/content/notices/new` 사례로 지목한 바로 그 모순이며, F2 가 `AppHeader.tsx` 를 손댔음에도(display tier 소비로 변경) `findNavLabel` 호출은 그대로다 | `/support/downloads/new` 진입 → AppHeader h1 = '고객센터', 본문 h1 = '자료 등록'. `document.querySelectorAll('h1').length` = 2. 기대: 가시 primary title 이 '자료 등록' 이고 title 소스가 하나 | **gap** |
...
| IA-13 | IA | 직접 | **미충족.** list query state 가 **컴포넌트 `useState` 에만** 존재한다 — `category`·`visibility`(`DownloadListPage.tsx:68-69`) · `keyword`(`:70`). `useSearchParams` 미사용 · 공용 `useListState`(`shared/crud/useListState.ts`, 10개 화면이 소비) **미사용**. URL 이 `/support/downloads` 로 고정이라 필터를 걸어도 주소가 변하지 않는다. 결과: (a) 수정 폼에 들어갔다 '목록으로' 돌아오면 **필터·검색이 전부 초기화**된다(`navigate(LIST_PATH)` 가 새 마운트 → useState 초기값), (b) F5 하면 초기화, (c) '이 조건 좀 봐주세요' 하며 링크를 공유할 수 없다. **(a)는 이 화면에서 특히 아프다** — 행 클릭이 곧 수정 진입이라(FS-030-EL-008.10) 필터 → 행 클릭 → 저장 → 목록 복귀가 **주 운영 루프**인데 그때마다 필터가 날아간다 | 카테고리 '브로슈어' + 검색 'brochure' 설정 → URL 은 여전히 `/support/downloads` → 행 클릭해 수정 진입 → '목록으로' → **필터·검색이 초기화된 전체 목록**. 기대: 동일 필터 복원. URL 을 새 탭에 복사 → 필터 없는 목록 | **gap** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| pass | 7 | STATE-01 · STATE-02 · TOKEN-01 · IA-01 · IA-05 · EXC-08 · EXC-09 |
...
| **gap** | **8** | **STATE-04 · COMP-10 · FEEDBACK-04 · A11Y-11 · IA-02 · IA-04 · IA-13 · EXC-04** |
| **합계** | **30** | 7 + 13 + 2 + 8 = **30** ✓ |

> **P0 gap 8건 → quality-bar '배치 실패' 사유.** 심각도 순: **STATE-04**(안 보이는 행이 삭제된다 — 데이터 손실) · **FEEDBACK-04**(입력이 조용히 사라진다 — quality-bar 가 '용납 불가' 로 부르는 silent 손실) · **EXC-04**(lost update) · **IA-13**(주 운영 루프에서 필터 소실).
...
| EXC-12 | P1 | **충족.** `loadFailure: 'not-found' \| 'error'` 로 갈리고(`useCrudForm.ts:138-143` `isNotFound`) 화면이 **404 는 '다시 시도' 를 주지 않는다**(`DownloadFormPage.tsx:174-178` — `loadFailure === 'error'` 일 때만 렌더). 무한 스피너 없음 | `?status=detail:404` → '자료 찾을 수 없습니다…' + '목록으로'만. `?status=detail:500` → '불러오지 못했습니다' + '다시 시도' + '목록으로' | pass |
...
| ERP-13 | P1 | 조사 헬퍼 미적용 — '**자료을(를)** 등록했습니다'(`useCrudForm.ts:215` 템플릿 + `ENTITY_LABEL='자료'`), '자료 찾을 수 없습니다'(조사 누락, `DownloadFormPage.tsx:171`), 리터럴 '을(를)'(`useCrudList.tsx:157`). **'자료' 는 받침이 없어 '를' 이 맞는데 '을(를)' 이 렌더된다** | 자료 등록 성공 → 토스트 '자료을(를) 등록했습니다.' 사용자 대상 문자열의 `'을(를)'` grep > 0 | gap |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | **STATE-04** | **P0** | 필터·검색 변경 시 `controller.clear()` 미호출(`DownloadListPage.tsx:68-70`) + 일괄 삭제가 `visibleItems` 와 교집합을 취하지 않음(`useCrudList.tsx:126`) → **안 보이는 행이 삭제된다**. 공용 훅의 명시적 계약 위반(`useRowSelection.ts:7-8`) | 이 화면(+`useCrudList` 의 교집합 방어는 전역 개선 후보) | A41 |
...
| 4 | **IA-13** | **P0** | 필터·검색이 `useState` 에만 존재(`:68-70`). `useListState` 미사용 → **행 클릭 → 저장 → 목록 복귀 주 루프마다 필터 소실** | 이 화면 | A41 |
| 5 | **IA-02** | **P0** | `/new`·`/:id/edit` 에서 AppHeader 가 branch 폴백으로 '고객센터' h1 을 그려 폼의 '자료 등록' h1 과 충돌 — h1 이 둘 | 앱 전역(`findNavLabel`) + 이 화면 | A40 |
| 6 | **IA-04** | **P0** | Pagination 부재 · 전량 렌더 · 상한 미정. 클라이언트 필터라 페이징 도입 시 필터를 서버로 옮겨야 한다 | 이 화면 · `CrudListShell` · 백엔드 | A41 · A63 · A11 |
| 7 | **COMP-10** | **P0** | 검색에 디바운스·IME 조합 처리 없음(`:141`). `useDebouncedSearch` 미사용 → 자모마다 재필터 | 이 화면 | A41 |
| 8 | **A11Y-11** | **P0** | 버전 입력(`:249-258`)이 `aria-invalid`·`aria-describedby` 미배선 → 오류·힌트가 AT 에 미연결. 같은 폼의 제목 필드는 정상 — **규칙이 필드마다 갈린다** | 이 화면 | A41 |
...
| 17 | ERP-13 | P1 | '자료을(를)' 조사 오류 · 리터럴 '을(를)' | 이 화면 · `useCrudForm`·`useCrudList` 템플릿 | A41 |
...
| 22 | MOTION-03 | 종속 | DS `ToggleSwitch.css:56` reduced-motion off 부재 | DS | A40 |
