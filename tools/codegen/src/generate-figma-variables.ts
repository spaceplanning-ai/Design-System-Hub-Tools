 ...
 *   { collection: 'TDS Tokens', modes: ['light'],
 *     variables: [{ name, type, value }] }
 *
 * 테마는 라이트 단일 모드다(2026-07-20 다크 제거). 무료 Figma 플랜의 컬렉션당 1모드 제한과도
 * 이제 맞아떨어진다 — 종전 ensureLightDarkModes 의 addMode('dark') 는 그 플랜에서 throw 했다.
 *
 ...
 *  - 참조 토큰 {a.b.c} 는 체인 끝까지 해석해 raw 값을 value 에 기록하고, 참조 대상 Variable
 *    이름(a/b/c)을 alias 필드에 병기한다. Figma VARIABLE_ALIAS 는 대상 Variable ID(파일별
 *    부여)가 필요해 codegen 시점에 만들 수 없다 — 현행 플러그인은 value(raw)만 바인딩하며
 *    alias 는 추후 이름→ID 해석 후 승격하기 위한 메타데이터다.
...
/** 플러그인 ensureLightMode 가 보장하는 단일 모드 — 이름 고정 */
const MODES: readonly string[] = ['light'];
...
/** 원시(미해석) 값 */
function rawValueFor(table: TokenTable, tokenPath: string): unknown {
...
/** 참조 체인을 재귀 해석 — 배열/객체 내부 참조까지 처리 */
function resolveValue(table: TokenTable, raw: unknown, stack: string[]): unknown {
  if (typeof raw === 'string') {
    const full = FULL_REF_RE.exec(raw);
    if (full && full[1] !== undefined) {
      // 전체가 단일 참조 → 값 타입(hex/숫자/배열/객체)을 보존하며 재귀
      return resolveToken(table, full[1], stack);
    }
    // 문자열 내 부분 참조 → 해석 값을 문자열로 치환
    return raw.replace(REF_RE, (_m, refPath: string) =>
      String(resolveToken(table, refPath, stack)),
    );
  }
  if (Array.isArray(raw)) {
    return raw.map((item) => resolveValue(table, item, stack));
  }
  if (isPlainObject(raw)) {
    const out: Record<string, unknown> = {};
    for (const [key, sub] of Object.entries(raw)) out[key] = resolveValue(table, sub, stack);
    return out;
  }
  return raw;
}

function resolveToken(table: TokenTable, tokenPath: string, stack: string[]): unknown {
  if (stack.includes(tokenPath)) {
    throw new Error(`토큰 순환 참조: ${[...stack, tokenPath].join(' → ')}`);
  }
  return resolveValue(table, rawValueFor(table, tokenPath), [...stack, tokenPath]);
}
...
/** alias 조립 — 없으면 필드 자체를 생략 */
function buildAlias(alias: string | undefined): { alias: string } | Record<string, never> {
...
function expandComposite(
  ...
  const parentAlias = aliasNameFor(raw);

  const subAlias = (key: string): string | undefined => {
  ...
  for (const [key, sub] of Object.entries(obj)) {
    ...
    if (sub === undefined || sub === null || isPlainObject(sub)) continue;

    const subName = `${name}/${kebab(key)}`;
    const subType = COMPOSITE_KEY_TYPES[key] ?? figmaTypeFor(undefined, sub);
    specs.push({
      name: subName,
      type: subType,
      value: convertValue(subName, subType, sub, undefined),
      ...buildAlias(subAlias(key)),
    });
  }
  return specs;
}
...
function tokenToVariableSpecs(table: TokenTable, token: FlatToken): FigmaVariableSpec[] {
  ...
  const raw = rawValueFor(table, token.path);
  const resolved = resolveToken(table, token.path, []);

  // typography 등 합성 객체 → 서브 Variable 전개
  if (isPlainObject(resolved)) {
    return expandComposite(name, resolved, raw);
  }

  const figmaType = figmaTypeFor(token.type, resolved);
  return [
    {
      name,
      type: figmaType,
      value: convertValue(name, figmaType, resolved, token.type),
      ...buildAlias(aliasNameFor(raw)),
    },
  ];
}
...
export function generateFigmaVariables(doc: Record<string, unknown>): GeneratedFile {
  const table: TokenTable = { flat: flattenTokens(doc) };
